...
class GeneticAlgorithmScheduler:
    ...
    def simulate_schedule(self, tasks, processors, security_levels):
        """Simulate the scheduling execution and return makespan and local slack times."""
        ...
        for task in sorted_tasks:
            ...
            finish_time = est + task.execution_times[processor_id - 1] + task.soN
            ...
            next_task_start = min(
                (self.tasks[succ].start_time for succ in task.successors if succ in self.tasks and self.tasks[succ].start_time is not None),
                default=self.deadline
            )

            local_slack_times[task.task_id] = max(0, next_task_start - finish_time)
    ...
    def calculate_est(self, task, processor_id, tasks, messages):
        """Calculate Earliest Start Time (EST) for a task."""
        ...
        for pred_id in task.predecessors:
            # Find the predecessor task by ID
            pred_task = next((t for t in tasks if t.task_id == pred_id), None)
            ...
            if pred_task.assigned_processor == processor_id:
                ...
                if message:
                    comm_time = message.size / self.network.bandwidth
    ...
    def plot_schedule(self):
        ...
        for entry in self.best_schedule:
            ...
            color = colors[(processor - 1) % len(colors)]
            ax.barh(processor, finish_time - start_time, left=start_time, height=0.6, 
                    color=color, alpha=0.75, edgecolor='black')
            
            # Add task label
            ax.text(start_time + (finish_time - start_time) / 2, processor, f"T{task_id}", 
    ...
    def plot_security_levels(self):
        """Plot the security levels assigned to messages."""
        # Get security services and their strengths
        ...
        for message in self.messages:
            ...
            confidentiality_levels.append(self.security.strengths['confidentiality'][message.assigned_security['confidentiality']])
            integrity_levels.append(self.security.strengths['integrity'][message.assigned_security['integrity']])
            authentication_levels.append(self.security.strengths['authentication'][message.assigned_security['authentication']])
        
        # Create bar chart
        fig, ax = plt.subplots(figsize=(12, 6))
        
        x = np.arange(len(message_ids))
        width = 0.25
        
        ax.bar(x - width, confidentiality_levels, width, label='Confidentiality', color='#3498db')
        ax.bar(x, integrity_levels, width, label='Integrity', color='#2ecc71')
        ax.bar(x + width, authentication_levels, width, label='Authentication', color='#e74c3c')
    ...
    class Task:
        def __init__(self, task_id, execution_times, predecessors, message_sizes=None):
            self.task_id = task_id
            self.execution_times = execution_times
            self.predecessors = predecessors
            self.successors = []
            self.message_sizes = message_sizes or {}
            ...
            self.soN = 0  # Security overhead needed

    class Processor:
        def __init__(self, proc_id, comm_costs=None):
            self.proc_id = proc_id
            self.comm_costs = comm_costs or {}
            self.available_time = 0

    class Message:
        def __init__(self, id, source_id, dest_id, size, min_security=None, weights=None):
            self.id = id
            self.source_id = source_id
            self.dest_id = dest_id
            self.size = size  # Size in KB
            
            # Default minimum security requirements
            self.min_security = min_security or {
                'confidentiality': 0.5,
                'integrity': 0.5,
                'authentication': 0.5
            }
            
            # Default weights for each security service
            ...
            # Assigned security levels (protocol indices)
    ...
    class Network:
        def __init__(self, bandwidth):
            self.bandwidth = bandwidth  # in KB/time unit

    class SecurityService:
        def __init__(self):
            # Security strengths for each protocol (indices correspond to protocol numbers)
            self.strengths = {
                'confidentiality': [0.0, 0.5, 0.75, 0.95, 1.0],  # Protocol 0 (no security) to 4
                'integrity': [0.0, 0.6, 0.8, 0.9, 1.0],
                'authentication': [0.0, 0.7, 0.85, 0.95, 1.0]
            }
            
            # Security overheads for each protocol on each processor
            # Format: overheads[service][protocol][processor]
            self.overheads = {
                'confidentiality': [
                    [0, 0, 0],  # Protocol 0 (no security)
                    [0.2, 0.3, 0.25],  # Protocol 1
                    [0.4, 0.5, 0.45],  # Protocol 2
                    [0.6, 0.7, 0.65],  # Protocol 3
                    [0.8, 0.9, 0.85]   # Protocol 4
                ],
                'integrity': [
                    [0, 0, 0],
                    [0.15, 0.2, 0.18],
    ...
    def plot_gantt_chart(schedule, processors):
        ...
        for entry in schedule:
            ...
            color = colors[(task_id - 1) % len(colors)]
            ax.barh(processor, finish_time - start_time, left=start_time, height=0.6, 
                    color=color, alpha=0.75, edgecolor='black')
        ...
        # Set y-ticks to processor IDs
        ax.set_yticks(range(1, len(processors) + 1))
    ...
    def main():
        """Main function to execute the genetic algorithm scheduling"""
        # Define processors
        processors = [
            Processor(1, {2: 5, 3: 10}),
            Processor(2, {1: 5, 3: 7}),
            Processor(3, {1: 10, 2: 7})
        ]

        # Define tasks (task_id, execution_times on each processor, predecessors)
        tasks = [
            Task(1, [10, 12, 14], []),
            Task(2, [15, 10, 20], [1]),
            Task(3, [12, 14, 8], [1]),
        ...
        # Define network
        network = Network(bandwidth=100)  # 100 KB/time unit
        ...
        if makespan:
            print(f"Successfully found a schedule with makespan {makespan} and security utility {security_utility}")
        else:
            print("Failed to find a valid schedule within the deadline")
