...
class Scheduler:
    ...
    def calculate_est(self, task, processor):
        ...
        for pred_id in task.predecessors:
            ...
            if pred_task.assigned_processor == processor:
                ...
                security_overhead = self.calc_security_involved(
                    message, 
    ...
    def calculate_security_overhead(self, task_id, processor_count=2):
        """
        Calculate security overhead for a given task across all processors.
        
        Args:
            task_id: ID of the task to calculate overhead for
            processor_count: Number of processors in the system (default: 2)
        
        Returns:
            Average security overhead across all processors
        """
        total_overhead = 0
        ...
        for processor in range(processor_count):
            ...
            # Debug print to check messages
            print(f"Looking for messages with destination/source {task_id}")
            print(f"All messages: {[(msg.source_id, msg.dest_id) for msg in self.messages]}")
            
            # Calculate overhead for receiving messages (incoming edges)
            for message in self.messages:
                if message.dest_id == task_id:
                    print(f"Found incoming message from {message.source_id} to {task_id}")
                    
                    # Find minimum security protocols that meet requirements
                    conf_req = message.min_security['confidentiality']
                    int_req = message.min_security['integrity']
                    auth_req = message.min_security['authentication']
                    
                    # Find minimum confidentiality protocol
                    conf_protocol = 1
                    for i, strength in enumerate(self.security_service.strengths['confidentiality'], 1):
                        if strength >= conf_req:
                            conf_protocol = i
                            break
                    
                    # Find minimum integrity protocol
                    int_protocol = 1
                    for i, strength in enumerate(self.security_service.strengths['integrity'], 1):
                        if strength >= int_req:
                            int_protocol = i
                            break
                    
                    # Find minimum authentication protocol
                    auth_protocol = 1
                    for i, strength in enumerate(self.security_service.strengths['authentication'], 1):
                        if strength >= auth_req:
                            auth_protocol = i
                            break
                    
                    # Calculate overhead
                    auth_overhead = self.security_service.overheads['authentication'][auth_protocol][processor]
                    conf_overhead = self.security_service.overheads['confidentiality'][conf_protocol][processor]
                    int_overhead = self.security_service.overheads['integrity'][int_protocol][processor]
                    
                    message_overhead = auth_overhead + message.size / (conf_overhead + int_overhead)
                    task_overhead += message_overhead
                    print(f"Added incoming overhead: {message_overhead}")
            
            # Calculate overhead for sending messages (outgoing edges)
            for message in self.messages:
                if message.source_id == task_id:
                    print(f"Found outgoing message from {task_id} to {message.dest_id}")
                    ...
                    message_overhead = auth_overhead + (message.size / (conf_overhead + int_overhead))
                    task_overhead += message_overhead
                    print(f"Added outgoing overhead: {message_overhead}")
            
            total_overhead += task_overhead
        
        # Return average overhead across all processors
        return total_overhead / processor_count
    
    def calc_security_involved(self, message, source_processor, dest_processor):
        """
        Calculate security overhead for sending a message from source processor to destination processor.
        
        Args:
            message: The message being sent between processors
            source_processor: The processor ID where the message originates
            dest_processor: The processor ID where the message is received
            
        Returns:
            Security overhead for sending the message between these processors
        """
        # Get security requirements from the message
        conf_req = message.conf_req if hasattr(message, 'conf_req') else message.min_security['confidentiality']
        int_req = message.int_req if hasattr(message, 'int_req') else message.min_security['integrity']
        auth_req = message.auth_req if hasattr(message, 'auth_req') else message.min_security['authentication']
        ...
        # Calculate overhead at the source processor
        source_auth_overhead = self.security_service.overheads['authentication'][auth_protocol][source_processor - 1]
        source_conf_overhead = self.security_service.overheads['confidentiality'][conf_protocol][source_processor - 1]
        source_int_overhead = self.security_service.overheads['integrity'][int_protocol][source_processor - 1]
        
        # Calculate overhead at the destination processor
        dest_auth_overhead = self.security_service.overheads['authentication'][auth_protocol][dest_processor - 1]
        dest_conf_overhead = self.security_service.overheads['confidentiality'][conf_protocol][dest_processor - 1]
        dest_int_overhead = self.security_service.overheads['integrity'][int_protocol][dest_processor - 1]
        
        # Calculate message size
        message_size = message.size if hasattr(message, 'size') else 1.0
        
        # Calculate total overhead (source overhead + destination overhead)
        source_overhead = source_auth_overhead + (message_size / (source_conf_overhead + source_int_overhead))
        dest_overhead = dest_auth_overhead + (message_size / (dest_conf_overhead + dest_int_overhead))
    ...
    def schedule_tasks(self):
        ...
        for task in sorted_tasks:
            ...
            for processor in self.processors:
                ...
                print(task)
                eft = est + task.execution_times[processor.proc_id - 1] + calculate_security_overhead(task, 2)
                print(f"Processor {processor.proc_id}: EST={est}, EFT={eft}")
    ...
    def schedule_tasks(self):
        ...
        for task in sorted_tasks:
            ...
            if best_processor:
                ...
                # Calculate security overhead only for inter-processor communications
                ...
                for pred_id in task.predecessors:
                    ...
                    if pred_task and pred_task.is_scheduled and pred_task.assigned_processor != processor.proc_id:
                        message = self.get_message(pred_id, task.task_id)
                        if message:
                            sec_overhead = self.calc_security_involved(
                                message, 
                                pred_task.assigned_processor, 
                                processor.proc_id
                            )
                            total_security_overhead += sec_overhead
                            # Add the security overhead to the EFT
                            eft += sec_overhead
