...
class ImprovedRFSHIELD(RFSHIELD):
    ...
    def calculate_adaptive_bcr(self, message, service, current_level):
        ...
        on_critical_path = (message.source_id in self.critical_path_tasks and 
                        message.dest_id in self.critical_path_tasks)
        critical_path_factor = 0.7 if on_critical_path else 1.0
        
        # Deadline proximity factor - as we get closer to deadline, favor security less
        current_makespan = max(task.finish_time for task in self.tasks if task.is_scheduled)
        deadline_proximity = current_makespan / self.deadline  # 0.0 to 1.0
        
        # Adjust utility weight based on proximity to deadline
        # If we're close to deadline, reduce weight on security utility
        adjusted_utility_weight = self.adaptive_utility_weight * (1 - (deadline_proximity ** 2))
        
        # Level proximity factor - higher payoff for completing a service
        max_level = len(self.security.strengths[service]) - 1
        level_proximity = next_level / max_level  # 0.0 to 1.0
        level_bonus = 1.0 + (0.5 * level_proximity)  # 1.0 to 1.5
        
        # Calculate BCR with adjusted factors
        bcr = (utility_gain * adjusted_utility_weight * level_bonus) / (overhead_increase * critical_path_factor)
    ...
    def enhance_adaptive_security(self):
        """Enhanced security upgrade algorithm with adaptive confidence thresholds and rollback."""
        # Initialize BCR heap with all possible upgrades
        ...
        # Batching approach - evaluate upgrades in small batches
        batch_size = 3
        current_batch = []
        batches_since_improvement = 0
        max_no_improvement_batches = 5  # Early stopping after no improvement
        
        # Process upgrades in order of highest adaptive BCR
        while self.bcr_heap and batches_since_improvement < max_no_improvement_batches:
            # Build a batch of potential upgrades
            batch_upgrades_attempted = 0
            current_batch = []
            
            while len(current_batch) < batch_size and self.bcr_heap and batch_upgrades_attempted < batch_size * 3:
                # Get the upgrade with the highest BCR
                bcr, (message_id, service, current_level) = heapq.heappop(self.bcr_heap)
                ...
                will_succeed = True  # Default to trying if prediction unclear
                ...
                batch_upgrades_attempted += 1
                
                # Skip if ML predicts failure with high confidence
                if not will_succeed:
                    # Double-check for high utility gains despite prediction
                    utility_gain = self.calculate_security_utility_gain(message, service, current_level, next_level)
                    
                    # Try anyway if gain is significant or it completes a service's security
                    if (utility_gain > 0.1 * best_security_utility or 
                        next_level == max_level):
                        print(f"🔍 High utility upgrade detected! Trying despite prediction.")
                    else:
                        continue
                
                # Add to current batch
                current_batch.append((message, service, current_level, next_level, bcr, prediction_confidence))
                
                if len(current_batch) >= batch_size:
                    break
            
            # If no more upgrades to try
            if not current_batch:
                break
            
            # Apply the batch of upgrades
            successful_upgrades = []
            
            # Save current state before batch
            pre_batch_configuration = self.save_current_security_configuration()
            
            # Try each upgrade in the batch
            for message, service, current_level, next_level, bcr, confidence in current_batch:
                # Try the upgrade
                print(f"\nTrying to upgrade {message.id} {service} from level {current_level} to {next_level} (BCR: {bcr:.4f}, Confidence: {confidence:.2f})")
                ...
                if success:
                    self.upgrade_successes += 1
                    successful_upgrades.append((message, service, current_level, next_level))
                    print(f"✅ Upgrade successful! New {service} level: {next_level}")
                    ...
                    # Update confidence threshold based on failure
                    self.confidence_thresholds[service] = min(0.8, self.confidence_thresholds[service] + 0.05)
                    print(f"Increased confidence threshold for {service} to {self.confidence_thresholds[service]:.2f}")
            
            # Evaluate the entire batch
            current_makespan = self.finalize_schedule()
            current_security_utility = self.calculate_security_utility()
            
            # Check if batch improved overall solution
            improvement = current_security_utility > best_security_utility and current_makespan <= self.deadline
            
            if improvement:
                best_security_utility = current_security_utility
                best_configuration = self.save_current_security_configuration()
                best_makespan = current_makespan
                upgrade_count += len(successful_upgrades)
                batches_since_improvement = 0
                
                print(f"📈 Batch successful! New security utility: {best_security_utility:.4f}")
                
                # Decrease confidence thresholds for successful services
                for _, service, _, _ in successful_upgrades:
                    self.confidence_thresholds[service] = max(0.3, self.confidence_thresholds[service] - 0.05)
                    print(f"Decreased confidence threshold for {service} to {self.confidence_thresholds[service]:.2f}")
                    
            else:
                # Rollback this batch if it didn't improve
                print(f"⚠️ Batch didn't improve solution. Rolling back.")
                self.restore_security_configuration(pre_batch_configuration)
