...
class QLSHIELD(HSMS):
    ...
    def __init__(self, tasks, messages, processors, security_service, deadline, 
                 episodes=1000, reward_scale=1000):
        super().__init__(tasks, messages, processors, security_service, deadline)
        ...
        self.best_schedule = None
        self.best_makespan = float('inf')
        self.bcr_heap = []  # For BCR phase
        self.upgrade_history = []  # Track security upgrades
    ...
    def train_ql_agent(self):
        ...
        if self.hsms_makespan:
            print("Pretraining from HSMS schedule...")
            self.pretrain_from_schedule(self.env, self.hsms_seed.schedule, q_table=self.q_table)
        ...
        for episode in range(self.episodes):
            ...
            while ready_tasks:
                ...
                current_makespan = max(scheduled_tasks.values())
                
                # Calculate reward
                if len(scheduled_tasks) == len(self.tasks):
                    # All tasks are scheduled, check if makespan meets deadline
                    if current_makespan <= self.deadline:
                        reward = self.reward_scale * (1 - current_makespan) / self.deadline
                    else:
                        reward = -self.reward_scale * (current_makespan/self.deadline - 1)
                else:
                    # Reward inversely proportional to task completion time
                    reward = self.reward_scale / (len(self.tasks) * 10)
            ...
            if scheduled_tasks:
                ...
                if makespan < best_makespan:  # Remove the deadline constraint here
        ...
        if best_schedule:
            ...
            if best_makespan <= self.deadline:
                ...
                # Still apply the best schedule for potential security upgrades
    ...
    def pretrain_from_schedule(env, schedule, q_table=None, replay_buffer=None, gamma=0.99):
        """
        Pretrain Q-Learning using the schedule from HSMS or SHIELD.

        Args:
            env: The environment object with get_state(), step(), and reset() methods.
            schedule: A list of dicts with keys ['task_id', 'processor', 'start_time', 'finish_time']
            q_table: Dictionary Q-table {state: {action: q_value}} (optional)
            replay_buffer: A deque for DQN pretraining (optional)
            gamma: Discount factor
        """
        # Reset environment to start of the schedule
        env.reset()
        ...
        for entry in sorted(schedule, key=lambda x: x['start_time']):
            ...
            state = env.get_state()
            action = (task_id, processor_id)
            
            # Simulate scheduling action in env
            next_state, reward, done, _ = env.step(action)
            
            # Update Q-table (tabular)
            if q_table is not None:
                if state not in q_table:
                    q_table[state] = {}
                if action not in q_table[state]:
                    q_table[state][action] = 0.0
                # Q-learning update (assumes future Q-values are 0 during pretrain)
                q_table[state][action] = reward  # Or use: += α * (reward + γ * max_q - q)

            # Add to replay buffer (DQN)
    ...
    def assign_minimum_security(self):
        ...
        for message in self.messages:
            # Initialize security levels to minimum (level 0)
            message.assigned_security = {
                'confidentiality': 0,
                'integrity': 0,
                'authentication': 0
