...
class QLSHIELD(HSMS):
    """Q-Learning based Security-aware Heterogeneous Scheduling with Deadline Constraints"""
    
    def __init__(self, tasks, messages, processors, security_service, deadline, 
             episodes=1000, reward_scale=1000, hsms=None, hsms_makespan=None, shield=None,shield_makespan=None):
        super().__init__(tasks, messages, processors, security_service, deadline)
        ...
        # Store HSMS and SHIELD solutions if provided
        self.hsms_schedule = None
        self.hsms_makespan = None
        self.shield_schedule = None
        self.shield_makespan = None
        
        if hsms:
            self.hsms = hsms
            self.hsms_makespan = hsms_makespan
            
        if shield:
            self.shield = shield
            self.shield_makespan = shield_makespan
        
    def get_hsms_solution(self):
        """Get HSMS solution if available"""
        if hasattr(self, 'hsms') and self.hsms:
            # Assuming self.hsms is an instance of HSMS class
            makespan, _ = self.hsms.run()
            if makespan:
                return self.hsms.schedule, makespan
        return None

    def get_shield_solution(self):
        """Get SHIELD solution if available"""
        if hasattr(self, 'shield') and self.shield:
            # Assuming self.shield is an instance of SHIELD class
            makespan, _ = self.shield.run()
            if makespan:
                return self.shield.schedule, makespan
        return None
    def set_prior_solutions(self, hsms_schedule=None, hsms_makespan=None, 
                       shield_schedule=None, shield_makespan=None):
        """Store HSMS and SHIELD solutions for pretraining"""
        if hsms_schedule:
            self.hsms_schedule = hsms_schedule
            self.hsms_makespan = hsms_makespan
            print(f"Stored HSMS solution with makespan {hsms_makespan}")
        
        if shield_schedule:
            self.shield_schedule = shield_schedule
            self.shield_makespan = shield_makespan
            print(f"Stored SHIELD solution with makespan {shield_makespan}")
            
    def pretrain_from_schedule(self, schedule,scheduler):
        """
        Pretrain Q-Learning using the schedule from HSMS or SHIELD.
        
        Args:
            schedule: A list of dicts with keys ['task_id', 'processor', 'start_time', 'finish_time']
        """
        print(f"Pretraining agent with {len(schedule)} task assignments...")
        
        # Need to simulate the scheduling process to build states and actions
        for task in self.tasks:
            task.is_scheduled = False
        ...
        # Track scheduled tasks and their finish times
        scheduled_tasks = {}  # task_id -> finish_time
        task_proc_map = {}    # task_id -> processor_id
        
        # Process tasks in order of start time from the provided schedule
        for entry in sorted(schedule, key=lambda x: x['start_time']):
            task_id = entry['task_id']
            processor_id = entry['processor']
            ...
            # Calculate the current state - which tasks are ready
            ready_tasks = self.compute_ready_tasks(scheduled_tasks)
            if task_id not in ready_tasks:
                continue  # Skip if this task is not ready yet
                
            # Sort ready tasks by priority (upward rank)
            ready_tasks.sort(key=lambda t_id: -self.get_task_by_id(t_id).priority)
            
            # Create state for the agent
            state = (task_id, tuple(ready_tasks))
            
            # The action is the processor assignment
            action = processor_id - 1  # Convert 1-indexed to 0-indexed for q-table
            
            # Calculate EST for the task on the chosen processor
            est = self.calculate_task_est(task_id, processor_id, scheduled_tasks, scheduled_tasks)
            
            # Calculate execution time and security overhead
            exec_time = current_task.execution_times[processor_id - 1]
            security_overhead = self.calculate_task_security_overhead(
                task_id, processor_id, scheduled_tasks, task_proc_map
            )
            
            # Calculate finish time
            finish_time = est + exec_time + security_overhead
            
            # Update processor availability
            processor = self.processors[processor_id - 1]
            processor.available_time = finish_time
            
            # Update task information for simulation
            ...
            # Prepare next state
            next_ready_tasks = self.compute_ready_tasks(scheduled_tasks)
            next_ready_tasks.sort(key=lambda t_id: -self.get_task_by_id(t_id).priority)
            
            next_state = (next_ready_tasks[0] if next_ready_tasks else -1, tuple(next_ready_tasks))
            next_valid_processors = list(range(1, len(self.processors) + 1)) if next_ready_tasks else []
            
            # Calculate reward - use a positive reward since this is a good schedule
            reward = self.reward_scale / len(schedule)
            
            # Update Q-value with high reward for this known good action
            state_key = self._get_state_key(state) if hasattr(self, '_get_state_key') else state
            if state_key not in self.agent.q_table:
                self.agent.q_table[state_key] = np.zeros(len(scheduler.processors))
            
            # Set a high positive Q-value for this known good action
            reward = self.reward_scale * 20
            self.agent.q_table[state_key][action] = reward
            # Also set small positive values for other actions in this state
            for other_action in range(len(scheduler.processors)):
                if other_action != action:
                    self.agent.q_table[state_key][other_action] = reward 
            ...
            self.agent.replay_buffer.add(
                ...
                reward, 
                next_state, 
                next_valid_processors,
                priority=reward * 2  # Higher priority for pretraining examples
            )
        
        print(f"Pretraining complete. Updated {len(schedule)} Q-values.")
    def run(self):
        """Run the QL-SHIELD scheduler"""
        print("Phase 1: Assigning minimum security levels to all messages...")
        self. assign_minimum_security()
        
        # Try to get HSMS and SHIELD solutions if available
        
        if self.hsms:
            print(f"Using HSMS solution with makespan {self.hsms.makespan} for pretraining")
        
        if self.shield:
            print(f"Using SHIELD solution with makespan {self.shield.makespan} for pretraining")
        
        print("\nPhase 2: Training Q-Learning agent for task scheduling...")
        self.train_ql_agent()
        # Apply the best schedule found during training
        if self.best_schedule:
            self.apply_best_schedule()
            
            # Calculate initial makespan and security utility
            initial_makespan = self.finalize_schedule()
            initial_security_utility = self.calculate_security_utility()
            
            print(f"\nAfter Q-Learning phase: Makespan={initial_makespan}, Security utility={initial_security_utility}")
            self.display_schedule_gantt()
            if initial_makespan <= self.deadline:
                
                print("\nPhase 3: Enhancing security using BCR heuristic...")
                self.enhance_security()
                ...
                print(f"Security utility improvement: {(final_security_utility - initial_security_utility) / initial_security_utility * 100:.2f}%")
                
                # Print task-processor mappings
                self.print_task_processor_mappings()
                
                return final_makespan, final_security_utility
            else:
                print(f"QL-SHIELD failed to meet deadline: {initial_makespan} > {self.deadline}")
                return None, None
        else:
            print("QL-SHIELD failed: No valid schedule found during training")
            return None, None
        
    # Add this method to the QLSHIELD class
    def display_schedule_gantt(self):
        """Display the current schedule as a Gantt chart."""
        if not self.schedule:
            print("No schedule available to display.")
            return
        
        # Print detailed schedule information
        print("\nSchedule after Phase 2 (Q-Learning):")
        print("-" * 80)
        print(f"{'Task ID':<8} {'Task Name':<20} {'Processor':<10} {'Start Time':<12} {'Finish Time':<12} {'Duration':<10}")
        print("-" * 80)
        
        # Sort tasks by processor, then by start time
        sorted_schedule = sorted(
            self.schedule,
            key=lambda entry: (entry['processor'], entry['start_time'])
        )
        ...
        for entry in sorted_schedule:
            ...
            print(f"{task_id:<8} {task_name:<20} {processor:<10} {start_time:<12.2f} {finish_time:<12.2f} {duration:<10.2f}")
        
        # Calculate overall schedule metrics
        makespan = max(entry['finish_time'] for entry in self.schedule)
        num_tasks = len(self.schedule)
        num_processors = len(set(entry['processor'] for entry in self.schedule))
        
        print("-" * 80)
        print(f"Total tasks: {num_tasks}")
        print(f"Total processors: {num_processors}")
        print(f"Makespan: {makespan:.2f}")
        print(f"Deadline: {self.deadline:.2f}")
        print(f"Slack: {(self.deadline - makespan):.2f}")
        
        # Create ASCII Gantt chart
        print("\nGantt Chart:")
        print("-" * 80)
        
        # Find chart dimensions
        chart_width = 60  # Characters for the time axis
        time_scale = chart_width / makespan  # Scale factor for time to chart width
        
        # Find all processors used in the schedule
        processors = sorted(set(entry['processor'] for entry in self.schedule))
        
        # Print header with time scale
        print(f"{'Proc':<6}|", end="")
        for i in range(11):
            position = i * (chart_width // 10)
            time_value = position / time_scale
            print(f"{time_value:<6.1f}", end="")
        print()
        print("-" * 6 + "+" + "-" * chart_width)
        
        # Print each processor's tasks
        for proc in processors:
            print(f"P{proc:<5}|", end="")
            
            # Get tasks for this processor
            proc_tasks = [entry for entry in self.schedule if entry['processor'] == proc]
            proc_tasks.sort(key=lambda x: x['start_time'])
            
            # Create the chart line
            chart_line = [" " for _ in range(chart_width)]
            
            # Fill in tasks on the chart
            for task in proc_tasks:
                start_pos = int(task['start_time'] * time_scale)
                end_pos = int(task['finish_time'] * time_scale)
                
                # Ensure positions are within chart bounds
                start_pos = max(0, min(start_pos, chart_width - 1))
                end_pos = max(0, min(end_pos, chart_width - 1))
                
                # Ensure task is at least 1 character wide
                if end_pos <= start_pos:
                    end_pos = start_pos + 1
                
                # Fill task area with task ID
                for i in range(start_pos, end_pos):
                    chart_line[i] = str(task['task_id'])[-1]  # Use last digit of task ID
                
                # Mark task boundaries with [ and ]
                if start_pos < chart_width:
                    chart_line[start_pos] = "["
                if end_pos - 1 < chart_width:
                    chart_line[end_pos - 1] = "]"
            
            # Print the chart line
            print("".join(chart_line))
        
        print("-" * 6 + "+" + "-" * chart_width)
        print("Legend: [#] represents Task ID #")
        print("-" * 80)
            
    # IMPROVED REWARD FUNCTION with parallelism incentive
    def calculate_reward(self, scheduled_tasks, current_makespan):
        """Calculate reward with deadline constraint penalty"""
        if len(scheduled_tasks) == len(self.tasks):
            # All tasks are scheduled
            if current_makespan <= self.deadline:
                # Give large reward for solutions within deadline
                reward = self.reward_scale * (2.0 - current_makespan/self.deadline) * 10
            else:
                # Stronger penalty for solutions that exceed deadline by large margins
                deadline_ratio = current_makespan / self.deadline
                if deadline_ratio >= 2.0:
                    # Extremely negative reward for solutions that are 2x+ over deadline
                    reward = -self.reward_scale * 1000  # Large fixed penalty
                else:
                    # Increasingly negative reward as we exceed deadline
                    reward = -self.reward_scale * (deadline_ratio - 1) * 100
        else:
            # Calculate processor utilization to incentivize parallelism
            # Count how many processors have tasks scheduled at current time
            current_time = max(scheduled_tasks.values()) if scheduled_tasks else 0
            active_processors = 0
            
            for proc in self.processors:
                # Check if processor is doing work at this moment
                for task in self.tasks:
                    if (task.is_scheduled and task.assigned_processor == proc.proc_id and
                        task.start_time <= current_time < task.finish_time):
                        active_processors += 1
                        break
            
            # Reward is higher when more processors are used in parallel
            parallelism_factor = active_processors / len(self.processors)
            
            # Add check for partial solutions that are already exceeding deadline
            if current_time > self.deadline * 1.5:
                # Penalize partial solutions that are trending toward exceeding deadline
                reward = -self.reward_scale * 10
            else:
                # Base reward plus parallelism bonus
                reward = (self.reward_scale / len(self.tasks)) * (1 + parallelism_factor)
        
        return reward
    def compute_ready_tasks(self, scheduled_tasks):
        """Compute the list of ready tasks given the already scheduled tasks"""
        ready_tasks = []
        
        for task in self.tasks:
            # Skip already scheduled tasks
            if task.task_id in scheduled_tasks:
                continue
                
            # Check if all predecessors are scheduled
            all_predecessors_scheduled = True
            for pred_id in task.predecessors:
                if pred_id not in scheduled_tasks:
    ...
    def calculate_task_est(self, task_id, proc_id, scheduled_tasks, task_finish_times):
        ...
        for pred_id in task.predecessors:
            if pred_id in scheduled_tasks:
                pred_task = self.get_task_by_id(pred_id)
                pred_proc_id = pred_task.assigned_processor
                
                # Get communication time
                message = self.get_message(pred_id, task_id)
                if message:
                    comm_time = 0
                    if pred_proc_id != proc_id:
                        source_proc = self.processors[pred_proc_id - 1]
                        dest_proc = processor
                        comm_time = source_proc.get_communication_time(message, source_proc, dest_proc)
                    
                    # Calculate security overhead
                    security_overhead = self.calculate_security_overhead_for_message(message, pred_proc_id, proc_id)
                    
                    # Update EST
                    pred_finish = task_finish_times[pred_id]
                    est = max(est, pred_finish + comm_time + security_overhead)
    ...
    def calculate_security_overhead_for_message(self, message, source_proc_id, dest_proc_id):
    ...
    def calculate_task_security_overhead(self, task_id, proc_id, scheduled_tasks, task_proc_map):
        """Calculate the total security overhead for a task on a processor"""
        task = self.get_task_by_id(task_id)
        ...
        for pred_id in task.predecessors:
            if pred_id in scheduled_tasks:
                pred_proc_id = task_proc_map[pred_id]
                message = self.get_message(pred_id, task_id)
                if message:
                    incoming_overhead += self.calculate_security_overhead_for_message(message, pred_proc_id, proc_id)
        ...
        for succ_id in task.successors:
            ...
            if message:
                ...
                auth_idx = message.assigned_security['authentication'] + 1
                auth_overhead = self.security.overheads['authentication'][auth_idx][proc_id - 1]
                outgoing_overhead += auth_overhead
        
        return incoming_overhead + outgoing_overhead
    def reinforce_schedule(self, schedule):
        """Reinforce a good schedule by updating Q-values"""
        # Sort schedule by start time to follow execution order
        ...
        for entry in sorted_schedule:
            task_id = entry['task_id']
            proc_id = entry['processor']
            task = self.get_task_by_id(task_id)
            
            if not task:
                continue
            
            # Get ready tasks at this point
            ready_tasks = self.compute_ready_tasks(scheduled_tasks)
            if task_id not in ready_tasks:
                continue  # This shouldn't happen in a valid schedule
            
            # Create state as it would be during training
            state = (task_id, tuple(ready_tasks))
            state_key = self.agent._get_state_key(state)
            
            # Give a strong positive reinforcement to this action
            if state_key not in self.agent.q_table:
                self.agent.q_table[state_key] = np.zeros(len(self.processors))
            
            # Set a high value for the chosen processor (relative to other options)
            current_values = self.agent.q_table[state_key]
            max_other_value = max([v for i, v in enumerate(current_values) if i != proc_id-1], default=0)
            self.agent.q_table[state_key][proc_id-1] = max(
                max_other_value + self.reward_scale * 0.5,  # Ensure it's higher than others
                self.agent.q_table[state_key][proc_id-1]    # Don't decrease if already high
            )
            ...
            scheduled_tasks[task_id] = task.finish_time
            task_proc_map[task_id] = proc_id
            
            # Update processor availability
            processor = self.processors[proc_id - 1]
            processor.available_time = max(processor.available_time, task.finish_time)
    def train_ql_agent(self):
        """Train the Q-Learning agent to find optimal task scheduling"""
        # Calculate upward rank for task priority
        self.compute_task_priorities()
        
        best_makespan = float('inf')
        best_schedule = None
        
        print(f"Starting Q-Learning training for {self.episodes} episodes...")
        
        # # CRITICAL CHANGE: First, explicitly initialize with the best known solution
        # if self.shield and self.shield_makespan < best_makespan:
        #     print(f"Starting with SHIELD solution (makespan: {self.shield_makespan})")
        #     best_makespan = self.shield_makespan
        #     best_schedule = self.shield.schedule.copy()
            
        # if self.hsms and self.hsms_makespan < best_makespan:
        #     print(f"Starting with HSMS solution (makespan: {self.hsms_makespan})")
        #     best_makespan = self.hsms_makespan
        #     best_schedule = self.hsms.schedule.copy()
        best_makespan = self.hsms.deadline + 1
        best_schedule = self.hsms.schedule.copy() if self.hsms else None
        # Pretrain from HSMS and/or SHIELD if available
        if self.hsms:
            print("Pretraining from HSMS schedule...")
            self.pretrain_from_schedule(self.hsms.schedule, self.hsms)
            
        if self.shield:
            print("Pretraining from SHIELD schedule...")
            self.pretrain_from_schedule(self.shield.schedule, self.shield)

        # IMPORTANT: Adjust these parameters
        self.agent.epsilon = 0.5  # Lower initial exploration rate
        self.agent.epsilon_decay = 0.005  # Faster decay
        self.agent.epsilon_min = 0.1  # Much lower minimum exploration rate
        extra_training_rounds = 5  # Extra rounds for reinforcing good solutions
        ...
        for episode in range(self.episodes):
            ...
            # NEW: With probability 25%, start from a known good solution
            if np.random.random() < 0.5 and best_schedule:
                # Apply partial solution - first half of tasks from best schedule
                num_tasks_to_use = len(best_schedule) 
                # num_tasks_to_use = len(best_schedule) // 2
                if num_tasks_to_use > 0:
                    sorted_schedule = sorted(best_schedule, key=lambda x: x['start_time'])
                    for i in range(num_tasks_to_use):
                        entry = sorted_schedule[i]
            ...
            # Track scheduled tasks and their finish times
            scheduled_tasks = {task.task_id: task.finish_time for task in self.tasks if task.is_scheduled}
            task_proc_map = {task.task_id: task.assigned_processor for task in self.tasks if task.is_scheduled}
            
            # Get initial ready tasks
            ready_tasks = self.compute_ready_tasks(scheduled_tasks)
            
            # If no ready tasks, there's a cycle in the task graph
            if not ready_tasks:
                print("Warning: No ready tasks at the beginning. Possibly a cycle in the task graph.")
                continue
            ...
            while ready_tasks:
                ...
                current_task_id = ready_tasks[0]
                current_task = self.get_task_by_id(current_task_id)
                
                # Create state for the agent
                state = (current_task_id, tuple(ready_tasks))
                
                # Valid processors are all processors
                valid_processors = list(range(1, len(self.processors) + 1))
                
                # Choose action (processor)
                chosen_proc = self.agent.choose_action(state, valid_processors)
                
                # Calculate EST for the task on the chosen processor
                est = self.calculate_task_est(current_task_id, chosen_proc, scheduled_tasks, scheduled_tasks)
                
                # Calculate execution time and security overhead
                exec_time = current_task.execution_times[chosen_proc - 1]
                security_overhead = self.calculate_task_security_overhead(
                    current_task_id, chosen_proc, scheduled_tasks, task_proc_map
                )
                
                # Calculate finish time
                finish_time = est + exec_time + security_overhead
                # In your train_ql_agent method, inside the while ready_tasks loop
                # After calculating finish_time
                if finish_time > self.deadline * 2:
                    # Early termination - this episode is clearly not producing a good solution
                    print(f"Early termination of episode {episode+1} - makespan exceeding 2x deadline")
                    break
                # Update processor availability
                processor = self.processors[chosen_proc - 1]
                processor.available_time = finish_time
                
                # Update task information
                current_task.assigned_processor = chosen_proc
                current_task.start_time = est
                current_task.finish_time = finish_time
                current_task.is_scheduled = True
                
                # Update scheduled tasks
                scheduled_tasks[current_task_id] = finish_time
                task_proc_map[current_task_id] = chosen_proc
                
                # Remove the task from ready tasks
                ready_tasks.remove(current_task_id)
                
                # Add newly ready tasks
                for task in self.tasks:
                    if task.task_id not in scheduled_tasks and all(pred in scheduled_tasks for pred in task.predecessors):
                        ready_tasks.append(task.task_id)
                ...
                # Prepare next state
                next_ready_tasks = ready_tasks.copy()
                next_state = (next_ready_tasks[0] if next_ready_tasks else -1, tuple(next_ready_tasks))
                next_valid_processors = list(range(1, len(self.processors) + 1)) if next_ready_tasks else []
                
                # Calculate makespan for reward
                current_makespan = max(scheduled_tasks.values()) if scheduled_tasks else 0
                
                # In train_ql_agent method where reward is calculated:
                reward = self.calculate_reward(scheduled_tasks, current_makespan)
                    
                # Update Q-value
                self.agent.update_q_value(state, chosen_proc - 1, reward, next_state, next_valid_processors)
                
                # Learn from replay buffer
                self.agent.replay()
            
            # After scheduling all tasks, calculate final makespan
            if scheduled_tasks:
                makespan = max(scheduled_tasks.values())
                ...
                if makespan < best_makespan:
                    ...
                    # NEW: Reinforce this good solution multiple times
                    for _ in range(extra_training_rounds):
                        # Replay this good schedule to strengthen its Q-values
                        self.reinforce_schedule(best_schedule)
                
                if episode % 10 == 0 or episode == self.episodes - 1:
                    print(f"Episode {episode+1}/{self.episodes}, Makespan: {makespan:.2f}, Best: {best_makespan:.2f}")
        ...
        if best_schedule:
            ...
            print(f"Training complete. Best makespan: {best_makespan:.2f}")
            if best_makespan <= self.deadline:
                print("Valid schedule found within deadline!")
            else:
                print(f"Best makespan exceeds deadline by {best_makespan - self.deadline:.2f} units")
    ...
    def apply_best_schedule(self):
        ...
        # Reset all tasks and processors
        for task in self.tasks:
            task.is_scheduled = False
            task.assigned_processor = None
            task.start_time = None
    ...
    def print_task_processor_mappings(self):
        """Print the task to processor mappings"""
        print("\nTask-Processor Mappings:")
        print("--------------------------")
        print(f"{'Task ID':<8} {'Task Name':<20} {'Processor':<10} {'Start Time':<12} {'Finish Time':<12}")
        print("-" * 65)
        
        # Sort tasks by processor, then by start time
        sorted_tasks = sorted(
            [t for t in self.tasks if t.is_scheduled],
            key=lambda t: (t.assigned_processor, t.start_time)
        )
        
        for task in sorted_tasks:
            print(f"{task.task_id:<8} {task.name:<20} {task.assigned_processor:<10} {task.start_time:<12.2f} {task.finish_time:<12.2f}")
    
    # BCR methods from SHIELD 
    
    def enhance_security(self):
        """Phase 3: Enhance security levels using available slack time."""
        # Initialize BCR heap with all possible upgrades
        self.initialize_bcr_heap()
    ...
    def save_task_states(self):
        ...
        for task in self.tasks:
            states[task.task_id] = {
                ...
                'assigned_processor': task.assigned_processor,
                'start_time': task.start_time,
                'finish_time': task.finish_time
            }
        return states
    ...
    def restore_task_states(self, states):
        ...
        for task in self.tasks:
            if task.task_id in states:
                saved_state = states[task.task_id]
                task.is_scheduled = saved_state['is_scheduled']
                task.assigned_processor = saved_state['assigned_processor']
                task.start_time = saved_state['start_time']
                task.finish_time = saved_state['finish_time']

    def save_processor_states(self):
        """Save current processor states for potential rollback."""
        states = {}
        for i, proc in enumerate(self.processors):
            states[i + 1] = proc.available_time
        return states

    def restore_processor_states(self, states):
        """Restore processor states from saved states."""
        for proc_id, available_time in states.items():
            self.processors[proc_id - 1].available_time = available_time

    def update_schedule_with_upgrade(self, upgraded_message):
        """Update the schedule after a security upgrade."""
        # Reset all finish times but keep processor assignments
        for task in self.tasks:
            if task.is_scheduled:
                task.start_time = None
                task.finish_time = None
        ...
        # Make a map of task-processor assignments
        task_proc_map = {task.task_id: task.assigned_processor for task in self.tasks if task.is_scheduled}
        
        # Topologically sort tasks based on precedence
        scheduled_tasks = {}
        ready_tasks = [task for task in self.tasks if task.is_scheduled and not task.predecessors]
        
        while ready_tasks:
            current_task = ready_tasks.pop(0)
            
            # Calculate EST for this task
            proc_id = current_task.assigned_processor
            est = 0
            
            for pred_id in current_task.predecessors:
                if pred_id in scheduled_tasks:
                    pred_finish = scheduled_tasks[pred_id]
                    pred_proc = task_proc_map[pred_id]
                    ...
                    if message:
                        ...
                        if pred_proc != proc_id:
                            source_proc = self.processors[pred_proc - 1]
                            dest_proc = self.processors[proc_id - 1] 
                            comm_time = source_proc.get_communication_time(message, source_proc, dest_proc)
                        
                        security_overhead = self.calculate_security_overhead_for_message(message, pred_proc, proc_id)
            ...
            # Calculate execution time and security overhead for outgoing messages
            exec_time = current_task.execution_times[proc_id - 1]
            outgoing_overhead = 0
            ...
            # Update task timing
            finish_time = est + exec_time + outgoing_overhead
            current_task.start_time = est
            current_task.finish_time = finish_time
            ...
            scheduled_tasks[current_task.task_id] = finish_time
            
            # Add newly ready tasks
            for task in self.tasks:
                if (task.is_scheduled and 
                    task.task_id not in scheduled_tasks and 
                    all(pred in scheduled_tasks for pred in task.predecessors)):
                    ready_tasks.append(task)

    def finalize_schedule(self):
        """Finalize the schedule and return the makespan."""
        # Make sure all tasks are properly scheduled with times
        for task in self.tasks:
            if task.is_scheduled and (task.start_time is None or task.finish_time is None):
                self.update_schedule_with_upgrade(None)  # Recompute the entire schedule
                break
    ...
    def calculate_security_utility(self):
        """Calculate the overall security utility of the current schedule."""
        total_utility = 0
    ...
    def assign_minimum_security(self):
        ...
        for message in self.messages:
            ...
            print(f"Assigned minimum security levels to message {message.id}: {message.assigned_security}")
    ...
    def get_message(self, source_id, dest_id):
        """Get message between source and destination tasks."""
        for message in self.messages:
            if message.source_id == source_id and message.dest_id == dest_id:
                return message
        return None
    ...
    def compute_task_priorities(self):
        """Compute upward rank (priority) for each task."""
        # Initialize priorities
        for task in self.tasks:
            task.priority = 0
        
        # Process tasks in reverse topological order
        # First, identify all exit tasks (tasks with no successors)
        exit_tasks = [task for task in self.tasks if not task.successors]
        
        # Compute average computation cost for each task
        for task in self.tasks:
            task.avg_comp_cost = sum(task.execution_times) / len(task.execution_times)
        
        # Start from exit tasks and move upward
        processed = set()
        
        def compute_upward_rank(task):
            if task.task_id in processed:
                return task.priority
            
            # If it's an exit task, priority is just its average computation cost
            if not task.successors:
                task.priority = task.avg_comp_cost
            else:
                # Calculate max of successors
                max_succ_rank = 0
                for succ_id in task.successors:
                    succ_task = self.get_task_by_id(succ_id)
                    if succ_task:
                        ...
                        comm_cost = 0
                        if message:
                            # Simplified average communication cost assumption
                            comm_cost = message.size / 100  # Some arbitrary scaling
                        
                        succ_rank = compute_upward_rank(succ_task)
                        max_succ_rank = max(max_succ_rank, comm_cost + succ_rank)
                
                task.priority = task.avg_comp_cost + max_succ_rank
        ...
        print("Task priorities computed:")
        for task in sorted(self.tasks, key=lambda t: -t.priority):
            print(f"Task {task.task_id} ({task.name}): Priority = {task.priority:.2f}")
...
class QLearningAgent:
    ...
    def replay(self):
        ...
        for experience in batch:
            state, action, reward, next_state, next_valid_processors = experience
            self.update_q_value(state, action, reward, next_state, next_valid_processors)
...
class PriorityReplayBuffer:
    ...
    def add(self, state, action, reward, next_state, next_valid_processors, priority=None):
        """Add experience to buffer"""
        experience = (state, action, reward, next_state, next_valid_processors)
    ...
    def sample(self, batch_size):
        """Sample batch from buffer based on priorities"""
...
def main():
    ...
    ql_shield = QLSHIELD(copy.deepcopy(tasks), copy.deepcopy(messages),
                    copy.deepcopy(processors), security_service, deadline, 1000,1000,hsms,hsms_makespan,shield,shield_makespan)
