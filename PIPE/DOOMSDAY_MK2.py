...
class QLearningAgent:
    ...
    def __init__(self, num_tasks, num_processors, alpha=0.1, gamma=0.9, epsilon=0.8, 
                 epsilon_decay=0.01, epsilon_min=0.1, replay_size=5000, batch_size=32):
...
class QLSHIELD(HSMS):
        ...
        for entry in sorted(schedule, key=lambda x: x['start_time']):
            ...
            for other_action in range(len(scheduler.processors)):
                if other_action != action:
                    self.agent.q_table[state_key][other_action] = reward * 0.1
    ...
    def train_ql_agent(self):
        ...
        for episode in range(self.episodes):
            ...
            if np.random.random() < 0.25 and best_schedule:
            ...
            while ready_tasks:
                ...
                # Calculate reward - much stronger incentive to meet deadline
                if len(scheduled_tasks) == len(self.tasks):
                    ...
                    if current_makespan <= self.deadline:
                        ...
                        reward = self.reward_scale * (2.0 - current_makespan/self.deadline)
                    else:
                        # Strong negative reward for exceeding deadline
                        reward = -self.reward_scale * (current_makespan/self.deadline - 1)
                else:
                    # Intermediate state, small positive reward
                    reward = self.reward_scale / len(self.tasks)
...
class QLearningAgent:
    ...
    def __init__(self, num_tasks, num_processors, alpha=0.1, gamma=0.9, epsilon=0.99, 
                 epsilon_decay=0.001, epsilon_min=0.8, replay_size=5000, batch_size=32):
        self.num_tasks = num_tasks
        self.num_processors = num_processors
        self.alpha = alpha          # Learning rate
        self.gamma = gamma          # Discount factor
