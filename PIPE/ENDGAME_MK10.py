...
class SecurityDQN(HSMS):
    """
    Security-aware DQN-based Scheduler using Deep Reinforcement Learning 
    for task scheduling and security level selection.
    """
    ...
    def __init__(self, tasks, messages, processors, security_service, deadline):
        ...
        # Initialize DQN networks (primary and target)
        ...
        print(f"DQN Scheduler initialized with state size: {self.state_size}, action size: {self.action_size}")
    
    def _calculate_state_size(self):
        """Calculate the size of the state representation."""
        # State includes:
        # - Task status features (scheduled/unscheduled)
        # - Processor load features
        # - Security level features for each message
        task_features = len(self.tasks)
        processor_features = len(self.processors)
        security_features = len(self.messages) * 3  # 3 security services per message
        
        return task_features + processor_features + security_features
    ...
    def _calculate_action_size(self):
        ...
        # - What security level to set for each message
        return len(self.tasks) * len(self.processors)
    
    def get_state(self):
        """Convert the current scheduling state into a feature vector."""
        # Initialize state vector with zeros
        state = np.zeros(self.state_size)
        
        # Fill task status features (1 if scheduled, 0 if not)
        for i, task in enumerate(self.tasks):
            state[i] = 1 if task.is_scheduled else 0
        
        # Fill processor load features (normalized by makespan)
        max_load = max([p.available_time for p in self.processors]) if any([p.available_time > 0 for p in self.processors]) else 1
        for i, processor in enumerate(self.processors):
            state[len(self.tasks) + i] = processor.available_time / max_load if max_load > 0 else 0
        
        # Fill security level features (normalized to 0-1 range)
        security_offset = len(self.tasks) + len(self.processors)
        for i, message in enumerate(self.messages):
            # Get max security levels for normalization
            max_conf = len(self.security.strengths['confidentiality']) - 1
            max_integ = len(self.security.strengths['integrity']) - 1
            max_auth = len(self.security.strengths['authentication']) - 1
            
            # Add normalized security levels
            state[security_offset + i*3] = message.assigned_security['confidentiality'] / max_conf if max_conf > 0 else 0
            state[security_offset + i*3 + 1] = message.assigned_security['integrity'] / max_integ if max_integ > 0 else 0
            state[security_offset + i*3 + 2] = message.assigned_security['authentication'] / max_auth if max_auth > 0 else 0
        
        return torch.FloatTensor(state)
    
    def decode_action(self, action_idx):
        """Decode action index into task and processor assignment."""
        num_processors = len(self.processors)
        task_idx = action_idx // num_processors
        processor_idx = action_idx % num_processors
        
        return task_idx, processor_idx + 1  # +1 because processors are 1-indexed
    
    def is_valid_action(self, task_idx, processor_id):
    ...
    def calculate_reward(self, makespan, security_utility):
    # Normalized distance to deadline (closer to 0 is better)
        normalized_deadline_distance = max(0, min(1, (self.deadline - makespan) / self.deadline))
        
        # Normalized security utility (higher is better)
        ...
        # Weight security more as we get closer to deadline
        deadline_weight = 0.7 if makespan > self.deadline else 0.3
        security_weight = 1 - deadline_weight
        
        reward = (deadline_weight * normalized_deadline_distance) + (security_weight * normalized_security)
        
        # Penalty for deadline violation
        if makespan > self.deadline:
            reward = -1 + (reward * 0.1)  # Small credit for being close
        ...
        for task_idx, task in enumerate(self.tasks):
            if not task.is_scheduled:
                ...
                if all_pred_scheduled:
                    # This task can be scheduled - add actions for all processors
                    for proc_idx in range(len(self.processors)):
                        action_idx = task_idx * len(self.processors) + proc_idx
                        valid_actions.append(action_idx)
        
        return valid_actions
    def select_security_action(self, state, message_idx):
        """Select security levels for a specific message"""
        # Extract features relevant to security decision
        security_features = self.extract_security_features(state, message_idx)
        
        # Use a separate security policy network
        with torch.no_grad():
            security_q_values = self.security_policy_net(security_features)
        
        # Each output corresponds to a security configuration
        # (confidentiality_level, integrity_level, authentication_level)
        best_config_idx = security_q_values.argmax().item()
        
        # Map index to actual security configuration
        conf_level = best_config_idx // (self.num_integrity_levels * self.num_auth_levels)
        remaining = best_config_idx % (self.num_integrity_levels * self.num_auth_levels)
        integ_level = remaining // self.num_auth_levels
        auth_level = remaining % self.num_auth_levels
    ...
    def select_action(self, state):
        ...
        if random.random() < self.epsilon:
            ...
            # Exploitation: choose best action from valid actions
            with torch.no_grad():
                q_values = self.policy_net(state)
            
            # Filter for valid actions only
            valid_q_values = [(idx, q_values[idx].item()) for idx in valid_actions]
    ...
    def take_action(self, action_idx):
        """
        Apply the selected action to the environment and return the reward.
        """
        if action_idx is None:
            return -100  # Large negative reward for invalid action
            
        task_idx, processor_id = self.decode_action(action_idx)
        
        if not self.is_valid_action(task_idx, processor_id):
            return -100  # Large negative reward for invalid action
        ...
        for pred_id in task.predecessors:
            pred_task = self.get_task_by_id(pred_id)
            if not pred_task.is_scheduled:
                return -100  # Should never happen if action is valid
        ...
        # Calculate finish time with all overheads included
        finish_time = est + base_exec_time + total_security_overhead
        ...
        # Calculate final makespan and security utility
            ...
            # Reward is based on both makespan and security utility
            if makespan <= self.deadline:
                # If we meet the deadline, reward is based on security utility
                reward = security_utility
                # Update best schedule if this meets deadline and:
                # 1. We don't have a best schedule yet, OR
                # 2. This has better security utility than our previous best
                if self.best_schedule is None or security_utility > self.best_security_utility:
                    self.best_schedule = copy.deepcopy(self.schedule)
                    self.best_makespan = makespan
                    self.best_security_utility = security_utility
                    # Store the current episode number
                    self.best_episode = self.current_episode
                    print(f"Episode {self.current_episode}: Found better schedule: Makespan={makespan:.2f}, Security={security_utility:.2f}")
            else:
            # If we miss the deadline, give a large negative reward
                reward = -100 + (self.deadline - makespan)
        
            return reward, True 
        else:
            # Intermediate reward based on partial schedule quality
            # Encourage scheduling tasks that don't increase makespan much
            reward = -0.1  # Small negative reward for each step
            
            # Add positive reward if this task didn't increase the makespan too much
            if current_makespan <= self.deadline:
                reward += 0.5
                
            return reward, False  # Return reward and done=False
    def apply_curriculum(self, episode):
        """Apply curriculum learning based on episode number"""
        # Start with simplified problem
        if episode < 100:
            # Phase 1: Focus only on makespan, minimum security
            self.security_weight = 0.0
            self.deadline_factor = 1.5  # More relaxed deadline
            
        elif episode < 200:
            # Phase 2: Introduce security considerations
            self.security_weight = 0.3
            self.deadline_factor = 1.3
            
        elif episode < 300:
            # Phase 3: Equal weighting
            self.security_weight = 0.5
            self.deadline_factor = 1.1
            
        else:
            # Phase 4: Full problem
            self.security_weight = 0.7
            self.deadline_factor = 1.0
            
        # Adjust effective deadline for training
        self.effective_deadline = self.deadline * self.deadline_factor
    def optimize_model(self):
        """Perform one step of optimization on the DQN model."""
    ...
    def train(self, num_episodes=1000, target_update=10):
        ...
        for episode in range(num_episodes):
            ...
            while not done:
                ...
                reward_info = self.take_action(action)
                
                if isinstance(reward_info, tuple):
                    reward, done = reward_info
                else:
                    reward, done = reward_info, False
                ...
                if action is not None:  # Only store if action was valid
                    self.memory.push(state.unsqueeze(0), action, 
                                  None if next_state is None else next_state.unsqueeze(0), 
                                  reward, done)
            ...
            makespan = max(task.finish_time for task in self.tasks if task.is_scheduled) if any(task.is_scheduled for task in self.tasks) else float('inf')
            security_utility = self.calculate_security_utility()
            
            self.makespan_history.append(makespan)
            self.security_utility_history.append(security_utility)
        ...
        print(f"Best makespan found: {self.best_makespan:.2f} in episode {self.best_episode}")
        print(f"Best security utility found: {self.best_security_utility:.2f}")
        
        # Restore the best schedule found during training
        if self.best_schedule:
            self.schedule = self.best_schedule
            self.finalize_schedule()
    
    def security_enhancement_phase(self):
        """
        Phase for enhancing security levels after a feasible schedule is found.
        This is similar to SHIELD's enhance_security method.
        """
        print("\nStarting DQN Security Enhancement Phase...")
        ...
        for message in self.messages:
            ...
            for service in ['confidentiality', 'integrity', 'authentication']:
                ...
                # Calculate BCR (Benefit-to-Cost Ratio)
                next_level = current_level + 1
                ...
                # Calculate service overhead for current and next levels
                current_overhead = self.calculate_service_overhead(message, service, current_level, source_proc, dest_proc)
                next_overhead = self.calculate_service_overhead(message, service, next_level, source_proc, dest_proc)
                ...
                if overhead_increase <= 0:
                    bcr = float('inf')  # Special case: benefit with no cost
                else:
                    bcr = utility_gain / overhead_increase
                
                if bcr > 0:
                    # Use negative BCR for max-heap in a min-heap implementation
                    heapq.heappush(bcr_heap, (-bcr, (message.id, service, current_level)))
        
        print(f"Initialized BCR heap with {len(bcr_heap)} potential upgrades")
        
        # Save original state for rollback if needed
        original_task_states = self.save_task_states()
        original_processor_states = self.save_processor_states()
        ...
        while bcr_heap:
            # Get the upgrade with the highest BCR
            bcr, (message_id, service, current_level) = heapq.heappop(bcr_heap)
            bcr = -bcr  # Negate back to positive value (heap is min-heap)
            ...
            # Save current state
            old_level = message.assigned_security[service]
            task_states = self.save_task_states()
            processor_states = self.save_processor_states()
            ...
            if makespan <= self.deadline:
                ...
                if next_level < max_level:
                    new_bcr = self.calculate_bcr(message, service, next_level)
                    if new_bcr > 0:
                        heapq.heappush(bcr_heap, (-new_bcr, (message_id, service, next_level)))
                
                print(f"✅ Upgrade successful! New {service} level: {next_level}")
            else:
                # Rollback the changes
                message.assigned_security[service] = old_level
                self.restore_task_states(task_states)
                self.restore_processor_states(processor_states)
                rejected_count += 1
                print(f"❌ Upgrade rejected: makespan {makespan:.2f} exceeds deadline {self.deadline:.2f}")
        
        # Restore original states if we haven't made any successful upgrades
        if upgrade_count == 0:
            self.restore_task_states(original_task_states)
            self.restore_processor_states(original_processor_states)
        
        print(f"\nSecurity enhancement complete: {upgrade_count} upgrades applied, {rejected_count} rejected")
        
        # Final recalculation of the schedule
        final_makespan = self.finalize_schedule()
        final_security_utility = self.calculate_security_utility()
        
        print(f"Final makespan: {final_makespan:.2f}, Final security utility: {final_security_utility:.2f}")
    ...
    def run(self):
        """Execute the DQN-based scheduling algorithm."""
        print("Running Security-aware DQN Scheduler...")
        
        # Train the DQN model
        self.train(num_episodes=500, target_update=10)
        
        # Verify we have a deadline-satisfying schedule before enhancement
        if self.best_schedule is None or self.best_makespan > self.deadline:
            print(f"DQN Scheduler couldn't find a schedule meeting the deadline during training.")
            return None, None
        
        print(f"Using best schedule found in episode {self.best_episode} as starting point")
        
        # Restore the best schedule found during training
        self.schedule = copy.deepcopy(self.best_schedule)
        self.finalize_schedule()  # Recalculate times
        
        # Save this known-good schedule as a fallback
        saved_schedule = copy.deepcopy(self.schedule)
        saved_makespan = self.best_makespan
        saved_security = self.best_security_utility
        
        # Perform security enhancement phase
        makespan, security_utility = self.security_enhancement_phase()
        
        # Check if enhancement preserved deadline compliance
        if makespan is None or makespan > self.deadline:
            print(f"Security enhancement led to deadline violation. Rolling back to best training schedule from episode {self.best_episode}.")
            self.schedule = saved_schedule
            makespan = saved_makespan
            security_utility = saved_security
        
        if makespan > self.deadline:
            print(f"DQN Scheduler failed to meet deadline. Makespan: {makespan}, Deadline: {self.deadline}")
            return None, None
        else:
            print(f"DQN Scheduler successful. Final solution based on episode {self.best_episode}.")
            print(f"Final makespan: {makespan:.2f}, Security Utility: {security_utility:.2f}")
            return makespan, security_utility
...
def main():
    ...
    ql_shield = SecurityDQN(copy.deepcopy(tasks), copy.deepcopy(messages),
                    copy.deepcopy(processors), security_service, deadline)
    ...
    with open(output_path, 'w') as f:
        ...
        fig1,ax=plot_gantt_chart("SHIELD Schedule", ql_shield.schedule, len(processors), ql_shield_makespan,ql_shield_security_utility,ql_shield.messages,ql_shield.tasks)
        fig2=plot_modified_gantt_chart("SHIELD Schedule", ql_shield.schedule, ql_shield.tasks, ql_shield.processors, ql_shield_makespan)
        plt.show()
