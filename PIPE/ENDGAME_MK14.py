...
class QLAgent:
    """
    Q-Learning agent for task scheduling optimization.
    Uses a state-action value function to learn optimal scheduling decisions.
    """
    ...
    def get_state_key(self, state):
        ...
        # State includes task features and processor features
    ...
    def select_action(self, state, available_actions):
        ...
        Args:
            ...
            available_actions: List of valid actions indices
    ...
    def update_q_value(self, state, action, reward, next_state, next_available_actions):
        ...
        # Best next action value (max Q-value for next state among available actions)
        if next_available_actions:
            max_next_q = max([self.q_table[next_state_key][a] for a in next_available_actions])
        else:
            max_next_q = 0  # Terminal state or no valid actions
        ...
        return abs(td_error)  # Return absolute TD error for prioritized replay

class QLSHIELDScheduler(SHIELD):
    """
    QL-SHIELD: Q-Learning enhanced Security-aware scHedulIng with sEcurity utiLity maximization unDer real-time constraints
    
    This algorithm extends SHIELD by using reinforcement learning to improve the initial schedule
    before applying security enhancement techniques.
    """
    ...
    def __init__(self, tasks, messages, processors, security_service, deadline):
        ...
        self.state_size = self.num_tasks * 3 + self.num_processors  # Task state features + processor availability
        self.action_size = self.num_processors  # Each processor is a possible action
        
        # Initialize QL agent
        self.agent = QLAgent(self.state_size, self.action_size)
    ...
    def run(self):
        """Execute the QL-SHIELD algorithm."""
        print("\nQL-SHIELD: Starting algorithm execution...")
        
        # Phase 1 & 2: Run HSMS first to get minimum security schedule
        print("\nPhase 1-2: Computing initial schedule with HSMS...")
        hsms_scheduler = HSMS(copy.deepcopy(self.tasks), copy.deepcopy(self.messages), copy.deepcopy(self.processors), self.security, self.deadline)
        hsms_makespan, hsms_security_utility = hsms_scheduler.run()
        
        if hsms_makespan is None or hsms_makespan > self.deadline:
            print("QL-SHIELD cannot proceed: HSMS failed to meet deadline requirements")
            return None, None
        
        # Also run SHIELD to get a better initial schedule
        print("\nRunning SHIELD for comparison and to seed initial experiences...")
        shield_scheduler = SHIELD(copy.deepcopy(self.tasks), copy.deepcopy(self.messages), copy.deepcopy(self.processors), self.security, self.deadline)
        shield_makespan, shield_security_utility = shield_scheduler.run()
        
        # Phase 3: QL Schedule Optimization
        print("\nPhase 3: QL-based Schedule Optimization...")
        self.train_ql_scheduler(hsms_scheduler, shield_scheduler)
        
        # Phase 4: Security Enhancement
        print("\nPhase 4: Security Enhancement using SHIELD approach...")
        self.enhance_security()
        
        # New Phase 5: Final Schedule Optimization
        print("\nPhase 5: Final Schedule Optimization...")
        deadline_met = self.optimize_final_schedule()
        
        if not deadline_met:
            print("Warning: Could not meet deadline after all optimizations")
        ...
        print(f"\nQL-SHIELD complete:")
        print(f"  - Initial HSMS makespan: {hsms_makespan:.2f}, security utility: {hsms_security_utility:.2f}")
        if shield_makespan:
            print(f"  - SHIELD makespan: {shield_makespan:.2f}, security utility: {shield_security_utility:.2f}")
        print(f"  - QL-SHIELD makespan: {final_makespan:.2f}, security utility: {final_security_utility:.2f}")
        
        if shield_security_utility:
            improvement = (final_security_utility - shield_security_utility) / shield_security_utility * 100
            print(f"Security utility improvement over SHIELD: {improvement:.2f}%")
        
        return final_makespan, final_security_utility
    
    def copy_schedule_from(self, scheduler):
        """Copy schedule details from another scheduler"""
        # Copy task assignments and timing
        for task_idx, task in enumerate(self.tasks):
            other_task = scheduler.tasks[task_idx]
            task.assigned_processor = other_task.assigned_processor
            task.start_time = other_task.start_time
            task.finish_time = other_task.finish_time
            task.is_scheduled = other_task.is_scheduled
            task.priority = other_task.priority
        
        # Copy processor states
        for proc_idx, proc in enumerate(self.processors):
            proc.available_time = scheduler.processors[proc_idx].available_time
        
        # Copy message security assignments
        for msg_idx, msg in enumerate(self.messages):
            other_msg = scheduler.messages[msg_idx]
            if hasattr(other_msg, 'assigned_security'):
                msg.assigned_security = other_msg.assigned_security.copy()
            if not hasattr(msg, 'id'):
                msg.id = f"M_{msg.source_id}_{msg.dest_id}"
        
        # Copy schedule list
        self.schedule = scheduler.schedule.copy()
        self.makespan = scheduler.makespan
    
    def encode_state(self, unscheduled_task, scheduled_tasks_info):
        """
        Encode the current scheduling state for RL agent
        
        Args:
            unscheduled_task: Task object to be scheduled next
            scheduled_tasks_info: Information about already scheduled tasks
        ...
        # 1. Feature 1: Task priority (normalized)
        max_priority = max(task.priority for task in self.tasks) if self.tasks else 1
        task_priority = unscheduled_task.priority / max_priority if max_priority > 0 else 0
        state.append(task_priority)
        
        # 2. Feature 2: Task complexity (execution time variance across processors)
        exec_times = unscheduled_task.execution_times
        if exec_times and max(exec_times) > 0:
            variance = np.var(exec_times) / max(exec_times)
        else:
            variance = 0
        state.append(variance)
        
        # 3. Feature 3: Task connectivity (number of predecessors and successors)
        connectivity = (len(unscheduled_task.predecessors) + len(unscheduled_task.successors)) / (2 * self.num_tasks)
        state.append(connectivity)
        
        # 4. Processor availability (normalized)
        max_finish_time = max(task_info['finish_time'] for task_info in scheduled_tasks_info.values()) if scheduled_tasks_info else 0
        for proc in self.processors:
            # Normalize processor availability time
            if max_finish_time > 0:
                normalized_availability = proc.available_time / max_finish_time
            else:
                normalized_availability = 0
            state.append(normalized_availability)
        
        return state
    
    def get_available_actions(self, task):
        """
        Get list of available processor assignments (actions) for a task
        
        Returns:
            List of valid processor indices (0-based)
        """
        # All processors are valid actions
        return list(range(self.num_processors))
    
    def calculate_reward(self, old_makespan, new_makespan, old_security_utility, new_security_utility):
        """
        Calculate reward based on makespan improvement and security utility
        
        Returns:
            reward: Float value representing action quality
        """
        safety_deadline = self.deadline * 0.95
    
        # Base reward for meeting deadline with safety margin
        if new_makespan > safety_deadline:
            # Exponential penalty as we approach deadline
            deadline_violation_factor = (new_makespan - safety_deadline) / (self.deadline - safety_deadline)
            penalty = -100 * (1 + deadline_violation_factor**2)
            return penalty
        
        # Calculate makespan improvement ratio
        makespan_improvement = (old_makespan - new_makespan) / old_makespan if old_makespan > 0 else 0
        
        # Calculate slack increase (normalized)
        old_slack = max(0, safety_deadline - old_makespan) / safety_deadline if safety_deadline > 0 else 0
        new_slack = max(0, safety_deadline - new_makespan) / safety_deadline if safety_deadline > 0 else 0
        slack_increase = new_slack - old_slack
        
        new_security_utility = self.calculate_security_utility()  # Calculate actual new utility
        security_improvement = (new_security_utility - old_security_utility) / old_security_utility if old_security_utility > 0 else 0
        ...
        # Combined reward
        reward = makespan_weight * makespan_improvement + security_weight * security_improvement + slack_weight * slack_increase
        return reward
    
    def reset_schedule(self):
        """Reset the schedule to initial state for a new episode"""
        # Reset task states
        for task in self.tasks:
            task.is_scheduled = False
    ...
    def validate_schedule(self):
        """
        Validate the schedule for consistency and fix any issues
        
        Returns:
            Boolean indicating if schedule is valid and meets deadline
        """
        # Check if all tasks are scheduled
        for task in self.tasks:
            if not task.is_scheduled:
                print(f"Warning: Task {task.task_id} is not scheduled")
                return False
        ...
        for task in self.tasks:
            for pred_id in task.predecessors:
                ...
                # Check if predecessor finishes before this task starts
                message = self.get_message(pred_id, task.task_id)
                if not message:
                    continue
                    
                # Calculate communication time if on different processors
                if pred_task.assigned_processor != task.assigned_processor:
                    source_proc = self.processors[pred_task.assigned_processor - 1]
                    dest_proc = self.processors[task.assigned_processor - 1]
                ...
                if task.start_time < pred_task.finish_time + comm_time + security_overhead:
                    # Fix by adjusting task start time
                    old_duration = task.finish_time - task.start_time
                    task.start_time = pred_task.finish_time + comm_time + security_overhead
                    task.finish_time = task.start_time + old_duration
                    ...
                    for succ_id in task.successors:
                        ...
                        if succ_task and succ_task.is_scheduled:
                            # Check if successor needs to be adjusted too
                            message = self.get_message(task.task_id, succ_id)
                            if message:
                                source_proc =  self.processors[task.assigned_processor - 1]
                                comm_time = source_proc.get_communication_time(message, source_proc, dest_proc) if source_proc != dest_proc else 0
                                # comm_time = self.calculate_communication_time(task, succ_task, message)
                                security_overhead = self.calc_security_overhead(message, task.assigned_processor, succ_task.assigned_processor)
                                if succ_task.start_time < task.finish_time + comm_time + security_overhead:
                                    # Need to recursively adjust successors
                                    # Mark for update in next pass
                                    succ_task.updated = False
        
        # Check for processor availability violations and eliminate gaps
        for proc_idx in range(len(self.processors)):
            self._eliminate_schedule_gaps(proc_idx)
    ...
    def _eliminate_schedule_gaps(self, processor_idx):
        """
        Eliminate unnecessary gaps between tasks on the same processor
        
        Args:
            processor_idx: 0-based index of processor
        """
        processor_id = processor_idx + 1  # Convert to 1-based index
        
        # Get all tasks assigned to this processor
        processor_tasks = [t for t in self.tasks if t.is_scheduled and t.assigned_processor == processor_id]
        
        # Sort tasks by start time
        processor_tasks.sort(key=lambda t: t.start_time)
        
        # Check for gaps and shift tasks as needed
        for i in range(1, len(processor_tasks)):
            prev_task = processor_tasks[i-1]
            current_task = processor_tasks[i]
            
            # Check if there's a gap
            if current_task.start_time > prev_task.finish_time:
                # Check if we can move current task earlier
                can_move_earlier = True
                
                # Check if all predecessors complete before prev_task.finish_time
                for pred_id in current_task.predecessors:
                    ...
                    # If predecessor is on different processor, check timing with comm time
                    if pred_task.assigned_processor != processor_id:
                        message = self.get_message(pred_id, current_task.task_id)
                        if message:
                            comm_time = self.processors[pred_task.assigned_processor - 1].get_communication_time(
                                message, 
                                self.processors[pred_task.assigned_processor - 1], 
                            ...
                            if pred_task.finish_time + comm_time + security_overhead > prev_task.finish_time:
                                can_move_earlier = False
                                break
                
                if can_move_earlier:
                    # Calculate duration of the current task
                    task_duration = current_task.finish_time - current_task.start_time
                    
                    # Move the task to start right after previous task
                    current_task.start_time = prev_task.finish_time
                    current_task.finish_time = current_task.start_time + task_duration
                    
                    # Update in schedule list
                    for entry in self.schedule:
                        if entry['task_id'] == current_task.task_id:
                            entry['start_time'] = current_task.start_time
                            entry['finish_time'] = current_task.finish_time
                            break
    def assign_task(self, task, processor_idx):
        """
        Assign a task to a processor and update schedule,
        charging full security overhead on both incoming and outgoing edges.
        """
        processor_id = processor_idx + 1
        processor = self.processors[processor_idx]

        # 1) Earliest Start Time (EST) based on predecessors
        tasks_on_processor = [t for t in self.tasks if t.is_scheduled and t.assigned_processor == processor_id]
        for other_task in tasks_on_processor:
            # Make sure all values are initialized before comparison
            if (other_task.start_time is not None and task.finish_time is not None and
                other_task.finish_time is not None and task.start_time is not None and
                other_task.start_time < task.finish_time and other_task.finish_time > task.start_time):
                # Conflict detected
                return float('inf')  # Signal invalid assignment
                
        # Initialize task start and finish times if they're None
        if task.start_time is None:
            task.start_time = 0
        if task.finish_time is None:
            # Set a temporary finish time based on processor's exec time
            task.finish_time = task.start_time + task.execution_times[processor_id - 1]
                
        est = 0
        total_incoming_overhead = 0
        for pred_id in task.predecessors:
            pred_task = self.get_task_by_id(pred_id)
            if not pred_task or not pred_task.is_scheduled:
                return float('inf')

            msg = self.get_message(pred_id, task.task_id)
            if not msg:
                continue

            # comm time if on different processors
            if pred_task.assigned_processor != processor_id:
                src = self.processors[pred_task.assigned_processor - 1]
                dst = processor
                comm_time = src.get_communication_time(msg, src, dst)
            else:
                comm_time = 0

            # **full** security overhead (lock + unlock)
            sec_ov = self.calc_security_overhead(
                msg,
                pred_task.assigned_processor,
                processor_id
            )
            total_incoming_overhead += sec_ov

            est = max(est, pred_task.finish_time + comm_time + sec_ov)

        # also respect processor availability
        est = max(est, processor.available_time)

        # 2) Base execution
        base_exec = task.execution_times[processor_id - 1]

        # 3) Outgoing security overhead on all successors
        total_outgoing_overhead = 0
        for succ_id in task.successors:
            succ_task = self.get_task_by_id(succ_id)
            # if not succ_task or not succ_task.is_scheduled:
            #     continue

            msg = self.get_message(task.task_id, succ_id)
            if not msg:
                continue

            # **full** security overhead for sender→receiver
            sec_ov = self.calc_security_overhead(
                msg,
                processor_id,
                succ_task.assigned_processor
            )
            total_outgoing_overhead += sec_ov

        # 4) Finalize times & update schedule
        task.start_time = est
        task.finish_time = est + base_exec + total_incoming_overhead + total_outgoing_overhead
        task.assigned_processor = processor_id
        task.is_scheduled = True

        # eliminate any gaps you may have (optional)
        self._eliminate_schedule_gaps(processor_idx)

        # update processor availability
        processor.available_time = task.finish_time

        # record in the schedule list
        self.schedule.append({
            'task_id': task.task_id,
            'name': task.name,
            ...
            'start_time': task.start_time,
            'finish_time': task.finish_time,
            'security_overhead': total_incoming_overhead + total_outgoing_overhead
        })

        # return updated makespan
        return max(t.finish_time for t in self.tasks if t.is_scheduled)

    def train_ql_scheduler(self, hsms_scheduler, shield_scheduler=None):
        """
        Train the Q-learning agent for task scheduling optimization
        
        Args:
            hsms_scheduler: HSMS scheduler with initial schedule
            shield_scheduler: Optional SHIELD scheduler for seeding experiences
        """
        print("\nStarting Q-Learning training for schedule optimization...")
        
        # Compute task priorities if not already done
        self.compute_task_priorities()
        ...
        # Seed replay buffer with HSMS experience
        print("Seeding replay buffer with HSMS schedule...")
        self._seed_experiences_from_scheduler(hsms_scheduler)
        
        # Also seed with SHIELD experience if available
        if shield_scheduler:
            print("Seeding replay buffer with SHIELD schedule...")
            self._seed_experiences_from_scheduler(shield_scheduler)
        ...
        for episode in range(self.episodes):
            ...
            # Track this episode's makespan and security utility
            old_makespan = float('inf')
            current_makespan = 0
            
            # Dictionary to keep track of scheduled tasks for state encoding
            scheduled_tasks_info = {}
            
            # Schedule each task in priority order
            for task in sorted_tasks:
                # Get current state
                state = self.encode_state(task, scheduled_tasks_info)
                
                # Get available actions
                available_actions = self.get_available_actions(task)
                
                # Select action (processor assignment)
                action = self.agent.select_action(state, available_actions)
                
                if action is None:
                    print(f"No valid actions for task {task.task_id} in episode {episode}")
                    continue
                
                # Remember old makespan for reward calculation
                old_makespan = current_makespan
                
                # Apply action (assign task to processor)
                current_makespan = self.assign_task(task, action)
                
                # Update scheduled tasks info
                scheduled_tasks_info[task.task_id] = {
                    'processor': task.assigned_processor,
                    'start_time': task.start_time,
                    'finish_time': task.finish_time
                }
                ...
                if next_task_idx < len(sorted_tasks):
                    ...
                    next_state = self.encode_state(next_task, scheduled_tasks_info)
                    next_available_actions = self.get_available_actions(next_task)
                else:
                    # Terminal state
                ...
                # Calculate reward
                old_security_utility = self.calculate_security_utility()
                reward = self.calculate_reward(old_makespan, current_makespan, 
                                              old_security_utility, old_security_utility)
                
                # Store experience in replay buffer
                experience = (state, action, reward, next_state, next_available_actions)
                priority = max(abs(reward), 1.0)  # Set initial priority
                self.replay_buffer.add(experience, priority)
                
                # Learn from experiences (batch training)
                if len(self.replay_buffer) >= self.batch_size:
                    self._train_from_replay_buffer()
            
            # Calculate final security utility for this episode
            security_utility = self.calculate_security_utility()
            
            # Check if this is the best schedule so far
            if current_makespan <= self.deadline and (
                current_makespan < self.best_makespan or 
                (current_makespan == self.best_makespan and security_utility > self.best_security_utility)
            ):
                self.best_makespan = current_makespan
            ...
            self.training_history['security_utility'].append(security_utility)
            self.training_history['deadline_met'].append(current_makespan <= self.deadline)
            
            # Print progress
            if (episode + 1) % 10 == 0:
                print(f"Episode {episode+1}/{self.episodes}: Makespan = {current_makespan:.2f}, " +
                      f"Security Utility = {security_utility:.2f}, " +
                      f"Deadline {'Met' if current_makespan <= self.deadline else 'Not Met'}")
        
        # Apply the best schedule found
        self._apply_best_schedule()
        
        print("\nQL Training complete!")
        print(f"Best schedule makespan: {self.best_makespan:.2f}")
        print(f"Best schedule security utility: {self.best_security_utility:.2f}")
        
        # Add this section to print task assignments after Phase 3
        print("\n=== Task-Processor Assignments after Phase 3 ===")
        print(f"{'Task ID':<10} {'Task Name':<20} {'Processor':<10} {'Start Time':<12} {'Finish Time':<12}")
        print("-" * 65)
        
        # Sort tasks by start time for clearer output
        sorted_tasks = sorted([t for t in self.tasks if t.is_scheduled], key=lambda t: t.start_time)
        
        for task in sorted_tasks:
            print(f"{task.task_id:<10} {task.name:<20} {task.assigned_processor:<10} {task.start_time:<12.2f} {task.finish_time:<12.2f}")
        
        # Print processor utilization
        print("\n=== Processor Utilization ===")
        for i, proc in enumerate(self.processors):
            tasks_on_proc = [t for t in self.tasks if t.is_scheduled and t.assigned_processor == i+1]
            proc_makespan = max([t.finish_time for t in tasks_on_proc]) if tasks_on_proc else 0
            utilization = sum([t.finish_time - t.start_time for t in tasks_on_proc]) / proc_makespan if proc_makespan > 0 else 0
            print(f"Processor {i+1}: {len(tasks_on_proc)} tasks, Utilization: {utilization:.2%}")
    
    def _seed_experiences_from_scheduler(self, scheduler):
        """
        Seed the replay buffer with experiences from existing scheduler
        
        Args:
            scheduler: Existing scheduler with a valid schedule
        """
        ...
        # Use the schedule from the existing scheduler to create experiences
        scheduled_tasks_info = {}
        current_makespan = 0
        
        # Process each task in priority order
        for task in sorted_tasks:
            # Find task in the scheduler's schedule
            scheduler_task = scheduler.get_task_by_id(task.task_id)
            if not scheduler_task or not scheduler_task.is_scheduled:
                continue
                
            # Get current state
            state = self.encode_state(task, scheduled_tasks_info)
            
            # Get action from the scheduler's assignment
            action = scheduler_task.assigned_processor - 1  # Convert to 0-based index
            ...
            # Calculate reward - positive reward for good schedules
            reward = 10.0 if current_makespan <= self.deadline else -10.0
            
            # Store experience in replay buffer with high priority
            experience = (state, action, reward, next_state, next_available_actions)
            self.replay_buffer.add(experience, priority=10.0)  # High priority for expert demonstrations
    
    def _train_from_replay_buffer(self):
        """Train Q-learning agent from replay buffer"""
        ...
        for i, (state, action, reward, next_state, next_available_actions) in enumerate(samples):
            # Update Q-value with importance sampling weight
            td_error = self.agent.update_q_value(state, action, reward, next_state, next_available_actions)
            
            # Update priority based on TD error
            new_priority = td_error
            new_priorities.append(new_priority)
        
        # Update priorities in replay buffer
        self.replay_buffer.update_priorities(indices, new_priorities)
    
    def _apply_best_schedule(self):
        """Apply the best schedule found during training"""
        if not self.best_schedule:
            print("No valid schedule found during training")
            return
        ...
        # Apply best schedule
        for task_info in self.best_schedule:
            task = self.get_task_by_id(task_info['task_id'])
            if task:
                task.assigned_processor = task_info['processor']
                task.start_time = task_info['start_time']
                task.finish_time = task_info['finish_time']
                task.is_scheduled = True
        ...
        # Finalize the schedule to ensure consistency
        self.finalize_schedule()

    def display_improvement_summary(self, hsms_scheduler, shield_scheduler):
        """
        Display improvement summary comparing QL-SHIELD with HSMS and SHIELD
        
        Args:
            hsms_scheduler: HSMS scheduler with baseline metrics
            shield_scheduler: SHIELD scheduler with improved metrics
        """
        print("\n" + "="*100)
        print("QL-SHIELD IMPROVEMENT SUMMARY")
        print("="*100)
        
        # Get metrics from each scheduler
        hsms_makespan = hsms_scheduler.makespan
        hsms_security = hsms_scheduler.calculate_security_utility()
        
        shield_makespan = shield_scheduler.makespan
        shield_security = shield_scheduler.calculate_security_utility()
        
        ql_shield_makespan = self.makespan
        ql_shield_security = self.calculate_security_utility()
        
        # Calculate improvements
        makespan_improvement_over_hsms = (hsms_makespan - ql_shield_makespan) / hsms_makespan * 100
        security_improvement_over_hsms = (ql_shield_security - hsms_security) / hsms_security * 100
        
        makespan_improvement_over_shield = (shield_makespan - ql_shield_makespan) / shield_makespan * 100
        security_improvement_over_shield = (ql_shield_security - shield_security) / shield_security * 100
        
        # Print comparison table
        print(f"{'Algorithm':<15} {'Makespan (ms)':<15} {'Security Utility':<20} {'Deadline Met':<15}")
        print("-"*70)
        print(f"{'HSMS':<15} {hsms_makespan:<15.2f} {hsms_security:<20.2f} {'✓' if hsms_makespan <= self.deadline else '✗':<15}")
        print(f"{'SHIELD':<15} {shield_makespan:<15.2f} {shield_security:<20.2f} {'✓' if shield_makespan <= self.deadline else '✗':<15}")
        print(f"{'QL-SHIELD':<15} {ql_shield_makespan:<15.2f} {ql_shield_security:<20.2f} {'✓' if ql_shield_makespan <= self.deadline else '✗':<15}")
        print("-"*70)
        
        print("\nImprovements:")
        print(f"Makespan improvement over HSMS: {makespan_improvement_over_hsms:.2f}%")
        print(f"Makespan improvement over HSMS: {makespan_improvement_over_hsms:.2f}%")
        print(f"Security utility improvement over HSMS: {security_improvement_over_hsms:.2f}%")
        print(f"Makespan improvement over SHIELD: {makespan_improvement_over_shield:.2f}%")
        print(f"Security utility improvement over SHIELD: {security_improvement_over_shield:.2f}%")
    ...
    def update_schedule_with_upgrade(self, message):
        ...
        # Mark all tasks as not updated yet
        for task in self.tasks:
            task.updated = False
        
        # First pass: update source and destination tasks
        source_task.updated = True
        dest_task.updated = True
        
        # Update all tasks using topological order to ensure dependency constraints
        for task in sorted_tasks:
            if not task.is_scheduled:
                continue
            ...
            # Check if this update affects other tasks on the same processor
            if old_start != task.start_time:
                # Update processor availability
                processor.available_time = task.finish_time
                
                # Shift dependent tasks on other processors
                for other_task in sorted_tasks:
                    if (other_task.is_scheduled and not other_task.updated and 
                        task.task_id in other_task.predecessors):
                        other_task.updated = False  # Mark for update in next iteration
        ...
        # Second pass: eliminate unnecessary gaps
        for proc_idx in range(len(self.processors)):
            self._eliminate_schedule_gaps(proc_idx)
    def try_security_upgrade(self, message, service, new_level):
        """
        ...
        old_task_states = self.save_task_states()
        old_processor_states = self.save_processor_states()
        old_schedule = copy.deepcopy(self.schedule)
        ...
        # Validate the schedule and ensure consistency
        if not self.validate_schedule():
            # Schedule is invalid or deadline is missed - rollback
            message.assigned_security[service] = old_level
            self.restore_task_states(old_task_states)
            self.restore_processor_states(old_processor_states)
        ...
        # Calculate current makespan after upgrade
        makespan = max(task.finish_time for task in self.tasks if task.is_scheduled)
        
        if makespan <= self.deadline:
            # Fix any remaining gaps in the schedule
            for proc_idx in range(len(self.processors)):
                self._eliminate_schedule_gaps(proc_idx)
            return True  # Upgrade successful
        else:
    ...
    def enhance_security(self):
        ...
        initial_security_utility = self.calculate_security_utility()
        
        print(f"\nSecurity enhancement complete: {upgrade_count} upgrades applied, {rejected_count} rejected")
        print(f"  - Security utility: {initial_security_utility:.2f}")
        print(f"  - Final makespan: {self.makespan:.2f} (deadline: {self.deadline:.2f})")
    def initialize_bcr_heap(self):
        """Initialize the benefit-to-cost ratio heap with all possible upgrades."""
        self.bcr_heap = []
    ...
    def can_increase_security(self, message):
        """
        Check if security can be increased for a message
        
        Returns:
            Boolean indicating if security can be increased
        """
        for sec_dim in range(len(message.assigned_security)):
            if message.assigned_security[sec_dim] < self.security.get_max_security_level(sec_dim):
                return True
        return False
    
    def recalculate_makespan(self):
        """
        Recalculate makespan after security changes
        ...
        # Reschedule all tasks with updated security overheads
        for task in sorted_tasks:
            if not task.is_scheduled:
                continue
                
            processor_idx = task.assigned_processor - 1
            processor = self.processors[processor_idx]
        ...
        # Calculate updated makespan
        return max(task.finish_time for task in self.tasks if task.is_scheduled)
    def calculate_security_utility(self):
        """
        Calculate overall security utility of the schedule
        
        Returns:
            Total security utility value
        """
        security_utility = 0
        
        for msg in self.messages:
            source_task = self.get_task_by_id(msg.source_id)
            dest_task = self.get_task_by_id(msg.dest_id)
            ...
            for service in ['confidentiality', 'integrity', 'authentication']:
                level = msg.assigned_security[service]
                # Calculate security utility based on weight and strength
                service_weight = msg.weights[service]
                service_strength = self.security.strengths[service][level]
                utility = service_weight * service_strength
                security_utility += utility
        
        return security_utility

    def calc_security_overhead(self, message, source_proc_id, dest_proc_id):
        """
        Calculate total security overhead for a message between two processors
        
        Args:
            message: Message object
            source_proc_id: Source processor ID
            dest_proc_id: Destination processor ID
            
        Returns:
            Security overhead time
        """
        source_proc = self.processors[source_proc_id - 1]
        dest_proc = self.processors[dest_proc_id - 1]
        ...
        for service in ['confidentiality', 'integrity', 'authentication']:
            ...
            if service in ['confidentiality', 'integrity']:
                # Data-dependent overhead
                source_overhead = message.size / self.security.overheads[service][protocol_idx][source_proc_id - 1]
                dest_overhead = message.size / self.security.overheads[service][protocol_idx][dest_proc_id - 1]
            else:  # Authentication
                # Fixed overhead
                source_overhead = self.security.overheads[service][protocol_idx][source_proc_id - 1]
                dest_overhead = self.security.overheads[service][protocol_idx][dest_proc_id - 1]
    ...
    def calculate_sender_security_overhead(self, message, processor_id):
        ...
        return total_overhead
    def optimize_final_schedule(self):
        """
        Perform final optimization on the schedule to eliminate gaps and ensure deadline is met
        
        Returns:
            Boolean indicating if optimization was successful
        """
        print("\nPerforming final schedule optimization...")
        
        # First, validate and fix any issues
        valid = self.validate_schedule()
        
        if not valid:
            print("Warning: Schedule has inconsistencies after security enhancements")
        
        # Eliminate gaps on all processors
        for proc_idx in range(len(self.processors)):
            self._eliminate_schedule_gaps(proc_idx)
        
        # Calculate the final makespan
        makespan = self.finalize_schedule()
        
        # If we're over deadline, try to reduce security levels selectively
        if makespan > self.deadline:
            print(f"Schedule exceeds deadline ({makespan:.2f} > {self.deadline:.2f})")
            print("Attempting security level adjustments to meet deadline...")
            
            # Sort messages by security level and least impact on utility
            messages_to_adjust = []
            for message in self.messages:
                source_task = self.get_task_by_id(message.source_id)
                dest_task = self.get_task_by_id(message.dest_id)
                ...
                # For each security service, check if we can reduce the level
                for service in ['confidentiality', 'integrity', 'authentication']:
                    current_level = message.assigned_security[service]
                    min_level = 0
                    
                    # Find the minimum required level based on min_security
                    for i, strength in enumerate(self.security.strengths[service]):
                        if strength >= message.min_security[service]:
                            min_level = i
                            break
                    
                    # Only consider if we can reduce the level
                    if current_level > min_level:
                        # Calculate utility impact of downgrade
                        utility_loss = message.weights[service] * (
                            self.security.strengths[service][current_level] - 
                            self.security.strengths[service][current_level-1]
                        )
                        
                        messages_to_adjust.append({
                            'message': message,
                            'service': service,
            ...
            # Sort by utility loss (ascending)
            messages_to_adjust.sort(key=lambda x: x['utility_loss'])
            
            # Try downgrades until we meet the deadline
            for adjustment in messages_to_adjust:
                # Save current state
                ...
                # Check if we meet deadline now
                makespan = self.finalize_schedule()
                
                if makespan <= self.deadline:
                    print(f"Downgraded {message.id} {service} from {adjustment['current_level']} to {new_level}")
                    print(f"New makespan: {makespan:.2f} (meets deadline)")
                    break
                elif makespan >= old_makespan:
                    # No improvement, rollback
                    message.assigned_security[service] = adjustment['current_level']
                    self.restore_task_states(old_task_states)
                    self.restore_processor_states(old_processor_states)
                    self.schedule = old_schedule
                    makespan = old_makespan
        
        # Final validation and gap elimination
        for proc_idx in range(len(self.processors)):
            self._eliminate_schedule_gaps(proc_idx)
        
        # Return whether deadline is met
        makespan = self.finalize_schedule()
        return makespan <= self.deadline
    ...
    def finalize_schedule(self):
        """
        Finalize the schedule and update makespan
        
        Returns:
            Final makespan
        """
        # Sort schedule by task start time
        self.schedule = sorted(self.schedule, key=lambda x: x['start_time'])
        
        # Update makespan
        if self.schedule:
            self.makespan = max(task['finish_time'] for task in self.schedule)
        else:
            self.makespan = 0
    ...
    def get_message(self, source_id, dest_id):
        """Get message object by source and destination task IDs"""
        for msg in self.messages:
            if msg.source_id == source_id and msg.dest_id == dest_id:
                return msg
        return None
    
    def compute_task_priorities(self):
        """
        Compute task priorities using upward rank method
        Similar to HSMS and SHIELD implementations
        """
        # Initialize upward rank for all tasks
        for task in self.tasks:
            task.upward_rank = 0
        ...
        # Calculate upward rank recursively (bottom-up)
        for task in reversed(self.tasks):
            # For exit tasks (no successors)
            if not task.successors:
                task.upward_rank = task.avg_exec_time
            else:
                # Calculate max rank of successors
                max_succ_rank = 0
                for succ_id in task.successors:
                    succ_task = self.get_task_by_id(succ_id)
                    if not succ_task:
                    ...
                    comm_cost = 0
                    if message:
                        # Estimate average communication cost
                        proc_pairs = [(i+1, j+1) for i in range(self.num_processors) for j in range(self.num_processors) if i != j]
                        if proc_pairs:
                            comm_costs = [self.processors[src-1].get_communication_time(message, 
                                           self.processors[src-1], self.processors[dst-1]) 
                                           for src, dst in proc_pairs]
                            comm_cost = sum(comm_costs) / len(comm_costs) if comm_costs else 0
                            
                    # Calculate rank
                    succ_rank = comm_cost + succ_task.upward_rank
                    max_succ_rank = max(max_succ_rank, succ_rank)
                    
                # Update task's upward rank
                task.upward_rank = task.avg_exec_time + max_succ_rank
            
            # Assign priority based on upward rank
            task.priority = task.upward_rank
            
        # Normalize priorities
        max_priority = max(task.priority for task in self.tasks) if self.tasks else 1
        for task in self.tasks:
            task.priority = task.priority / max_priority

# Example usage of QL-SHIELD algorithm
...
def main():
    ...
    #     sys.stdout = f
    #     sys.stderr = f
    ql_shield = QLSHIELDScheduler(copy.deepcopy(tasks), copy.deepcopy(messages),
                    copy.deepcopy(processors), security_service, deadline)
    ...
    if ql_shield_makespan:
        ...
        fig1.savefig(os.path.join(save_dir, "QL_SHIELD_GANTT_CHART(3).png"), dpi=300, bbox_inches='tight')
        fig2.savefig(os.path.join(save_dir, "QL_SHIELD_SECURITY_CHART(3).png"), dpi=300, bbox_inches='tight')
