...
class GASecurityScheduler(SHIELD):
    """
    Genetic Algorithm for Security-Aware Task Scheduling
    
    This scheduler combines GA's global search capabilities with HSMS and SHIELD for security-aware
    task scheduling under precedence constraints and real-time deadlines.
    """
    
    def __init__(self, tasks, messages, processors, network, security_service, deadline, **params):
        ...
        self.stagnation_threshold = params.get('stagnation_threshold', 15)
        self.max_lifespan = params.get('max_lifespan', 10)
        self.crossover_rate = params.get('crossover_rate', 0.8)
        self.mutation_rate = params.get('mutation_rate', 0.2)
        self.local_search_prob = params.get('local_search_prob', 0.1)
        self.lamarckian = params.get('lamarckian', True)  # True: Lamarckian, False: Baldwinian
        ...
        self.min_crossover_rate = 0.5
    
    def run(self):
        """Execute the GA-based security-aware scheduler."""
        print("\n" + "="*80)
        print("STARTING GENETIC ALGORITHM FOR SECURITY-AWARE SCHEDULING")
        print("="*80)
        
        # Phase 1: Generate initial solution using HSMS
        print("\nPhase 1: Generating initial solution using HSMS...")
        hsms_makespan, hsms_utility = super(SHIELD, self).run()  # Call the HSMS run method
        
        if hsms_makespan is None or hsms_makespan > self.deadline:
            print("Failed to find a feasible initial solution within deadline constraints")
            return None, None
        
        # Phase 2: Generate initial solution using SHIELD
        print("\nPhase 2: Enhancing initial solution using SHIELD...")
        shield_makespan, shield_utility = super().run()  # Call the SHIELD run method
        
        # Phase 3: Initialize GA population with CTS-driven initialization
        print("\nPhase 3: Initializing GA population...")
        self.initialize_population(hsms_solution=(hsms_makespan, hsms_utility), 
                                   shield_solution=(shield_makespan, shield_utility))
        
        # Phase 4: Run GA evolution
        print("\nPhase 4: Running GA evolution...")
        start_time = time.time()
        best_gen = 0
        
        for gen in range(self.generations):
            # Adjust rates based on progress
            self.adapt_rates(gen)
            ...
            if current_best_fitness > self.best_fitness:
                ...
                best_gen = gen
                self.stagnation_counter = 0
                print(f"Generation {gen}: Found new best solution with fitness: {self.best_fitness:.4f}")
            else:
            ...
            elites = [copy.deepcopy(self.population[i]) for i in sorted_indices[:self.elite_size]]
            
            # Update lifespan counters for elites and cull if necessary
            for i, elite in enumerate(elites):
                elite['lifespan'] = elite.get('lifespan', 0) + 1
                if elite['lifespan'] > self.max_lifespan and i > 0:  # Don't cull the very best
                    # Replace with a new random individual
                    elites[i] = self.create_random_individual()
                    elites[i]['lifespan'] = 0
            ...
            while len(next_population) < self.population_size:
                ...
                # Apply cataclysmic mutation if stagnation
                if self.stagnation_counter >= self.stagnation_threshold:
                    self.cataclysmic_mutation(offspring1)
                    self.cataclysmic_mutation(offspring2)
                
                # Repair if needed
                self.repair_chromosome(offspring1)
                self.repair_chromosome(offspring2)
                
                # Local search with Lamarckian/Baldwinian learning
                if random.random() < self.local_search_prob:
                    self.local_search(offspring1)
                    self.local_search(offspring2)
                
                # Reset lifespan for new offspring
                offspring1['lifespan'] = 0
                offspring2['lifespan'] = 0
            ...
            # Stop if stagnation persists after cataclysmic mutation
            if self.stagnation_counter >= 2 * self.stagnation_threshold:
                print(f"Stopping early due to prolonged stagnation after {gen} generations")
                break
        
        # Phase 5: Finalize best solution
        print("\nPhase 5: Finalizing best solution...")
        ...
        print("FINAL RESULTS")
        print("="*80)
        print(f"Best solution found at generation {best_gen}")
        print(f"HSMS:   Makespan={hsms_makespan:.2f}, Security Utility={hsms_utility:.2f}")
        print(f"SHIELD: Makespan={shield_makespan:.2f}, Security Utility={shield_utility:.2f}")
        print(f"GA:     Makespan={final_makespan:.2f}, Security Utility={final_security_utility:.2f}")
        
        if final_makespan <= self.deadline:
            utility_improvement = (final_security_utility - shield_utility) / shield_utility * 100
            print(f"GA improved security utility by {utility_improvement:.2f}% compared to SHIELD")
            print(f"Final solution meets deadline: {final_makespan:.2f} <= {self.deadline:.2f}")
        else:
            print(f"Warning: Final solution exceeds deadline: {final_makespan:.2f} > {self.deadline:.2f}")
        
        return final_makespan, final_security_utility
    
    def initialize_population(self, hsms_solution, shield_solution):
        """Initialize GA population with diverse individuals, seeded with HSMS and SHIELD solutions."""
        # Save initial solutions
        self.hsms_solution = hsms_solution
        self.shield_solution = shield_solution
        
        # Start with empty population
        self.population = []
        
        # Add chromosome based on HSMS solution
        hsms_chromosome = self.solution_to_chromosome()
        hsms_chromosome['lifespan'] = 0
        self.population.append(hsms_chromosome)
        
        # Apply SHIELD's security enhancement to create shield_chromosome
        self.enhance_security()  # This modifies the current schedule with SHIELD algorithm
        shield_chromosome = self.solution_to_chromosome()
        shield_chromosome['lifespan'] = 0
        self.population.append(shield_chromosome)
        
        # Fill remaining population using CTS-driven initialization
        for _ in range(self.population_size - 2):
            individual = self.create_random_individual()
            individual['lifespan'] = 0
            self.population.append(individual)
    
    def solution_to_chromosome(self):
        """Convert current schedule into a chromosome representation."""
        # Extract task sequence from current schedule
        task_sequence = [task.task_id for task in sorted(self.tasks, key=lambda t: t.start_time if t.is_scheduled else float('inf')) 
                        if task.is_scheduled]
        
        # Extract processor mapping
        proc_map = {task.task_id: task.assigned_processor for task in self.tasks if task.is_scheduled}
        
        # Extract security assignments for messages
        security_map = {msg.id: {
            'confidentiality': msg.assigned_security['confidentiality'],
            'integrity': msg.assigned_security['integrity'],
            'authentication': msg.assigned_security['authentication']
        } for msg in self.messages}
    ...
    def create_random_individual(self):
        """Create a random individual using CTS-driven initialization to guarantee precedence constraints."""
        # Start with empty task sequence
        task_sequence = []
        proc_map = {}
        
        # Create a copy of tasks
        available_tasks = {task.task_id: copy.deepcopy(task) for task in self.tasks}
        
        # Track which tasks have been scheduled
        scheduled = set()
        
        # Create a queue of ready tasks
        ready_tasks = [task.task_id for task in self.tasks if not task.predecessors]
        
        # Generate task sequence using CTS (Critical Task Sequence)
        while ready_tasks:
            # Randomly select a ready task
            task_id = random.choice(ready_tasks)
            ...
            proc_map[task_id] = random.randint(1, len(self.processors))
            
            # Mark as scheduled
            scheduled.add(task_id)
            ready_tasks.remove(task_id)
            
            # Update ready tasks with any new tasks that have all predecessors scheduled
            for task_id, task in available_tasks.items():
                if task_id not in scheduled and task_id not in ready_tasks:
                    if all(pred in scheduled for pred in task.predecessors):
                        ready_tasks.append(task_id)
        
        # If there are unscheduled tasks due to cycles, add them in some order
        unscheduled = set(available_tasks.keys()) - scheduled
        while unscheduled:
            # Find tasks with the most predecessors already scheduled
            best_task = max(unscheduled, 
                            key=lambda t: sum(1 for p in available_tasks[t].predecessors if p in scheduled))
            
            task_sequence.append(best_task)
            proc_map[best_task] = random.randint(1, len(self.processors))
            scheduled.add(best_task)
            unscheduled.remove(best_task)
        
        # Copy security settings from SHIELD solution but with random variations
        security_map = {}
        for msg in self.messages:
            security_map[msg.id] = {
                'confidentiality': min(
                    random.randint(msg.min_security_level['confidentiality'],
                                   len(self.security.strengths['confidentiality']) - 1),
                    len(self.security.strengths['confidentiality']) - 1
                ),
                'integrity': min(
                    random.randint(msg.min_security_level['integrity'],
                                  len(self.security.strengths['integrity']) - 1),
                    len(self.security.strengths['integrity']) - 1
                ),
                'authentication': min(
                    random.randint(msg.min_security_level['authentication'],
                                  len(self.security.strengths['authentication']) - 1),
                    len(self.security.strengths['authentication']) - 1
                )
            }
    ...
    def calculate_fitness(self, individual):
        ...
        Fitness combines security utility and makespan, with penalties for constraint violations.
        """
        # Save current state
        old_task_states = self.save_task_states()
        old_processor_states = self.save_processor_states()
        old_message_states = self.save_message_states()
        
        # Apply the solution
        self.apply_solution(individual)
        ...
        # Calculate penalties
        deadline_penalty = max(0, makespan - self.deadline) * 1000  # Large penalty for deadline violation
        
        # Precedence penalty (should be 0 due to CTS)
        precedence_penalty = 0
        for task in self.tasks:
            if task.is_scheduled:
                for pred_id in task.predecessors:
                    pred_task = self.get_task_by_id(pred_id)
                    if pred_task and pred_task.is_scheduled and pred_task.finish_time > task.start_time:
                        precedence_penalty += (pred_task.finish_time - task.start_time)
        
        # Security constraint penalty
        security_penalty = 0
        for msg in self.messages:
            source_task = self.get_task_by_id(msg.source_id)
            dest_task = self.get_task_by_id(msg.dest_id)
            
            if source_task and dest_task and source_task.is_scheduled and dest_task.is_scheduled:
                for service in ['confidentiality', 'integrity', 'authentication']:
                    if msg.assigned_security[service] < msg.min_security_level[service]:
                        security_penalty += 1000  # Large penalty for security violation
        
        # Final fitness: maximize security utility while minimizing violations
        fitness = security_utility - deadline_penalty - precedence_penalty - security_penalty
        
        # Restore state
        self.restore_task_states(old_task_states)
        self.restore_processor_states(old_processor_states)
        self.restore_message_states(old_message_states)
        
        return fitness
    
    def save_message_states(self):
        """Save current message security states for potential rollback."""
        states = {}
        for msg in self.messages:
            states[msg.id] = {
                'confidentiality': msg.assigned_security['confidentiality'],
    ...
    def restore_message_states(self, states):
        """Restore message security states from saved states."""
        for msg in self.messages:
            if msg.id in states:
                saved = states[msg.id]
                msg.assigned_security['confidentiality'] = saved['confidentiality']
                msg.assigned_security['integrity'] = saved['integrity']
                msg.assigned_security['authentication'] = saved['authentication']
    
    def apply_solution(self, individual):
        """Apply a solution from the chromosome to the current scheduler state."""
        # Reset all tasks
        for task in self.tasks:
        ...
        for msg in self.messages:
            if msg.id in individual['security_map']:
                for service in ['confidentiality', 'integrity', 'authentication']:
                    msg.assigned_security[service] = individual['security_map'][msg.id][service]
        
        # Apply task sequence and processor mapping
        task_sequence = individual['task_sequence']
        proc_map = individual['proc_map']
        
        # Schedule tasks in sequence order
        for task_id in task_sequence:
            ...
            # Set finish time temporarily (will be updated with security overhead later)
            task.finish_time = est + base_exec_time
    ...
    def order_preserving_crossover(self, parent1, parent2):
        """
        Perform order-preserving crossover that maintains CTS ordering.
        Ensures precedence constraints are not violated.
        """
        # Create child chromosome structures
        child1 = {
            'task_sequence': [],
            'proc_map': {},
            'security_map': {}
        ...
        # Get task sequences
        seq1 = parent1['task_sequence']
        seq2 = parent2['task_sequence']
        
        # Create position maps for each task in both parents
        pos1 = {task_id: idx for idx, task_id in enumerate(seq1)}
        pos2 = {task_id: idx for idx, task_id in enumerate(seq2)}
        ...
        # Copy middle segment
        child1['task_sequence'] = seq1[point1:point2+1]
        child2['task_sequence'] = seq2[point1:point2+1]
        
        # Fill remaining positions with tasks in order from other parent
        # Child 1
        remaining1 = [t for t in seq2 if t not in child1['task_sequence']]
        child1['task_sequence'] = remaining1[:point1] + child1['task_sequence'] + remaining1[point1:]
        
        # Child 2
        remaining2 = [t for t in seq1 if t not in child2['task_sequence']]
        child2['task_sequence'] = remaining2[:point1] + child2['task_sequence'] + remaining2[point1:]
        
        # Verify precedence constraints (should be preserved by CTS initialization)
        child1['task_sequence'] = self.verify_precedence(child1['task_sequence'])
        child2['task_sequence'] = self.verify_precedence(child2['task_sequence'])
        
        # Crossover processor mapping
        for task_id in seq1:
            # Randomly select which parent to inherit from
            if random.random() < 0.5:
                child1['proc_map'][task_id] = parent1['proc_map'][task_id]
                child2['proc_map'][task_id] = parent2['proc_map'][task_id]
            else:
                child1['proc_map'][task_id] = parent2['proc_map'][task_id]
                child2['proc_map'][task_id] = parent1['proc_map'][task_id]
        
        # Crossover security mapping
        for msg_id in parent1['security_map']:
            # Random crossover for each security service
            child1['security_map'][msg_id] = {}
            child2['security_map'][msg_id] = {}
            
            for service in ['confidentiality', 'integrity', 'authentication']:
                if random.random() < 0.5:
                    child1['security_map'][msg_id][service] = parent1['security_map'][msg_id][service]
                    child2['security_map'][msg_id][service] = parent2['security_map'][msg_id][service]
                else:
                    child1['security_map'][msg_id][service] = parent2['security_map'][msg_id][service]
                    child2['security_map'][msg_id][service] = parent1['security_map'][msg_id][service]
        
        return child1, child2
    
    def verify_precedence(self, task_sequence):
        """
        Verify and repair task sequence to ensure precedence constraints.
        Returns a valid task sequence.
        """
        # Create task objects
        task_objs = {task.task_id: task for task in self.tasks}
        
        # Check each task
        for i, task_id in enumerate(task_sequence):
            task = task_objs.get(task_id)
            if not task:
                continue
            ...
            for pred_id in task.predecessors:
                ...
                if pred_id in task_sequence:
                    pred_pos = task_sequence.index(pred_id)
                    if pred_pos > i:
                        # Precedence violation - swap to fix
                        task_sequence[i], task_sequence[pred_pos] = task_sequence[pred_pos], task_sequence[i]
                        # Recursive call to verify again after swap
                        return self.verify_precedence(task_sequence)
    ...
    def mutation(self, individual):
        ...
        mutation_type = random.choice(['proc', 'security', 'sequence'])
        
        if mutation_type == 'proc':
            # Mutate processor assignment
            task_id = random.choice(individual['task_sequence'])
            individual['proc_map'][task_id] = random.randint(1, len(self.processors))
        
        elif mutation_type == 'security':
            # Mutate security assignments
            if not individual['security_map']:
                return
                
            msg_id = random.choice(list(individual['security_map'].keys()))
            ...
            min_level = msg.min_security_level.get(service, 0)
            max_level = len(self.security.strengths[service]) - 1
            
            # Set a new level within range
            if max_level > min_level:
                current = individual['security_map'][msg_id][service]
                
                # Increase or decrease by 1, staying within bounds
                if random.random() < 0.5 and current > min_level:
                    individual['security_map'][msg_id][service] = current - 1
                elif current < max_level:
                    individual['security_map'][msg_id][service] = current + 1
        
        elif mutation_type == 'sequence':
            # Swap two adjacent tasks if it doesn't violate precedence
            if len(individual['task_sequence']) < 2:
                return
                
            idx1 = random.randint(0, len(individual['task_sequence']) - 2)
            idx2 = idx1 + 1
            
            task1 = self.get_task_by_id(individual['task_sequence'][idx1])
            task2 = self.get_task_by_id(individual['task_sequence'][idx2])
            
            # Check if swap would violate precedence
            if (task1 and task2 and 
                task1.task_id not in task2.predecessors and 
                task2.task_id not in task1.successors):
                # Safe to swap
                individual['task_sequence'][idx1], individual['task_sequence'][idx2] = \
                    individual['task_sequence'][idx2], individual['task_sequence'][idx1]
    
    def cataclysmic_mutation(self, individual):
        """Apply heavy mutation to escape local optima."""
        # Modify processor assignments extensively
        for task_id in individual['task_sequence']:
            if random.random() < 0.3:  # 30% chance for each task
                individual['proc_map'][task_id] = random.randint(1, len(self.processors))
        
        # Modify security settings extensively
        for msg_id in individual['security_map']:
            msg = None
            for m in self.messages:
                if m.id == msg_id:
                    msg = m
            ...
            for service in ['confidentiality', 'integrity', 'authentication']:
                if random.random() < 0.3:  # 30% chance for each service
                    min_level = msg.min_security_level.get(service, 0)
                    max_level = len(self.security.strengths[service]) - 1
                    individual['security_map'][msg_id][service] = random.randint(min_level, max_level)
    
    def repair_chromosome(self, individual):
        """Repair a chromosome if it violates constraints."""
        # Repair task sequence precedence (should be already ok from CTS)
        individual['task_sequence'] = self.verify_precedence(individual['task_sequence'])
        
        # Repair security levels
        for msg_id in individual['security_map']:
            msg = None
            for m in self.messages:
                if m.id == msg_id:
            ...
            for service in ['confidentiality', 'integrity', 'authentication']:
                ...
                level = individual['security_map'][msg_id][service]
                individual['security_map'][msg_id][service] = max(min_level, min(level, max_level))
    def local_search(self, individual):
        """Apply local search to improve individual."""
        # Save original individual
        original = copy.deepcopy(individual)
        original_fitness = self.calculate_fitness(original)
        
        # Try different local improvements
        improved = False
        
        # 1. Try processor reassignments for random tasks
        for _ in range(3):  # Try a few random reassignments
            task_id = random.choice(individual['task_sequence'])
            original_proc = individual['proc_map'][task_id]
            
            # Try each processor
            best_proc = original_proc
            best_fitness = original_fitness
            
            for proc_id in range(1, len(self.processors) + 1):
                if proc_id == original_proc:
                    continue
                    
                # Try this processor
                individual['proc_map'][task_id] = proc_id
                fitness = self.calculate_fitness(individual)
            ...
            # Restore best processor
            individual['proc_map'][task_id] = best_proc
        
        # 2. Try security level improvements
        for msg_id in individual['security_map']:
            msg = None
            for m in self.messages:
                if m.id == msg_id:
            ...
            for service in ['confidentiality', 'integrity', 'authentication']:
                original_level = individual['security_map'][msg_id][service]
                min_level = msg.min_security_level.get(service, 0)
                max_level = len(self.security.strengths[service]) - 1
                
                # Try increasing/decreasing security level
                best_level = original_level
                best_fitness = self.calculate_fitness(individual)
                
                for level in range(min_level, max_level + 1):
                    if level == original_level:
                        continue
                        
                    individual['security_map'][msg_id][service] = level
                    fitness = self.calculate_fitness(individual)
                ...
                # Restore best level
                individual['security_map'][msg_id][service] = best_level
        
        # Apply Lamarckian/Baldwinian learning
        if not self.lamarckian:
            # Baldwinian learning: revert to original but keep fitness value
            individual.update(original)
    ...
    def calculate_diversity(self):
        ...
        # Average pairwise differences between individuals
        for i in range(pop_size):
            for j in range(i+1, pop_size):
                ind1 = self.population[i]
                ind2 = self.population[j]
                
                # Difference in processor mappings
                proc_diff = sum(1 for t in ind1['proc_map'] if t in ind2['proc_map'] and ind1['proc_map'][t] != ind2['proc_map'][t])
                
                # Difference in task sequence (position-based)
                seq_diff = 0
                for task_id in ind1['task_sequence']:
                    if task_id in ind2['task_sequence']:
                        pos1 = ind1['task_sequence'].index(task_id)
                        pos2 = ind2['task_sequence'].index(task_id)
                        seq_diff += abs(pos1 - pos2)
                
                # Difference in security mappings
                sec_diff = 0
                for msg_id in ind1['security_map']:
                    if msg_id in ind2['security_map']:
                        for service in ['confidentiality', 'integrity', 'authentication']:
                            sec_diff += abs(ind1['security_map'][msg_id][service] - ind2['security_map'][msg_id][service])
                
                # Normalize and combine differences
                norm_proc_diff = proc_diff / max(1, len(ind1['proc_map']))
                norm_seq_diff = seq_diff / max(1, len(ind1['task_sequence']) ** 2)
                norm_sec_diff = sec_diff / max(1, len(ind1['security_map']) * 3 * 5)  # Assuming max 5 levels per service
                
                # Overall difference between these two individuals
                ind_diff = (norm_proc_diff + norm_seq_diff + norm_sec_diff) / 3
                diversity += ind_diff
        
        # Normalize by number of comparisons
        comparison_count = (pop_size * (pop_size - 1)) / 2
        if comparison_count > 0:
            diversity /= comparison_count
        
        return diversity
    
    def adapt_rates(self, generation):
        """Adapt mutation and crossover rates based on generation and diversity."""
        # Calculate progress as percentage of total generations
        progress = generation / self.generations
        
        # Get current diversity
        diversity = self.calculate_diversity()
        
        # Base adaptation on generation progress
        if progress < 0.3:
            # Early: high exploration
            target_mutation = self.initial_mutation_rate
            target_crossover = self.initial_crossover_rate
        elif progress < 0.7:
            # Middle: gradual transition
            target_mutation = self.initial_mutation_rate * (1 - (progress - 0.3) / 0.4)
            target_crossover = self.initial_crossover_rate * (1 - (progress - 0.3) / 0.4 * 0.2)  # Less reduction for crossover
        else:
            # Late: exploitation
            target_mutation = self.min_mutation_rate
            target_crossover = self.min_crossover_rate
        
        # Adjust based on diversity
        diversity_factor = max(0.5, min(1.5, 1.0 / max(0.01, diversity)))
        
        # Apply adjustments with limits
        self.mutation_rate = max(self.min_mutation_rate, 
                                min(self.initial_mutation_rate * 1.5, 
                                    target_mutation * diversity_factor))
        
        self.crossover_rate = max(self.min_crossover_rate, 
                                min(self.initial_crossover_rate * 1.2, 
                                    target_crossover * diversity_factor))
        
        # If stagnation is detected, increase mutation temporarily
    ...
    def restore_task_states(self, states):
        ...
        for task in self.tasks:
            if task.task_id in states:
                saved = states[task.task_id]
                task.is_scheduled = saved['is_scheduled']
                task.assigned_processor = saved['assigned_processor']
                task.start_time = saved['start_time']
                task.finish_time = saved['finish_time']
    ...
    def save_processor_states(self):
        ...
        for i, proc in enumerate(self.processors):
            states[i] = {
                'available_time': proc.available_time
            }
        return states
    ...
    def restore_processor_states(self, states):
        ...
        for i, proc in enumerate(self.processors):
            if i in states:
                proc.available_time = states[i]['available_time']
    
    def finalize_schedule(self):
        """
        Finalize the schedule by updating task execution times with security overhead.
        Returns the makespan of the schedule.
        """
        # Recalculate finish times considering security overhead
        for task in self.tasks:
            if not task.is_scheduled:
                continue
                
            # Base execution time
            exec_time = task.execution_times[task.assigned_processor - 1]
            
            # Add security overhead for incoming messages
            for msg in self.messages:
                if msg.dest_id == task.task_id:
                    source_task = self.get_task_by_id(msg.source_id)
                    if source_task and source_task.is_scheduled:
                        # Calculate security overhead
                        overhead = self.security.get_overhead(msg)
                        # Update finish time
                        task.finish_time = task.start_time + exec_time + overhead
        
        # Calculate makespan
        makespan = max((task.finish_time for task in self.tasks if task.is_scheduled), default=0)
        return makespan
    
    def calculate_security_utility(self):
        """Calculate the security utility of the current schedule."""
        utility = 0.0
        ...
        for msg in self.messages:
            ...
            if source_task and dest_task and source_task.is_scheduled and dest_task.is_scheduled:
                # Calculate utility based on assigned security levels
                conf_utility = self.security.get_security_utility(msg, 'confidentiality')
                int_utility = self.security.get_security_utility(msg, 'integrity')
                auth_utility = self.security.get_security_utility(msg, 'authentication')
                
                # Sum utilities with weights
                msg_utility = (conf_utility + int_utility + auth_utility) / 3
                utility += msg_utility
        
        return utility
    
    def enhance_security(self):
        """Apply SHIELD's security enhancement algorithm."""
        # This method should implement the SHIELD algorithm to enhance security
        # It operates on the current schedule and modifies security assignments
        
        # For each message in descending order of criticality
        messages_by_criticality = sorted(self.messages, key=lambda m: m.criticality, reverse=True)
        
        for msg in messages_by_criticality:
            source_task = self.get_task_by_id(msg.source_id)
            dest_task = self.get_task_by_id(msg.dest_id)
            ...
            # Calculate slack time available
            slack = self.deadline - max(task.finish_time for task in self.tasks if task.is_scheduled)
            
            if slack <= 0:
                # No room for security enhancement
                continue
                
            # Try enhancing each security service
            for service in ['confidentiality', 'integrity', 'authentication']:
                current_level = msg.assigned_security[service]
                max_level = len(self.security.strengths[service]) - 1
                ...
                # Try next level
                old_level = msg.assigned_security[service]
                msg.assigned_security[service] = old_level + 1
                
                # Calculate new makespan
                makespan = self.finalize_schedule()
                
                if makespan > self.deadline:
                    # Revert if deadline exceeded
                    msg.assigned_security[service] = old_level
                    self.finalize_schedule()  # Recalculate with original level
