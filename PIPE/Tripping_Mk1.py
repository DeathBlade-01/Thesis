...
class PrioritizedReplayBuffer1:
    """Enhanced replay buffer with prioritized experience replay for more efficient learning"""
    ...
    def push(self, state, action, reward, next_state, done):
        """Add a new experience to memory with maximum priority for new experiences"""
        max_priority = self.max_priority if self.memory else 1.0
...
class MessageAttentionLayer(nn.Module):
    """Attention mechanism to focus on important messages"""
    def __init__(self, input_dim, num_messages):
        super(MessageAttentionLayer, self).__init__()
        self.attention = nn.Sequential(
            nn.Linear(input_dim, 64),
            nn.Tanh(),
            nn.Linear(64, num_messages),
            nn.Softmax(dim=1)
        )
        
    def forward(self, x, message_features):
        """
        Apply attention mechanism to message features
        
        Args:
            x: global state features [batch, input_dim]
            message_features: per-message features [batch, num_messages, feature_dim]
            
        Returns:
            weighted_messages: attention-weighted message features [batch, feature_dim]
            attention_weights: attention weights [batch, num_messages]
        """
        # Calculate attention weights
        attention_weights = self.attention(x)  # [batch, num_messages]
        
        # Apply attention to message features
        weighted_messages = torch.bmm(
            attention_weights.unsqueeze(1),
            message_features
        ).squeeze(1)  # [batch, feature_dim]
        
        return weighted_messages, attention_weights

class A3CNetwork(nn.Module):
    """Enhanced A3C network with attention mechanism and batch normalization"""
    def __init__(self, state_size, action_size, message_count, hidden_sizes=[256, 128, 64]):
        ...
        for h_size in hidden_sizes[:-1]:
            ...
            layers.append(nn.BatchNorm1d(h_size))  # Add batch normalization
            prev_size = h_size
        
        self.shared = nn.Sequential(*layers)
        
        # Message attention mechanism
        self.attention = MessageAttentionLayer(hidden_sizes[-2], message_count)
        
        # Actor network (policy)
        self.policy_layers = nn.Sequential(
            nn.Linear(hidden_sizes[-2] + 64, hidden_sizes[-1]),  # +64 for attention output
            nn.ReLU(),
        ...
        # Message feature extraction (for attention)
        self.message_feature_extractor = nn.Sequential(
            nn.Linear(state_size, 64),
            nn.ReLU()
        )
    
    def forward(self, x, message_data=None):
        """
        Forward pass with attention mechanism
        
        Args:
            x: Input state [batch, state_size]
            message_data: Optional message-specific data. If None, extract from state.
            
        Returns:
            policy_output: Action probabilities [batch, action_size]
            value_output: State value [batch, 1]
            attention_weights: Attention weights for messages (for visualization)
        """
        # Get shared features
        shared_output = self.shared(x)
        
        # Extract message features (simplified - in practice, you'd extract from state)
        if message_data is None:
            # Extract from state (simplified approach)
            message_features = self.message_feature_extractor(x)
            # Reshape to [batch, message_count, feature_dim]
            batch_size = x.shape[0]
            message_count = 10  # Placeholder - would be determined from state structure
            message_features = message_features.view(batch_size, message_count, -1)
        else:
            message_features = message_data
        
        # Apply attention
        weighted_message_features, attention_weights = self.attention(shared_output, message_features)
        
        # Concatenate shared features with message attention output
        combined_features = torch.cat([shared_output, weighted_message_features], dim=1)
        
        # Policy and value outputs
        policy_output = self.policy_layers(combined_features)
        value_output = self.value_layers(combined_features)
        
        return policy_output, value_output, attention_weights

# MAKE MODIFICATIONS HERE
class A3CAgent:
    def __init__(self, state_size, action_size,msg, learning_rate=0.001, gamma=0.99):
        ...
        # Create network
        self.network = A3CNetwork(state_size, action_size,msg).to(self.device)
    ...
    def act(self, state, explore=True, temperature=1.0):
        """Choose an action based on current state with temperature-controlled exploration"""
        ...
        if explore:
            ...
            if temperature != 1.0:
                ...
                logits = torch.log(policy + 1e-10) / temperature
                scaled_policy = F.softmax(logits, dim=1)
            else:
                scaled_policy = policy
                
            # Sample action from the probability distribution
            m = Categorical(scaled_policy)
...
class CriticalPathUpgrader:
    ...
    def __init__(self, scheduler, max_iterations=100, checkpoint_interval=5, deadline_buffer=0.05,msg=0):
        """
        ...
        self.agent = A3CAgent(state_size, action_size,msg)
    ...
    def calculate_reward(self, old_utility, new_utility, old_makespan, new_makespan):
        """
        Enhanced reward function that considers long-term effects and better balances
        security utility vs. timing constraints.
        """
        # Base utility improvement reward (scaled by remaining upgrade potential)
        max_possible_utility = self.calculate_max_possible_utility()
        utility_headroom = max_possible_utility - old_utility
        if utility_headroom <= 0:
            # No room for improvement
            utility_reward = 0
        else:
            # Scale reward by how much of the remaining potential was achieved
            utility_gained = new_utility - old_utility
            utility_reward = (utility_gained / utility_headroom) * 15.0
        
        # Makespan effect with progressive penalty
        if new_makespan > self.buffer_deadline:
            # Progressive penalty based on how close to hard deadline
            buffer_violation = (new_makespan - self.buffer_deadline) / (self.deadline - self.buffer_deadline)
            deadline_penalty = -20 * (1 + buffer_violation**2)
        else:
            # Reward for staying under buffer, scaled by remaining margin
            margin_ratio = (self.buffer_deadline - new_makespan) / self.buffer_deadline
            deadline_penalty = 5 * margin_ratio
        
        # Efficiency component - penalize increased makespan, reward decreased makespan
        makespan_change = old_makespan - new_makespan
        efficiency_reward = 8 * makespan_change / old_makespan
        
        # Balance reward - encourage balanced security levels across all messages
        balance_reward = self.calculate_security_balance_reward()
        
        # Critical resource utilization reward
        resource_reward = self.calculate_resource_utilization_reward()
        
        # Total reward
        total_reward = (utility_reward * 0.5 + deadline_penalty * 0.2 + 
                        efficiency_reward * 0.1 + balance_reward * 0.1 + 
                        resource_reward * 0.1)
...
class CriticalPathUpgradeScheduler:
    ...
    def run(self):
        ...
        upgrader = CriticalPathUpgrader(self.hsms_scheduler,100,5,.05,len(self.messages))
