...
class ImprovedQLearningScheduler(Scheduler):
    ...
    def choose_processor(self, task, scheduled_tasks=None):
        """Choose a processor using an epsilon-greedy strategy with emphasis on immediate execution."""
        if random.random() < self.epsilon:
            return random.choice(range(1, len(self.processors) + 1))
        else:
            # For tasks with predecessors, heavily favor processors that allow immediate execution
            if task.predecessors:
                ...
                for proc_id in range(1, len(self.processors) + 1):
                    ...
                    if pred_finish_times:
                        latest_pred_finish = max(pred_finish_times)
                        delay = est - latest_pred_finish
                        
                        # Calculate immediate execution score (higher is better)
                        # The smaller the delay, the higher the score
                        immediate_score = 1000 - (delay * 100)
                        
                        # Calculate locality bonus
                        locality_score = sum(50 for pred_id in task.predecessors 
                                        if self.get_task_by_id(pred_id).assigned_processor == proc_id)
                        
                        # Combine scores, heavily favoring immediate execution
                        total_score = immediate_score + locality_score
                        
                        candidate_processors.append((proc_id, total_score))
                
                if candidate_processors:
                    # Select processor with highest score
                    return max(candidate_processors, key=lambda x: x[1])[0]
            
            # Fallback to standard q-value selection
    ...
    def calculate_improved_reward(self, task, processor_id):
        """Enhanced reward function with extreme emphasis on immediate execution."""
        ...
        for pred_id in task.predecessors:
            ...
            if pred_task and pred_task.assigned_processor == processor_id:
                locality_bonus += 20  # Reward keeping dependent tasks on same processor
        ...
        for other_task in self.tasks:
            if other_task.is_scheduled and other_task.task_id != task.task_id:
                ...
                if (other_task.finish_time >= task.start_time and
                    ...
                    parallel_bonus += 30  # Increased weight for parallelism
        ...
        # EXTREME IMMEDIATE EXECUTION FOCUS
        ...
        if task.predecessors:
            ...
            if pred_tasks:
                ...
                if delay == 0:  # Perfect immediate execution
                    immediate_start_bonus = 500  # Extremely high bonus for zero delay
                elif delay <= 5:  # Very small tolerance for scheduling decisions
                    immediate_start_bonus = 400 - (delay * 80)  # Massive bonus that decreases rapidly with delay
                else:
                    immediate_start_bonus = -delay * 30  # Very steep penalty for delays
        
        # Combine all factors into final reward, with immediate execution having highest weight
        reward = (
            (slack > 0) * 30 +  # Binary reward for meeting deadline
            min(0, slack) * 0.2 +  # Reduced penalty for deadline overrun
            load_balance_score +  # Load balancing score
            locality_bonus +  # Reward for keeping related tasks together
            parallel_bonus +  # Reward for parallelism (increased)
            critical_path_bonus +  # Reward for critical path tasks
            immediate_start_bonus -  # DOMINANT factor - massive reward for immediate execution
            sec_overhead * 0.05  # Reduced penalty for security overhead
        )
    ...
    def schedule_tasks(self):
        """Schedule tasks using improved Q-learning with SHIELD seeding and immediate execution focus."""
        ...
        for episode in range(self.episodes):
            ...
            # Create a priority queue for future task completion events
            future_task_events = []
            
            # Track the last completed task's time to help prioritize immediate execution
            last_completed_tasks = {}  # Dictionary mapping task_id to finish_time
            
            while not all_scheduled and safety_counter < max_iterations:
                safety_counter += 1
                
                # Check if any tasks become ready at current_time
                newly_ready_tasks = []
                for task in sorted_tasks:
                    if not task.is_scheduled and task not in ready_tasks:
                        ...
                        if all_preds_scheduled:
                            # Get the max finish time of all predecessors
                            latest_pred_finish = max([self.get_task_by_id(pred_id).finish_time 
                                                    for pred_id in task.predecessors], default=0)
                            
                            # Check if all predecessors have finished by current_time
                            if latest_pred_finish <= current_time:
                                newly_ready_tasks.append((task, latest_pred_finish))
                
                # Sort newly ready tasks by predecessor finish time (recent first)
                # This prioritizes tasks that just became ready
                newly_ready_tasks.sort(key=lambda x: -x[1])
                
                # Add newly ready tasks to the ready queue
                for task, _ in newly_ready_tasks:
                    ready_tasks.append(task)
                
                if ready_tasks:
                    # Calculate readiness scores with focus on immediate execution after predecessors
                    task_readiness = []
                    for task in ready_tasks:
                        if not task.predecessors:
                            # Tasks with no predecessors have baseline readiness
                            task_readiness.append((task, task.priority))
                        else:
                            # For tasks with predecessors, calculate how long they've been waiting
                            pred_tasks = [self.get_task_by_id(pred_id) for pred_id in task.predecessors 
                                        if self.get_task_by_id(pred_id).is_scheduled]
                            
                            if pred_tasks:
                                # Get finish time of most recently completed predecessor
                                latest_pred = max(pred_tasks, key=lambda t: t.finish_time)
                                latest_pred_finish = latest_pred.finish_time
                                
                                # Calculate waiting time and immediate execution score
                                waiting_time = current_time - latest_pred_finish
                                
                                # Heavily favor tasks that just became eligible to run
                                if waiting_time == 0:
                                    readiness_score = 1000 + task.priority  # Extremely high score
                                else:
                                    # Score decreases rapidly with waiting time
                                    readiness_score = 500 + task.priority - (waiting_time * 50)
                                    
                                task_readiness.append((task, readiness_score))
                            else:
                                # Task with unscheduled predecessors gets lower score
                                task_readiness.append((task, task.priority))
                    
                    # Sort by readiness score (higher is more ready)
                    task_readiness.sort(key=lambda x: -x[1])
                    current_task = task_readiness[0][0]
                    ready_tasks.remove(current_task)
                    
                    # Choose best processor with focus on immediate execution
                    proc_candidates = []
                    for proc_id in range(1, len(self.processors) + 1):
                        processor = self.processors[proc_id - 1]
                        est = self.calculate_est(current_task, proc_id)
                        
                        # Calculate immediate execution score
                        immediate_score = 0
                        if current_task.predecessors:
                            ...
                            if pred_finish_times:
                                ...
                                delay = est - latest_pred_finish
                                if delay == 0:
                                    immediate_score = 1000  # Perfect timing
                                else:
                                    immediate_score = 500 - (delay * 100)
                        
                        # Calculate locality score
                        locality_score = sum(50 for pred_id in current_task.predecessors 
                                        if self.get_task_by_id(pred_id).assigned_processor == proc_id)
                        
                        # Calculate processor availability score
                        availability_score = 0
                        if processor.available_time <= current_time:
                            availability_score = 200  # Processor is immediately available
                        else:
                            availability_score = 100 - min(100, processor.available_time - current_time)
                        
                        # Calculate total score for this processor
                        total_score = immediate_score + locality_score + availability_score
                        proc_candidates.append((proc_id, est, total_score))
                    
                    # Choose processor with exploration/exploitation balance
                    if random.random() < self.epsilon:  # Exploration
                        proc_id = random.choice(range(1, len(self.processors) + 1))
                        est = self.calculate_est(current_task, proc_id)
                    else:  # Exploitation - use our processor scoring
                        # Sort candidates by score (higher is better)
                        proc_candidates.sort(key=lambda x: -x[2])
                        proc_id, est, _ = proc_candidates[0]
                    
                    processor = self.processors[proc_id - 1]
                    
                    # Calculate EST and EFT, ensuring we don't start before current_time or processor available time
                    est = max(current_time, est, processor.available_time)
                    ...
                    # Track this completed task
                    last_completed_tasks[current_task.task_id] = eft
                    ...
                    # Add a future event for when this task finishes
                    future_task_events.append((eft, current_task))
                    ...
                    # No tasks ready at current time, advance time to next event
                    next_times = []
                    
                    # Consider processor available times
                    for proc in self.processors:
                        if proc.available_time > current_time:
                            next_times.append(proc.available_time)
                    
                    # Consider future task completion events
                    for time, task in future_task_events:
