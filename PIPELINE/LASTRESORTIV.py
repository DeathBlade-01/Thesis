...
class SHIELD(Scheduler):
    ...
    def calculate_benefits(self, message, source_task, dest_task):
        ...
        for service in ['confidentiality', 'integrity', 'authentication']:
            ...
            if time_penalty > 0:
                ...
                benefit_cost_ratio = float('inf')  # Free security upgrade
    ...
    def recalculate_makespan_after_upgrade(self):
        ...
        # Reset all task start and finish times
        for task in self.tasks:
        ...
        # Topologically sort tasks for scheduling
        scheduled_tasks = [t for t in self.tasks if t.is_scheduled]
        task_queue = []
        
        # Add tasks with no predecessors or with all predecessors scheduled
        pending_tasks = list(scheduled_tasks)
        while pending_tasks:
            for task in list(pending_tasks):
                all_preds_done = all(
                    self.get_task_by_id(pred_id).is_scheduled and
                    self.get_task_by_id(pred_id).finish_time is not None
                    for pred_id in task.predecessors if self.get_task_by_id(pred_id)
                )
        ...
        # Now schedule tasks in sorted order
        ...
        # Update the makespan
        makespan = max(task.finish_time for task in self.tasks if task.is_scheduled and task.finish_time is not None)
        return makespan
    
    def test_upgrade_feasibility(self, upgrade):
        """Test if applying an upgrade still meets the deadline constraint."""
        # Create a temporary copy of the message to test the upgrade
    ...
    def apply_security_upgrade(self, upgrade):
        ...
        # Return the actual message object that was updated
        return message
    ...
    def run(self):
        ...
        # Deep copy the HSMS results to SHIELD
        for task in self.tasks:
        ...
        try:
            while slack_time > 0 and upgrade_iterations < max_iterations:
                ...
                # Find potential security upgrades
                for message in self.messages:
                    source_task = self.get_task_by_id(message.source_id)
                    dest_task = self.get_task_by_id(message.dest_id)
                ...
                for i, upgrade in enumerate(all_benefits[:5]):  # Try the top 5 for debugging
                    print(f"Testing upgrade {i+1}/{min(5, len(all_benefits))}: {upgrade['message'].id}, {upgrade['service']}, level {upgrade['protocol_idx']}")
                    print(f"  Benefit: {upgrade['security_benefit']}, Penalty: {upgrade['time_penalty']}")
                    
                    # Only consider upgrades that consume less than the available slack
                    if upgrade['time_penalty'] <= slack_time:
                        # Test if applying this upgrade still meets the deadline
                        print(f"  Testing feasibility...")
                        try:
                            is_feasible, new_makespan = self.test_upgrade_feasibility(upgrade)
                            print(f"  Feasible: {is_feasible}, New makespan: {new_makespan}")
                        ...
                        if is_feasible:
                            # Apply the upgrade
                            print(f"  Applying upgrade...")
                            try:
                                self.apply_security_upgrade(upgrade)
                                
                                # Recalculate the makespan
                                ...
                                applied_upgrades.append({
                                    ...
                                    'service': upgrade['service'],
                                    'old_level': upgrade['protocol_idx'] - 1,
                                    'new_level': upgrade['protocol_idx'],
                                    'benefit': upgrade['security_benefit'],
                                    'penalty': upgrade['time_penalty'],
                                    'ratio': upgrade['benefit_cost_ratio'],
                                    'new_makespan': current_makespan
                                })
                                
                                print(f"Applied upgrade to {upgrade['message'].id}, {upgrade['service']} to level {upgrade['protocol_idx']}")
                                print(f"New makespan: {current_makespan}, remaining slack: {slack_time}")
            ...
            print(f"Error in security upgrade phase: {e}")
        
        # Final recalculation to ensure correctness
        try:
            self.makespan = self.recalculate_makespan_after_upgrade()
            security_utility = self.calculate_security_utility()
        except Exception as e:
            print(f"Error in final recalculation: {e}")
            return None, None
        
        end_time = time.time()
        print(f"SHIELD completed in {end_time - start_time:.4f} seconds")
        print(f"SHIELD final makespan: {self.makespan}, security utility: {security_utility}")
        print(f"Applied {len(applied_upgrades)} security upgrades")
        
        # Print upgrade summary
        if applied_upgrades:
            print("\nSecurity Upgrades Summary:")
            print("Message ID | Service | Old Level | New Level | Benefit | Time Cost | Benefit/Cost")
            print("-" * 80)
            for upg in applied_upgrades:
                print(f"{upg['message_id']} | {upg['service']:12} | {upg['old_level']:9} | {upg['new_level']:9} | "
                    f"{upg['benefit']:.4f} | {upg['penalty']:.4f} | {upg['ratio']:.4f}")
