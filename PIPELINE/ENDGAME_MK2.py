...
class QLSecurityScheduler(HSMS):
    ...
    def visualize_training(self):
        ...
        try:
            ...
            ax1.set_title('Security Utility vs Episode', fontsize=14)
            ax1.set_xlabel('Episode', fontsize=12)
            ax1.set_ylabel('Security Utility', fontsize=12)
            ax1.legend()
            ax1.grid(True)
            ...
            ax2.axhline(y=self.deadline, color='k', linestyle='--', alpha=0.7, label=f'Deadline ({self.deadline})')
            ax2.set_title('Makespan vs Episode', fontsize=14)
            ax2.set_xlabel('Episode', fontsize=12)
            ax2.set_ylabel('Makespan', fontsize=12)
            ax2.legend()
            ax2.grid(True)
            
            # 3. 3D plot: Security Utility vs Makespan vs Episode
            ...
            # Plot 3D scatter
            scatter = ax3.scatter(self.training_history['security_utility'], 
                       self.training_history['makespan'], 
                       self.training_history['episodes'],
                       c=episodes, cmap='viridis', marker='o', alpha=0.7)
            ...
            # Add deadline plane
            x_min, x_max = min(self.training_history['security_utility']), max(self.training_history['security_utility'])
            ...
            ax3.plot_surface(x_plane, z_plane, y_plane, color='r', alpha=0.2)
            
            # Set labels
            ax3.set_xlabel('Security Utility', fontsize=12)
            ax3.set_ylabel('Makespan', fontsize=12)
            ax3.set_zlabel('Episode', fontsize=12)
            ax3.set_title('Security Utility vs Makespan vs Episode', fontsize=14)
            
            # Mark the beginning and end points
            ax3.scatter([self.training_history['security_utility'][0]], 
                       [self.training_history['makespan'][0]], 
                       [self.training_history['episodes'][0]], 
                       color='blue', marker='s', s=100, label='Start')
            ax3.scatter([self.training_history['security_utility'][-1]], 
                       [self.training_history['makespan'][-1]], 
                       [self.training_history['episodes'][-1]], 
                       color='green', marker='*', s=100, label='End')
            ax3.legend()
            ...
            # Save figure
            filename = f"ql_training_priority_{self.security_priority:.2f}.png"
            plt.savefig(filename, dpi=300, bbox_inches='tight')
            print(f"Training visualization saved as '{filename}'")
...
def main():
    ...
    ql_shield = QLSecurityScheduler(copy.deepcopy(tasks), copy.deepcopy(messages),
                                    copy.deepcopy(processors), network, security_service, deadline,0.1,0.9,0.3,2000,0.5)
