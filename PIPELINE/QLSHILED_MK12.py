...
class ImprovedQLearningScheduler(Scheduler):
    ...
    def schedule_tasks(self):
        ...
        for episode in range(self.episodes):
            ...
            # Create a list to track all ready tasks
            ready_tasks = []
            
            # Initialize ready tasks - those with no predecessors or all predecessors already scheduled
            ...
            # Keep track of task dependencies and their earliest possible start times
            task_earliest_start = {task.task_id: 0 for task in self.tasks}
            
            # Keep scheduling until all tasks are scheduled
            while not all_scheduled and safety_counter < max_iterations:
                safety_counter += 1
                
                if not ready_tasks:
                    # Check if any new tasks have become ready
                    for task in sorted_tasks:
                        if not task.is_scheduled and task not in ready_tasks:
                            ...
                            if all(self.get_task_by_id(pred_id).is_scheduled for pred_id in task.predecessors):
                                ...
                                for pred_id in task.predecessors:
                                    ...
                                    # Consider communication time if on different processors
                                    # (we'll determine processor later, so this is an estimate)
                                    max_pred_finish = max(max_pred_finish, pred_task.finish_time)
                                
                                # Update earliest start time for this task
                                task_earliest_start[task.task_id] = max_pred_finish
                                ready_tasks.append(task)
                    
                    if not ready_tasks:
                        # No tasks ready yet, advance time to next event
                        # Find the next task that might become ready
                        next_ready_time = float('inf')
                        for task in sorted_tasks:
                            if not task.is_scheduled and task not in ready_tasks:
                                # Check how many predecessors are still unscheduled
                                unscheduled_preds = [pred_id for pred_id in task.predecessors 
                                                if not self.get_task_by_id(pred_id).is_scheduled]
                                if len(unscheduled_preds) <= 1:  # At most one predecessor left
                                    for pred_id in unscheduled_preds:
                                        pred_task = self.get_task_by_id(pred_id)
                                        if pred_task in ready_tasks:
                                            # This predecessor is about to be scheduled, so consider it
                                            next_ready_time = min(next_ready_time, 
                                                                max(proc.available_time for proc in self.processors))
                        
                        if next_ready_time == float('inf'):
                            # If we can't find next ready time, use processor availability
                            processor_times = [p.available_time for p in self.processors if p.available_time > 0]
                            if processor_times:
                                next_ready_time = min(processor_times)
                            else:
                                next_ready_time = 1  # Minimal increment
                        
                        # No need to actually advance time since we're event-based
                        continue
                
                # Sort ready tasks by priority AND earliest possible start time
                # This is a key change: prioritize tasks that can start earlier
                ready_tasks.sort(key=lambda x: (-x.priority, task_earliest_start[x.task_id]))
                
                # Select the highest priority task
                current_task = ready_tasks.pop(0)
                
                # Try each processor and pick the one that allows earliest finish time
                best_proc_id = None
                best_est = float('inf')
                best_eft = float('inf')
                
                for proc_id in range(1, len(self.processors) + 1):
                    # Calculate actual EST considering predecessors, communication, and processor availability
                    est = self.calculate_est(current_task, proc_id)
                    eft = est + current_task.execution_times[proc_id - 1]
                    
                    # Use Q-values as a tiebreaker when EST/EFT are similar
                    q_value = self.q_table.get((current_task.task_id, proc_id), 0)
                    
                    # If this processor gives us an earlier finish time, select it
                    # Allow a small tolerance to prevent bouncing between similar solutions
                    if eft < best_eft - 1 or (abs(eft - best_eft) <= 1 and q_value > self.q_table.get((current_task.task_id, best_proc_id), 0)):
                ...
                # Schedule the task with no unnecessary waiting - this is crucial
                current_task.assigned_processor = best_proc_id
                current_task.start_time = best_est
                current_task.finish_time = best_eft
                current_task.is_scheduled = True
                processor.available_time = best_eft
                ...
                # Update ready tasks - check if new tasks are now ready
                for task in sorted_tasks:
                    if not task.is_scheduled and task not in ready_tasks:
                        # Check if all predecessors are scheduled
                        if all(self.get_task_by_id(pred_id).is_scheduled for pred_id in task.predecessors):
                            ...
                            for pred_id in task.predecessors:
                                pred_task = self.get_task_by_id(pred_id)
                                if pred_task.assigned_processor != best_proc_id:
                                    ...
                                    if message:
                                        comm_time = self.calculate_communication_time(
                                            message, pred_task.assigned_processor, best_proc_id)
                ...
                # Calculate reward for this scheduling decision - emphasize immediate execution
                reward = self.calculate_immediate_reward(current_task, best_proc_id)
                
                # Find next task that will be scheduled
                next_task = None
                if ready_tasks:
                    ready_tasks.sort(key=lambda x: (-x.priority, task_earliest_start[x.task_id]))
    ...
    def calculate_immediate_reward(self, task, processor_id):
        """Enhanced reward function that heavily rewards immediate start after predecessors"""
        # Calculate slack time (how much before deadline)
        slack = self.deadline - task.finish_time
        
        # Calculate waiting time after predecessors
        waiting_penalty = 0
        if task.predecessors:
            # Find the maximum finish time of all predecessors
            max_pred_finish = 0
            for pred_id in task.predecessors:
                pred_task = self.get_task_by_id(pred_id)
                if pred_task and pred_task.is_scheduled:
                    ...
                    if pred_task.assigned_processor != processor_id:
                        # Add communication time if on different processors
                        message = self.get_message(pred_id, task.task_id)
                        if message:
                            pred_finish += self.calculate_communication_time(
                                message, pred_task.assigned_processor, processor_id)
                    max_pred_finish = max(max_pred_finish, pred_finish)
            
            # Calculate waiting time (how long after predecessors finish before this task starts)
            waiting_time = max(0, task.start_time - max_pred_finish)
            
            # Heavily penalize any waiting time - this is the key fix
            if waiting_time > 0:
                waiting_penalty = -0.005 * waiting_time
            else:
                # Reward starting immediately after predecessors with a high bonus
                waiting_penalty = 2000  # Large positive reward for zero waiting
        ...
        # The waiting penalty now plays a much bigger role
        reward = (
            (slack > 0) * 50 +  # Binary reward for meeting deadline
            min(0, slack) * 0.5 +  # Small penalty for how much over deadline
            # waiting_penalty +  # Very important: large penalty for any waiting or reward for immediate execution
            load_balance_score +  # Load balancing score (smaller impact)
    ...
    def calculate_est(self, task, processor_id):
        ...
        for pred_id in task.predecessors:
            ...
            if not pred_task.is_scheduled:
                # If any predecessor is not scheduled yet, we can't calculate accurate EST
                continue
