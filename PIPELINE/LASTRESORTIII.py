...
class SHIELD(Scheduler):
    ...
    def apply_security_upgrade(self, upgrade):
        """Apply a security upgrade and update task execution times accordingly."""
        message = upgrade['message']
        service = upgrade['service']
        protocol_idx = upgrade['protocol_idx']
        source_task = upgrade['source_task']
        dest_task = upgrade['dest_task']
        
        # Store previous security level
        prev_protocol_idx = message.assigned_security[service]
        
        # Update message security level
        message.assigned_security[service] = protocol_idx
        
        # Calculate execution time impact
        if service in ['confidentiality', 'integrity']:
            # These overheads apply to the sender (source task)
            prev_protocol = prev_protocol_idx + 1
            new_protocol = protocol_idx + 1
            
            prev_overhead = (message.size / 1024) * self.security.overheads[service][prev_protocol][source_task.assigned_processor - 1]
            new_overhead = (message.size / 1024) * self.security.overheads[service][new_protocol][source_task.assigned_processor - 1]
            
            # Update source task finish time
            source_task.finish_time += (new_overhead - prev_overhead)
            ...
            prev_overhead = self.security.overheads[service][prev_protocol][dest_task.assigned_processor - 1]
            new_overhead = self.security.overheads[service][new_protocol][dest_task.assigned_processor - 1]
            
            # Update destination task finish time
            dest_task.finish_time += (new_overhead - prev_overhead)
        
        # Update the schedule entry
        for entry in self.schedule:
            if entry['task_id'] == source_task.task_id:
                entry['finish_time'] = source_task.finish_time
            elif entry['task_id'] == dest_task.task_id:
                entry['finish_time'] = dest_task.finish_time
    
    def recalculate_makespan_after_upgrade(self):
        """Recalculate task start times, finish times and makespan after security upgrades."""
        # Sort tasks by start time to process them in order
        sorted_tasks = sorted([t for t in self.tasks if t.is_scheduled], 
        ...
        for task in sorted_tasks:
            ...
            for pred_id in task.predecessors:
                ...
                if pred_task.assigned_processor == processor_id:
                    ...
                    # Calculate communication time with security overhead for inter-processor communication
                    comm_time = self.network.get_communication_time(message, pred_task.assigned_processor, processor_id)
                    security_overhead = self.calc_security_overhead(message, pred_task.assigned_processor, processor_id)
                    pred_finish = pred_task.finish_time + comm_time + security_overhead
            ...
            for succ_id in task.successors:
                succ_task = self.get_task_by_id(succ_id)
                if (succ_task and succ_task.is_scheduled and 
                    succ_task.assigned_processor != processor_id):
                    message = self.get_message(task.task_id, succ_id)
                    if message:
                        # Apply sender confidentiality and integrity overhead
                        for service in ['confidentiality', 'integrity']:
    ...
    def run(self):
        ...
        # First recalculation to ensure proper start
        current_makespan = self.recalculate_makespan_after_upgrade()
        
        # Initialize the slack time
        slack_time = self.deadline - current_makespan
        
        # Keep track of upgrades for debugging/reporting
        applied_upgrades = []
        ...
        while slack_time > 0:
            ...
            found_viable_upgrade = False
            
            for upgrade in all_benefits:
                if upgrade['time_penalty'] <= slack_time:
                    # Create temp copies to test the upgrade
                    tasks_copy = copy.deepcopy(self.tasks)
                    messages_copy = copy.deepcopy(self.messages)
                    schedule_copy = copy.deepcopy(self.schedule)
                    
                    # Apply upgrade in the temp copy
                    message_copy = next((m for m in messages_copy if m.source_id == upgrade['message'].source_id 
                                        and m.dest_id == upgrade['message'].dest_id), None)
                    source_task_copy = next((t for t in tasks_copy if t.task_id == upgrade['source_task'].task_id), None)
                    dest_task_copy = next((t for t in tasks_copy if t.task_id == upgrade['dest_task'].task_id), None)
                    
                    # Skip if we couldn't find the copies
                    if not message_copy or not source_task_copy or not dest_task_copy:
                        continue
                    
                    # Store current security level
                    prev_level = message_copy.assigned_security[upgrade['service']]
                    
                    # Apply the upgrade in the temp copy
                    message_copy.assigned_security[upgrade['service']] = upgrade['protocol_idx']
                    
                    # Test if this upgrade still meets the deadline
                    temp_scheduler = SHIELD(tasks_copy, messages_copy, copy.deepcopy(self.processors), 
                                          self.network, self.security, self.deadline)
                    temp_scheduler.schedule = schedule_copy
                    
                    # Calculate new makespan
                    new_makespan = temp_scheduler.recalculate_makespan_after_upgrade()
                    
                    if new_makespan <= self.deadline:
                        # Apply the upgrade to the real schedule
                        self.apply_security_upgrade(upgrade)
                        applied_upgrades.append(upgrade)
                        
                        # Recalculate makespan and slack time
                        current_makespan = self.recalculate_makespan_after_upgrade()
                        slack_time = self.deadline - current_makespan
                        
                        found_viable_upgrade = True
                        break
                    
            # If no viable upgrade is found, exit the loop
            if not found_viable_upgrade:
                break
        
        # Final recalculation to ensure all task times are consistent
        self.makespan = self.recalculate_makespan_after_upgrade()
        
        # Calculate final security utility for inter-processor messages only
        security_utility = 0
        for message in self.messages:
            source_task = self.get_task_by_id(message.source_id)
            dest_task = self.get_task_by_id(message.dest_id)
        ...
        end_time = time.time()
        print(f"SHIELD successful. Makespan: {self.makespan}, Security Utility: {security_utility}, Time Elapsed: {(end_time-start_time):.4f}")
