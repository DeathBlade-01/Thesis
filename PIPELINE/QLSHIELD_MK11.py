...
class ImprovedQLearningScheduler(Scheduler):
    ...
    def schedule_tasks(self):
        ...
        for episode in range(self.episodes):
            ...
            pred_completion_times = {task.task_id: 0 for task in self.tasks}
            
            # Create a time-based scheduling approach
            ready_tasks = [task for task in sorted_tasks if not task.predecessors]
            ...
            while not all_scheduled and safety_counter < max_iterations:
                ...
                # Handle tasks that just completed and make their successors ready
                newly_ready = []
                    ...
                    for task in sorted_tasks:
                        if not task.is_scheduled and task not in ready_tasks:
                            if completed_task.task_id in task.predecessors:
                                pred_completion_times[task.task_id] = max(pred_completion_times[task.task_id], completed_task.finish_time)
                ...
                if ready_tasks:
                    # Sort ready tasks by priority and their earliest start time
                    task_scores = []
                    for task in ready_tasks:
                        # Fixed: Use a default processor (1) instead of None for unassigned tasks
                        # We choose processor 1 arbitrarily for the EST calculation
                        proc_to_check = 1
                        est = self.calculate_est(task, proc_to_check)
                        score = task.priority - (current_time - est)  # Penalize for waiting time
                    ...
                    for proc_id in range(1, len(self.processors) + 1):
                        ...
                        if current_task.predecessors:
                            # Fixed: Check if there are any scheduled predecessors
                            scheduled_preds = [self.get_task_by_id(pred_id).finish_time 
                                            for pred_id in current_task.predecessors 
                                            if self.get_task_by_id(pred_id).is_scheduled]
                            max_pred_finish = max(scheduled_preds) if scheduled_preds else 0
                    ...
                    # Calculate reward with a focus on minimizing gaps
            ...
            if all(task.is_scheduled for task in self.tasks):
                ...
                print(f"Episode {episode}: Failed to schedule all tasks within max iterations")
