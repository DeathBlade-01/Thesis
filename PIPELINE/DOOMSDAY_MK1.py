...
class GeneticAlgorithm:
    def __init__(self, tasks, messages, processors, network, security_service, deadline,
                 population_size=50, elite_size=5, mutation_rate=0.2, crossover_rate=0.8,
                 max_generations=100, k=0.5):
        self.tasks = tasks
        self.messages = messages
        self.processors = processors
        self.network = network
        ...
        self.k = k  # Weight parameter for balancing makespan vs security utility
        
        # Create initial HSMS solution to use as seed
        self.hsms_scheduler = HSMS(self.tasks, self.messages, self.processors, 
                                   self.network, self.security_service, self.deadline)
        self.hsms_makespan, self.hsms_security_utility = self.hsms_scheduler.run()
    ...
    def initialize_population(self):
        """Initialize population using HSMS solution and variations"""
        # First solution is the HSMS solution
        hsms_solution = {
            'task_order': [t.task_id for t in self.tasks if t.is_scheduled],
            'processor_assignments': {t.task_id: t.assigned_processor for t in self.tasks if t.is_scheduled},
            'security_assignments': {m.id: copy.deepcopy(m.assigned_security) for m in self.messages},
            'makespan': self.hsms_makespan,
            'security_utility': self.hsms_security_utility
        }
        
        self.population.append(hsms_solution)
        
        # Generate variations of HSMS solution for the rest of the population
        for _ in range(self.population_size - 1):
            # Create a new solution by mutating the HSMS solution
            new_solution = self.mutate(copy.deepcopy(hsms_solution))
            
            # Evaluate the new solution
            makespan, security_utility = self.evaluate_solution(new_solution)
            new_solution['makespan'] = makespan
            new_solution['security_utility'] = security_utility
            
            self.population.append(new_solution)
    
    def get_valid_task_order(self, task_order=None):
        """Generate a valid task order respecting precedence constraints"""
        if task_order is None:
            # Start with all tasks
            remaining_tasks = copy.deepcopy(self.tasks)
            task_order = []
            ...
            while remaining_tasks:
                ...
                for task in remaining_tasks:
                    predecessors_scheduled = all(pred_id in [t.task_id for t in task_order] 
                                              for pred_id in task.predecessors)
                    if predecessors_scheduled:
                        schedulable_tasks.append(task)
                
                # Choose a random task from schedulable tasks
            ...
            return [t.task_id for t in task_order]
        else:
            # Verify and repair the provided task order
            task_ids = [t.task_id for t in self.tasks]
            scheduled = set()
            valid_order = []
            
            # Try to keep existing order if it respects precedence
            for task_id in task_order:
                if task_id not in scheduled and task_id in task_ids:
                    task = next(t for t in self.tasks if t.task_id == task_id)
                    if all(pred_id in scheduled for pred_id in task.predecessors):
                        valid_order.append(task_id)
                        scheduled.add(task_id)
            
            # Add any remaining tasks in valid order
            remaining_tasks = [t for t in self.tasks if t.task_id not in scheduled]
            while remaining_tasks:
                made_progress = False
                for task in remaining_tasks[:]:
                    if all(pred_id in scheduled for pred_id in task.predecessors):
                        valid_order.append(task.task_id)
    ...
    def select_parents(self):
        ...
        for solution in self.population:
            if solution['makespan'] > self.deadline:
                # Penalize solutions that don't meet deadline
                fitness = 0
            else:
                # Reward based on K parameter: (1-K) * makespan_reduction + K * security_improvement
                makespan_reduction = 1 - (solution['makespan'] / self.deadline)
                security_improvement = solution['security_utility'] / max(s['security_utility'] for s in self.population)
                fitness = (1 - self.k) * makespan_reduction + self.k * security_improvement
            
            fitness_scores.append(fitness)
        
        # Convert to selection probabilities
        total_fitness = sum(fitness_scores)
        selection_probs = [f/total_fitness for f in fitness_scores] if total_fitness > 0 else [1/len(fitness_scores)] * len(fitness_scores)
        
        # Select parents
        parent_indices = np.random.choice(len(self.population), size=2, p=selection_probs)
        return self.population[parent_indices[0]], self.population[parent_indices[1]]
    
    def crossover(self, parent1, parent2):
        """Perform order preserving crossover"""
        if random.random() > self.crossover_rate:
            return copy.deepcopy(parent1)
        
        # Task order crossover - Order Crossover (OX1)
        task_order1 = parent1['task_order']
        task_order2 = parent2['task_order']
        
        # Select two random crossover points
        start, end = sorted(random.sample(range(len(task_order1)), 2))
        
        # Create offspring
        middle_section = task_order1[start:end]
        remaining_tasks = [t for t in task_order2 if t not in middle_section]
        
        # Construct offspring using order crossover
        child_order = remaining_tasks[:start] + middle_section + remaining_tasks[start:]
        
        # Make sure it's a valid task order (respecting precedence)
        child_order = self.get_valid_task_order(child_order)
        
        # Processor assignment crossover - uniform crossover
        child_processor_assignments = {}
        for task_id in child_order:
            if random.random() < 0.5:
                child_processor_assignments[task_id] = parent1['processor_assignments'].get(task_id, 1)
            else:
                child_processor_assignments[task_id] = parent2['processor_assignments'].get(task_id, 1)
        
        # Security assignment crossover - uniform crossover
        child_security_assignments = {}
        for message_id in parent1['security_assignments'].keys():
            child_security = {}
            for service in ['confidentiality', 'integrity', 'authentication']:
                if random.random() < 0.5:
                    child_security[service] = parent1['security_assignments'][message_id][service]
                else:
                    child_security[service] = parent2['security_assignments'][message_id][service]
            child_security_assignments[message_id] = child_security
        
        # Create child solution
        child = {
            'task_order': child_order,
            'processor_assignments': child_processor_assignments,
            'security_assignments': child_security_assignments
        }
    ...
    def mutate(self, solution):
        ...
        # Task order mutation - swap two random tasks (respecting precedence)
        if random.random() < self.mutation_rate:
            task_order = solution['task_order']
            
            # Try up to 10 times to find a valid swap
            for _ in range(10):
                i, j = sorted(random.sample(range(len(task_order)), 2))
                
                # Create a new order with the swap
                new_order = task_order.copy()
                new_order[i], new_order[j] = new_order[j], new_order[i]
                
                # Check if this is a valid order (respecting precedence)
                valid_order = self.get_valid_task_order(new_order)
                if valid_order == new_order:
                    solution['task_order'] = new_order
                    break
        
        # Processor assignment mutation
        for task_id in solution['processor_assignments'].keys():
            if random.random() < self.mutation_rate:
                current_proc = solution['processor_assignments'][task_id]
                # Choose a new processor (different from current)
                new_proc = random.choice([p for p in range(1, len(self.processors) + 1) if p != current_proc])
                solution['processor_assignments'][task_id] = new_proc
        
        # Security assignment mutation
        for message_id in solution['security_assignments'].keys():
            for service in ['confidentiality', 'integrity', 'authentication']:
                if random.random() < self.mutation_rate:
                    max_idx = len(self.security_service.strengths[service]) - 1
                    new_level = random.randint(0, max_idx)
                    solution['security_assignments'][message_id][service] = new_level
        
        return solution
    
    def evaluate_solution(self, solution):
        """Evaluate a solution by calculating makespan and security utility"""
        # Create a copy of the original task and message lists
        tasks_copy = copy.deepcopy(self.tasks)
        messages_copy = copy.deepcopy(self.messages)
        
        # Apply the solution's task order and processor assignments
        for task in tasks_copy:
            if task.task_id in solution['processor_assignments']:
                task.assigned_processor = solution['processor_assignments'][task.task_id]
                task.is_scheduled = True
            else:
                task.is_scheduled = False
        
        # Apply the solution's security assignments
        for message in messages_copy:
            if message.id in solution['security_assignments']:
                message.assigned_security = solution['security_assignments'][message.id]
        
        # Reset processor available times
        for proc in self.processors:
            proc.available_time = 0
        
        # Calculate task start and finish times based on order
        task_order = solution['task_order']
        
        # Schedule tasks according to the given order
        for task_id in task_order:
            task = next((t for t in tasks_copy if t.task_id == task_id), None)
            if not task:
                continue
            ...
            for pred_id in task.predecessors:
                ...
                message = next((m for m in messages_copy 
                            if m.source_id == pred_id and m.dest_id == task.task_id), None)
                if not message:
                    continue
                ...
                if pred_task.assigned_processor == processor_id:
                    ...
                    # Calculate communication time with security overhead
                    source_proc = self.processors[pred_task.assigned_processor - 1]
                    dest_proc = processor
            ...
            # Calculate security overhead for all messages to/from this task
            security_overhead = 0
            ...
            for pred_id in task.predecessors:
                ...
                # Add security overhead for receiver side
                for service in ['confidentiality', 'integrity']:
                    protocol_idx = message.assigned_security[service] + 1  # 1-indexed
                    overhead_factor = self.security_service.overheads[service][protocol_idx][processor_id - 1]
                    security_overhead += message.size / overhead_factor
                
                # Authentication overhead
                auth_idx = message.assigned_security['authentication'] + 1
                security_overhead += self.security_service.overheads['authentication'][auth_idx][processor_id - 1]
            ...
            for succ_id in task.successors:
                ...
                message = next((m for m in messages_copy 
                            if m.source_id == task.task_id and m.dest_id == succ_id), None)
                if not message:
                    continue
                
                # Add security overhead for sender side
                for service in ['confidentiality', 'integrity']:
                    protocol_idx = message.assigned_security[service] + 1  # 1-indexed
                    overhead_factor = self.security_service.overheads[service][protocol_idx][processor_id - 1]
                    security_overhead += message.size / overhead_factor
        ...
        security_utility = self.calculate_security_utility(messages_copy)
        
        return makespan, security_utility

    def calc_security_overhead(self, message, source_processor_id, dest_processor_id):
        """Calculate security overhead for a message"""
        security_overhead = 0
        
        # Add security overhead for sender side (source processor)
        for service in ['confidentiality', 'integrity']:
            protocol_idx = message.assigned_security[service] + 1  # 1-indexed
            overhead_factor = self.security_service.overheads[service][protocol_idx][source_processor_id - 1]
            security_overhead += message.size / overhead_factor
        
        # Authentication overhead for sender
        auth_idx = message.assigned_security['authentication'] + 1
        security_overhead += self.security_service.overheads['authentication'][auth_idx][source_processor_id - 1]
        
        # Add security overhead for receiver side (destination processor)
        for service in ['confidentiality', 'integrity']:
            protocol_idx = message.assigned_security[service] + 1  # 1-indexed
            overhead_factor = self.security_service.overheads[service][protocol_idx][dest_processor_id - 1]
            security_overhead += message.size / overhead_factor
        
        # Authentication overhead for receiver
        security_overhead += self.security_service.overheads['authentication'][auth_idx][dest_processor_id - 1]
        
        return security_overhead

    def calculate_security_utility(self, messages=None):
        """Calculate total security utility across all messages"""
        if messages is None:
        ...
        for message in messages:
            ...
            for service in ['confidentiality', 'integrity', 'authentication']:
                strength_idx = message.assigned_security[service]
                strength = self.security_service.strengths[service][strength_idx]
                message_utility += strength * message.weights[service]
            
            # Weight by message importance
            total_utility += message_utility * message.importance
    ...
    def get_message(self, source_id, dest_id):
        """Helper method to get a message between two tasks"""
        for message in self.messages:
            if message.source_id == source_id and message.dest_id == dest_id:
                return message
        return None
    def create_new_generation(self):
        ...
        while len(new_population) < self.population_size:
            ...
            # Evaluate child
            makespan, security_utility = self.evaluate_solution(child)
            child['makespan'] = makespan
            child['security_utility'] = security_utility
    ...
    def calculate_fitness(self, solution):
        """Calculate fitness score for a solution based on K parameter"""
        if solution['makespan'] > self.deadline:
            return float('-inf')  # Penalize solutions that don't meet deadline
        
        # Calculate fitness score balancing makespan and security utility
        makespan_component = 1 - (solution['makespan'] / self.deadline)
        security_component = solution['security_utility'] / max(s['security_utility'] for s in self.population) if self.population else 0
        
        return (1 - self.k) * makespan_component + self.k * security_component
    
    def run(self):
        """Run the genetic algorithm"""
        # Initialize tracking variables
        best_makespan_history = []
        best_security_history = []
        avg_makespan_history = []
        avg_security_history = []
        
        # Run for maximum generations
        for generation in range(self.max_generations):
            # Create new generation
            self.create_new_generation()
            
            # Track best solution in this generation
            best_in_gen = max(self.population, key=self.calculate_fitness)
            
            # Track average values
            valid_solutions = [s for s in self.population if s['makespan'] <= self.deadline]
            avg_makespan = sum(s['makespan'] for s in valid_solutions) / len(valid_solutions) if valid_solutions else float('inf')
            avg_security = sum(s['security_utility'] for s in valid_solutions) / len(valid_solutions) if valid_solutions else 0
            
            # Update best solution overall
            if (best_in_gen['makespan'] <= self.deadline and 
                self.calculate_fitness(best_in_gen) > self.calculate_fitness(self.best_solution if self.best_solution else {'makespan': float('inf'), 'security_utility': 0})):
                self.best_solution = copy.deepcopy(best_in_gen)
                self.best_makespan = best_in_gen['makespan']
                self.best_security_utility = best_in_gen['security_utility']
                self.best_score = self.calculate_fitness(best_in_gen)
            
            # Record history
            best_makespan_history.append(best_in_gen['makespan'] if best_in_gen['makespan'] <= self.deadline else None)
            best_security_history.append(best_in_gen['security_utility'] if best_in_gen['makespan'] <= self.deadline else None)
            avg_makespan_history.append(avg_makespan if avg_makespan <= self.deadline else None)
            avg_security_history.append(avg_security if avg_makespan <= self.deadline else None)
            
            # Store generation data
            gen_data = {
                'generation': generation,
                'best_makespan': best_in_gen['makespan'],
                'best_security': best_in_gen['security_utility'],
                'avg_makespan': avg_makespan,
                'avg_security': avg_security,
                'valid_solutions': len(valid_solutions),
                'best_fitness': self.calculate_fitness(best_in_gen)
            }
            self.generation_history.append(gen_data)
            
            # Print progress every 10 generations
            if generation % 10 == 0:
                print(f"Generation {generation}: Best Makespan = {best_in_gen['makespan']:.2f}, "
                      f"Best Security = {best_in_gen['security_utility']:.4f}, "
                      f"Valid Solutions = {len(valid_solutions)}/{self.population_size}")
        
        # Print final results
        print("\nGA Optimization Results:")
        print(f"Initial HSMS:  Makespan = {self.hsms_makespan:.2f}, Security = {self.hsms_security_utility:.4f}")
        print(f"Best GA:       Makespan = {self.best_makespan:.2f}, Security = {self.best_security_utility:.4f}")
        print(f"Improvement:   Makespan = {((self.hsms_makespan - self.best_makespan) / self.hsms_makespan * 100):.2f}%, "
              f"Security = {((self.best_security_utility - self.hsms_security_utility) / self.hsms_security_utility * 100):.2f}%")
        
        return {
            'best_solution': self.best_solution,
            'best_makespan': self.best_makespan,
            'best_security_utility': self.best_security_utility,
            'history': {
                'best_makespan': best_makespan_history,
                'best_security': best_security_history,
                'avg_makespan': avg_makespan_history,
                'avg_security': avg_security_history
            },
            'generation_history': self.generation_history
        }
    
    def visualize_results(self):
        """Visualize the results of the GA optimization"""
        if not self.generation_history:
            print("No data to visualize. Run the algorithm first.")
            return
        
        # Extract data for plotting
        generations = [data['generation'] for data in self.generation_history]
        best_makespan = [data['best_makespan'] if data['best_makespan'] <= self.deadline else None for data in self.generation_history]
        best_security = [data['best_security'] for data in self.generation_history]
        avg_makespan = [data['avg_makespan'] if data['avg_makespan'] <= self.deadline else None for data in self.generation_history]
        avg_security = [data['avg_security'] for data in self.generation_history]
        
        # Filter out None values
        valid_indices = [i for i, m in enumerate(best_makespan) if m is not None]
        filtered_generations = [generations[i] for i in valid_indices]
        filtered_best_makespan = [best_makespan[i] for i in valid_indices]
        filtered_best_security = [best_security[i] for i in valid_indices]
        
        # Create a figure with 3 subplots
        fig = plt.figure(figsize=(18, 14))
        
        # 1. 3D plot of makespan vs security vs generation
        ax1 = fig.add_subplot(221, projection='3d')
        if filtered_generations:  # Only plot if we have valid data
            scatter = ax1.scatter(filtered_generations, filtered_best_makespan, filtered_best_security, 
                               c=filtered_generations, cmap='viridis', s=50, alpha=0.7)
            ax1.plot(filtered_generations, filtered_best_makespan, filtered_best_security, 'r-', alpha=0.5)
            
            # Add initial HSMS point
            ax1.scatter([0], [self.hsms_makespan], [self.hsms_security_utility], 
                       c='red', s=100, marker='*', label='HSMS')
            
            # Add best GA point
            ax1.scatter([filtered_generations[-1] if filtered_generations else 0], 
                       [self.best_makespan], [self.best_security_utility], 
                       c='green', s=100, marker='*', label='Best GA')
            
            ax1.set_xlabel('Generation')
            ax1.set_ylabel('Makespan')
            ax1.set_zlabel('Security Utility')
            ax1.set_title('3D Visualization of GA Optimization')
            plt.colorbar(scatter, ax=ax1, label='Generation')
            ax1.legend()
        else:
            ax1.text(0.5, 0.5, 0.5, "No valid solutions found", horizontalalignment='center',
                   verticalalignment='center', transform=ax1.transAxes)
        
        # 2. Makespan vs generation
        ax2 = fig.add_subplot(222)
        ax2.plot(generations, best_makespan, 'b-', marker='o', label='Best Makespan')
        ax2.plot(generations, avg_makespan, 'g--', marker='x', label='Average Makespan')
        ax2.axhline(y=self.hsms_makespan, color='r', linestyle='-', label='HSMS Makespan')
        ax2.axhline(y=self.deadline, color='k', linestyle='--', label='Deadline')
        ax2.set_xlabel('Generation')
        ax2.set_ylabel('Makespan')
        ax2.set_title('Makespan vs Generation')
        ax2.legend()
        ax2.grid(True)
        
        # 3. Security utility vs generation
        ax3 = fig.add_subplot(223)
        ax3.plot(generations, best_security, 'b-', marker='o', label='Best Security')
        ax3.plot(generations, avg_security, 'g--', marker='x', label='Average Security')
        ax3.axhline(y=self.hsms_security_utility, color='r', linestyle='-', label='HSMS Security')
        ax3.set_xlabel('Generation')
        ax3.set_ylabel('Security Utility')
        ax3.set_title('Security Utility vs Generation')
        ax3.legend()
        ax3.grid(True)
        
        # 4. Pareto frontier: makespan vs security
        ax4 = fig.add_subplot(224)
        
        # Create a list of non-dominated solutions
        solutions = []
        for data in self.generation_history:
            if data['best_makespan'] <= self.deadline:
                solutions.append((data['best_makespan'], data['best_security']))
        
        # Sort by makespan and security
        solutions_by_makespan = sorted(solutions, key=lambda x: x[0])
        
        # Extract pareto front (solutions with highest security utility for each makespan)
        makespan_to_security = defaultdict(float)
        for makespan, security in solutions:
            makespan_to_security[makespan] = max(makespan_to_security[makespan], security)
        
        pareto_front = [(makespan, security) for makespan, security in makespan_to_security.items()]
        pareto_front.sort(key=lambda x: x[0])  # Sort by makespan
        
        if pareto_front:
            # Plot pareto frontier
            pareto_makespan, pareto_security = zip(*pareto_front)
            ax4.plot(pareto_makespan, pareto_security, 'b-o', label='Pareto Front')
            
            # Add initial HSMS point
            ax4.scatter([self.hsms_makespan], [self.hsms_security_utility], 
                       c='red', s=100, marker='*', label='HSMS')
            
            # Add best GA point
            ax4.scatter([self.best_makespan], [self.best_security_utility], 
                       c='green', s=100, marker='*', label='Best GA')
            
            # Plot all solutions
            makespan_values, security_values = zip(*solutions)
            ax4.scatter(makespan_values, security_values, c='gray', alpha=0.3, label='All Solutions')
            
            ax4.set_xlabel('Makespan')
            ax4.set_ylabel('Security Utility')
            ax4.set_title('Pareto Frontier: Makespan vs Security Utility')
            ax4.legend()
            ax4.grid(True)
        else:
            ax4.text(0.5, 0.5, "No valid solutions found", horizontalalignment='center',
        ...
        summary_data = {
            'Metric': ['Makespan', 'Security Utility', 'Score (K={})'.format(self.k)],
            'HSMS': [self.hsms_makespan, self.hsms_security_utility, 
                    (1-self.k)*(1-self.hsms_makespan/self.deadline) + self.k*self.hsms_security_utility/self.best_security_utility],
            'Best GA': [self.best_makespan, self.best_security_utility, self.best_score],
            'Improvement (%)': [
                (self.hsms_makespan - self.best_makespan) / self.hsms_makespan * 100,
                (self.best_security_utility - self.hsms_security_utility) / self.hsms_security_utility * 100,
                'N/A'
            ]
        }
    ...
    def apply_solution_to_scheduler(self, solution):
        """Apply a solution to a new scheduler instance for detailed evaluation"""
        # Create copies of original data
        tasks_copy = copy.deepcopy(self.tasks)
        messages_copy = copy.deepcopy(self.messages)
        
        # Apply task processor assignments
        for task in tasks_copy:
            if task.task_id in solution['processor_assignments']:
                task.assigned_processor = solution['processor_assignments'][task.task_id]
                task.is_scheduled = True
        ...
        for task_id in task_order:
            ...
            # Calculate EST based on predecessors (same as in evaluate_solution)
            est = 0
            est = 0
            for pred_id in task.predecessors:
                pred_task = next((t for t in tasks_copy if t.task_id == pred_id), None)
...
def main():
    ...
    ga_shield = GeneticAlgorithm(copy.deepcopy(tasks), copy.deepcopy(messages),
                                         copy.deepcopy(processors), network, security_service, deadline)
    ga_makespan, ga_security_utility = ga_shield.run()
    plot_gantt_chart("GA Schedule", ga_shield.schedule, len(processors), ga_makespan)
    ga_shield.display_task_security_details()
