...
import random
import time
from SHIELD_MK1 import Task, Message, Processor, SecurityService, CommunicationNetwork, Scheduler, HSMS, SHIELD
...
class CombinedSecurityQLearningScheduler(Scheduler):
    ...
    def calculate_communication_time(self, message, source_proc, dest_proc):
        ...
        return base_comm_time + security_overhead
    ...
    def run_baselines(self):
        ...
        end_time = time.time()
        self.execution_times['SHIELD'] = end_time - start_time
        results['SHIELD'] = {
        ...
        end_time = time.time()
        self.execution_times['HSMS'] = end_time - start_time
    ...
    def schedule_tasks(self):
        ...
        for episode in range(self.episodes):
            ...
            if safety_counter >= max_iterations:
                ...
                self.episode_data.append({
                    ...
                    'security': current_security_utility,
                    'best_makespan': self.best_makespan,
                    'best_security': self.best_security_utility
                })
                continue
        ...
        self.enhance_security_with_slack()
        self.display_learning_progress_chart()
    
        
        return self.best_makespan
    
    def display_learning_progress_chart(self):
        """Display enhanced 3D chart with axis projections highlighting key points"""
        try:
            ...
            # Custom arrow class for 3D plots - updated for compatibility with newer matplotlib
            class Arrow3D(FancyArrowPatch):
                def __init__(self, xs, ys, zs, *args, **kwargs):
                    super().__init__((0, 0), (0, 0), *args, **kwargs)
                    self._verts3d = xs, ys, zs
                    self.axes = None  # Will be set when added to an axes
                ...
                def draw(self, renderer):
                    if self.axes is None:
                        # Find the axes if it hasn't been saved yet
                        self.axes = self.get_axes()
                    
                    xs3d, ys3d, zs3d = self._verts3d
                    # Use axes.M instead of renderer.M
            ...
            all_episodes = [data['episode'] for data in self.episode_data]
            all_makespans = [data['makespan'] for data in self.episode_data]
            all_security_values = [data['security'] for data in self.episode_data]
            
            # Determine sampling rate based on total number of episodes
            total_points = len(all_episodes)
            if total_points > 100:
                sample_rate = max(1, total_points // 60)
                
                # Sample data points
                episodes = all_episodes[::sample_rate]
                makespans = all_makespans[::sample_rate]
                security_values = all_security_values[::sample_rate]
                
                # Always include the last 5 episodes
                episodes += all_episodes[-5:]
                makespans += all_makespans[-5:]
                security_values += all_security_values[-5:]
            else:
                episodes = all_episodes
                makespans = all_makespans
                security_values = all_security_values
            
            # Create scatter plot with larger markers and increased transparency
            scatter = ax.scatter(episodes, makespans, security_values, 
                            c=episodes, cmap='viridis', 
                            s=70, alpha=0.6)
            ...
            current_best_security = 0
            
            improvement_threshold = 0.5  # % improvement needed to record a point
            
            for data in self.episode_data:
                is_significant_improvement = (
                    data['makespan'] < current_best_makespan * (1 - improvement_threshold/100) or
                    (abs(data['makespan'] - current_best_makespan) < 0.001 and 
                    data['security'] > current_best_security * (1 + improvement_threshold/100))
                )
                
                if is_significant_improvement:
                    current_best_makespan = data['makespan']
                    current_best_security = data['security']
                    best_episodes.append(data['episode'])
                    best_makespans.append(data['makespan'])
                    best_security.append(data['security'])
            
            # Always include first and final solutions
            if len(self.episode_data) > 0:
                # Add the initial solution
                first_data = self.episode_data[0]
                if not best_episodes or best_episodes[0] != 0:
                    best_episodes.insert(0, 0)
                    best_makespans.insert(0, first_data['makespan'])
                    best_security.insert(0, first_data['security'])
                
                # Add the final best solution
                final_data = self.episode_data[-1]
                final_best_makespan = final_data['best_makespan']
                final_best_security = final_data['best_security']
                final_episode = self.episodes - 1
                
                if not best_episodes or best_episodes[-1] != final_episode:
                    best_episodes.append(final_episode)
                    best_makespans.append(final_best_makespan)
                    best_security.append(final_best_security)
            
            # Plot the best solution trajectory
            if best_episodes:
                ax.plot(best_episodes, best_makespans, best_security, 
                    color='red', linestyle='-', linewidth=3, 
                    marker='o', markersize=8, label='Best Solution Path')
            
            # Highlight the final solution with a special marker and projections
            if best_episodes:
                final_x = best_episodes[-1]
                final_y = best_makespans[-1]
                final_z = best_security[-1]
                
                # Add a star marker for the final solution
                ax.scatter([final_x], [final_y], [final_z], 
                        color='gold', s=300, marker='*', 
                        edgecolor='black', linewidth=1.5,
                        label='Final Best Solution')
                
                # Add projection lines to each axis
                ax.plot([final_x, final_x], [final_y, final_y], [0, final_z], 'k--', alpha=0.5)
                ax.plot([final_x, final_x], [0, final_y], [final_z, final_z], 'k--', alpha=0.5)
                ax.plot([0, final_x], [final_y, final_y], [final_z, final_z], 'k--', alpha=0.5)
                
                # Add projection points on each plane
                ax.scatter([final_x], [final_y], [0], color='red', s=80, marker='o', alpha=0.7)
                ax.scatter([final_x], [0], [final_z], color='red', s=80, marker='o', alpha=0.7)
                ax.scatter([0], [final_y], [final_z], color='red', s=80, marker='o', alpha=0.7)
                
                # Add text annotations for final values
                ax.text(final_x, final_y, final_z * 1.05, 
                    f"Final: Episode {final_x}\nMakespan: {final_y:.1f}\nSecurity: {final_z:.2f}", 
                    fontsize=10, color='black', bbox=dict(facecolor='white', alpha=0.7))
            
            # Add an arrow pointing to the final solution
            if best_episodes:
                # Create arrow pointing to final solution
                arrow = Arrow3D([final_x - 30, final_x], 
                            [final_y + 20, final_y], 
                            [final_z + 0.3, final_z],
                            mutation_scale=15, lw=2, arrowstyle='-|>', color='black')
                ax.add_artist(arrow)
            
            # Add labels and title
            ax.set_xlabel('Episode', fontsize=12)
            ax.set_ylabel('Makespan', fontsize=12)
            ax.set_zlabel('Security Utility', fontsize=12)
            ax.set_title('Q-Learning Progress: Makespan vs Security over Episodes', fontsize=14)
            
            # Show min/max values on axes
            if best_episodes:
                min_makespan = min(best_makespans)
                max_security = max(best_security)
                ax.text(0, min_makespan, 0, f"Best Makespan: {min_makespan:.1f}", 
                    fontsize=10, color='darkred')
                ax.text(0, 0, max_security, f"Best Security: {max_security:.2f}", 
            ...
            cbar.set_label('Episode Number', fontsize=12)
            
            # Add legend
            ax.legend(fontsize=12, loc='upper right')
            
            # Optimize view angle for better visualization
            ax.view_init(elev=25, azim=-35)
            
            # Save the figure
            plt.savefig('qlearning_progress.png', dpi=300, bbox_inches='tight')
            print("\nEnhanced learning progress chart saved as 'qlearning_progress.png'")
            
            plt.show()
        except Exception as e:
            import traceback
            print(f"Error creating 3D chart: {e}")
    ...
    def display_execution_times(self):
        ...
        for algorithm, exec_time in self.execution_times.items():
            print(f"{algorithm}: {exec_time:.3f} seconds")
        print()
        ...
        for task in sorted_tasks:
            ...
            for message in related_messages:
                ...
                if message.assigned_security['confidentiality'] > 0:
                    protocol_idx = message.assigned_security['confidentiality']
                    conf_overhead += (self.security_service.overheads['confidentiality'][protocol_idx][task.assigned_processor - 1] 
                                    * message.size / 1000)  # Adjust for message size
                
                # Integrity overhead is data-dependent (χ2)
                if message.assigned_security['integrity'] > 0:
                    protocol_idx = message.assigned_security['integrity']
                    integ_overhead += (self.security_service.overheads['integrity'][protocol_idx][task.assigned_processor - 1] 
                                    * message.size / 1000)  # Adjust for message size
                
                # Authentication overhead is data-independent (χ1)
                if message.assigned_security['authentication'] > 0:
                    protocol_idx = message.assigned_security['authentication']
                    auth_overhead += self.security_service.overheads['authentication'][protocol_idx][task.assigned_processor - 1]
...
def create_tc_test_case():
    ...
    deadline = 2000  # As specified in the paper
...
def main():
    ...
    if q_makespan:
        ...
        q_scheduler.print_task_schedule()
    ...
    if q_makespan:
        ...
        print("TASK SECURITY DETAILS \n=========================")
        hsms.display_security_information("HSMS")
        shield.display_security_information("SHIELD")
        q_scheduler.display_security_information("ImprovedQLearningScheduler")
    plt.show()
