...
class QLSecurityScheduler(HSMS):
    ...
    """
    def __init__(self, tasks, messages, processors, network, security_service, deadline, 
                 alpha=0.1, gamma=0.9, epsilon=0.3, episodes=2000):
        super().__init__(tasks, messages, processors, network, security_service, deadline)
    ...
    def compute_reward(self, old_makespan, new_makespan, old_utility, new_utility):
        ...
        - If makespan stays below deadline, reward based on security utility improvement
        """
        # Check if deadline is violated
        if new_makespan > self.deadline:
            return -100  # Large negative reward for violating deadline
        
        # Reward based on security utility improvement
        utility_reward = (new_utility - old_utility) * 10
        
        # Reward based on makespan improvement (lower is better)
        makespan_reward = (old_makespan - new_makespan) * 5
        
        # Combine rewards with weighting
        return utility_reward + makespan_reward
    ...
    def train(self):
        ...
        for episode in range(self.episodes):
            ...
            if (current_makespan <= self.deadline and 
                (current_utility > best_security_utility or 
            ...
            if (episode + 1) % 100 == 0:
                print(f"Episode {episode+1}/{self.episodes} - "
                      ...
                      f"Best: {best_security_utility:.4f}")
