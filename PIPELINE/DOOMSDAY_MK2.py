...
class Solution:
    def __init__(self, scheduler=None, task_processor_map=None):
        self.scheduler = scheduler
        self.task_processor_map = task_processor_map  # Map of task_id to processor_id
        self.makespan = None
        self.security_utility = None
        self.fitness = None
    
    def deep_copy(self):
        """Create a deep copy of this solution"""
        new_solution = Solution()
        new_solution.scheduler = copy.deepcopy(self.scheduler)
        new_solution.task_processor_map = copy.deepcopy(self.task_processor_map)
        new_solution.makespan = self.makespan
        new_solution.security_utility = self.security_utility
        new_solution.fitness = self.fitness
        return new_solution
    
    def __str__(self):
        return f"Solution: Makespan={self.makespan:.2f}, Security={self.security_utility:.4f}, Fitness={self.fitness:.4f}"

class SecurityAwareGA:
    def __init__(self, tasks, messages, processors, network, security_service, deadline, 
                 population_size=500, generations=1000, mutation_rate=0.2, crossover_rate=0.8, k_param=0.5):
        self.tasks = tasks
        self.messages = messages
        self.processors = processors
        self.network = network
        ...
        self.k_param = k_param  # Parameter to balance makespan vs security utility (0=makespan priority, 1=security priority)
        
        # Setup initial HSMS solution for seeding
        self.initial_hsms = HSMS(self.tasks, self.messages, self.processors, self.network, self.security_service, self.deadline)
    ...
    def initialize_population(self):
        """Initialize population with the HSMS solution and random variations"""
        population = []
        
        # Run HSMS first to get the initial solution
        initial_makespan, initial_security = self.initial_hsms.run()
        
        if initial_makespan is None:
            print("Initial HSMS solution failed to meet deadline. Initializing with random solutions.")
            for _ in range(self.population_size):
                solution = self._create_random_solution()
                population.append(solution)
        else:
            # Add the HSMS solution as the first member of the population
            hsms_solution = Solution(self.initial_hsms, self._get_task_processor_map(self.initial_hsms))
            hsms_solution.makespan = initial_makespan
            hsms_solution.security_utility = initial_security
            hsms_solution.fitness = self._calculate_fitness(initial_makespan, initial_security)
            population.append(hsms_solution)
            
            # Add variations of the HSMS solution and random solutions
            for i in range(1, self.population_size):
                if i < self.population_size // 2:
                    # Create variations of the HSMS solution
                    solution = self._create_hsms_variation()
                else:
    ...
    def _get_task_processor_map(self, scheduler):
        """Extract task to processor mapping from a scheduler"""
        task_proc_map = {}
        for task in scheduler.tasks:
            if task.is_scheduled:
                task_proc_map[task.task_id] = task.assigned_processor
        return task_proc_map
    
    def _create_hsms_variation(self):
        """Create a variation of the HSMS solution"""
        # Create a deep copy of the initial HSMS scheduler
        variation_scheduler = copy.deepcopy(self.initial_hsms)
        
        # Get task processor map from the original solution
        task_proc_map = self._get_task_processor_map(self.initial_hsms)
        
        # Randomly change processor assignments for some tasks
        num_changes = random.randint(1, len(task_proc_map) // 3 + 1)  # Change 1 to 1/3 of tasks
        tasks_to_change = random.sample(list(task_proc_map.keys()), num_changes)
        
        for task_id in tasks_to_change:
            current_proc = task_proc_map[task_id]
            available_procs = [p.proc_id for p in self.processors]
            available_procs.remove(current_proc)
            if available_procs:  # If there are other processors available
                new_proc = random.choice(available_procs)
                task_proc_map[task_id] = new_proc
        
        # Create a new solution with the modified assignments
        solution = Solution(variation_scheduler, task_proc_map)
        self._evaluate_solution(solution)
        return solution
    
    def _create_random_solution(self):
        """Create a random solution"""
        # Create a deep copy of the initial tasks
        random_scheduler = HSMS(
            copy.deepcopy(self.tasks),
            copy.deepcopy(self.messages),
            copy.deepcopy(self.processors),
            copy.deepcopy(self.network),
            copy.deepcopy(self.security_service),
            self.deadline
        )
        
        # Create random processor assignments
        task_proc_map = {}
        for task in random_scheduler.tasks:
            task_proc_map[task.task_id] = random.choice([p.proc_id for p in self.processors])
        
        # Create a solution with these random assignments
        solution = Solution(random_scheduler, task_proc_map)
        self._evaluate_solution(solution)
        return solution
    
    def _evaluate_solution(self, solution):
        """Evaluate a solution by running the scheduler with the given task-processor map"""
        # Reset the scheduler state
        for task in solution.scheduler.tasks:
            task.is_scheduled = False
            task.assigned_processor = None
            task.start_time = None
            task.finish_time = None
        
        for proc in solution.scheduler.processors:
            proc.available_time = 0
        
        # Apply the task to processor mapping
        for task_id, proc_id in solution.task_processor_map.items():
            task = solution.scheduler.get_task_by_id(task_id)
            if task:
                task.assigned_processor = proc_id
        
        # Schedule the tasks with the fixed processor assignments
        try:
            makespan = self._schedule_with_fixed_assignments(solution.scheduler)
            security_utility = solution.scheduler.calculate_security_utility()
            
            solution.makespan = makespan
            solution.security_utility = security_utility
            solution.fitness = self._calculate_fitness(makespan, security_utility)
        except Exception as e:
            # In case of scheduling failure, assign poor fitness values
            print(f"Scheduling error: {e}")
            solution.makespan = float('inf')
            solution.security_utility = 0
            solution.fitness = 0
    
    def _schedule_with_fixed_assignments(self, scheduler):
        """Schedule tasks with pre-assigned processors"""
        # Compute task priorities (upward rank)
        scheduler.compute_task_priorities()
        
        # Assign minimum security levels
        scheduler.assign_minimum_security()
        
        # Sort tasks by priority (descending)
        sorted_tasks = sorted(scheduler.tasks, key=lambda t: -t.priority)
        
        # Schedule each task on its pre-assigned processor
        for task in sorted_tasks:
            if task.assigned_processor is None:
                continue  # Skip tasks without processor assignment
            
            processor_id = task.assigned_processor
            processor = scheduler.processors[processor_id - 1]
            
            # Calculate earliest start time (EST) for this task on this processor
            est = scheduler.calculate_est(task, processor_id)
            
            if est == float('inf'):
                raise ValueError(f"Cannot schedule task {task.task_id}: prerequisites not scheduled")
            ...
            # Set task timing
            task.start_time = est
            task.finish_time = est + base_exec_time  # Temporary finish time
            task.is_scheduled = True
            processor.available_time = task.finish_time
        ...
        # Recalculate with full security overhead
        return scheduler.finalize_schedule()
    
    def _calculate_fitness(self, makespan, security_utility):
        """Calculate fitness based on makespan and security utility"""
        if makespan > self.deadline:
            return 0  # Solutions that exceed deadline have zero fitness
        
        # Normalize makespan and security utility
        norm_makespan = 1 - (makespan / self.deadline)  # Lower makespan is better (max is 1.0)
        norm_security = security_utility / 10.0  # Assuming maximum possible security utility is around 10
        
        # Combine using the k parameter (k=0 -> makespan only, k=1 -> security only)
        fitness = (1 - self.k_param) * norm_makespan + self.k_param * norm_security
        
        return fitness
    
    def selection(self, population):
        """Tournament selection with elitism"""
        # Keep the top 10% as elites
        ...
        selected = []
        while len(selected) < len(population) - elite_count:
            tournament_size = 3
            tournament = random.sample(population, tournament_size)
            tournament.sort(key=lambda x: x.fitness, reverse=True)
            selected.append(tournament[0].deep_copy())
        
        # Combine elites and selected individuals
        return elites + selected
    
    def crossover(self, parent1, parent2):
        """Order-preserving crossover for task-processor mappings"""
        ...
        # Get list of task IDs ordered by priority
        task_ids = [task.task_id for task in sorted(parent1.scheduler.tasks, key=lambda t: -t.priority)]
        
        # Select crossover point
        crossover_point = random.randint(1, len(task_ids) - 1)
        
        # Create children by swapping processor assignments
        for i in range(crossover_point):
            task_id = task_ids[i]
            child1.task_processor_map[task_id] = parent2.task_processor_map[task_id]
            child2.task_processor_map[task_id] = parent1.task_processor_map[task_id]
    ...
    def mutate(self, solution):
        """Mutation operator that respects task ordering constraints"""
        ...
        # Select a random task to change its processor assignment
        task_id = random.choice(list(mutated.task_processor_map.keys()))
        current_proc = mutated.task_processor_map[task_id]
        
        # Find available processors excluding current one
        available_procs = [p.proc_id for p in self.processors if p.proc_id != current_proc]
        
        if available_procs:
            # Assign a new processor
            new_proc = random.choice(available_procs)
            mutated.task_processor_map[task_id] = new_proc
            
            # Re-evaluate the mutated solution
            self._evaluate_solution(mutated)
    ...
    def evolve_population(self, population):
        ...
        while i < len(population):
            if i + 1 < len(selected):
                ...
                # If we have an odd number, just mutate the last one
    ...
    def run(self):
        ...
        self.best_solution = best_solution.deep_copy()
        
        print(f"Initial best solution: {self.best_solution}")
        
        # Track history for first generation
        self._update_history(0, population)
        ...
        for generation in range(1, self.generations + 1):
            ...
            if current_best.fitness > self.best_solution.fitness:
                ...
                print(f"Generation {generation}: New best solution found!")
                print(f"  Makespan: {self.best_solution.makespan:.2f}")
                print(f"  Security: {self.best_solution.security_utility:.4f}")
                print(f"  Fitness: {self.best_solution.fitness:.4f}")
            
            # Update history
            self._update_history(generation, population)
            
            # Early stopping if we've reached a good solution
            if self.best_solution.makespan <= 0.85 * self.deadline and self.best_solution.security_utility > 7.0:
                print(f"Early stopping at generation {generation}: Good solution found")
                break
    ...
    def _update_history(self, generation, population):
        """Update history with current generation stats"""
        # Calculate stats
        valid_solutions = [s for s in population if s.makespan <= self.deadline]
        
        if valid_solutions:
            avg_makespan = sum(s.makespan for s in valid_solutions) / len(valid_solutions)
            avg_security = sum(s.security_utility for s in valid_solutions) / len(valid_solutions)
        else:
            avg_makespan = float('inf')
            avg_security = 0
            
        best_makespan = min([s.makespan for s in population])
        best_security = max([s.security_utility for s in population if s.makespan <= self.deadline] or [0])
        
        # Update history
        self.history['generation'].append(generation)
        self.history['best_makespan'].append(best_makespan)
        self.history['best_security'].append(best_security)
        self.history['avg_makespan'].append(avg_makespan)
        self.history['avg_security'].append(avg_security)
    
    def visualize_results(self):
        """Visualize the results of the GA optimization"""
        self._plot_history_2d()
        self._plot_history_3d()
    
    def _plot_history_2d(self):
        """Plot 2D graphs of makespan and security utility over generations"""
        plt.figure(figsize=(12, 8))
        
        # Plot makespan vs generation
        plt.subplot(2, 1, 1)
        plt.plot(self.history['generation'], self.history['best_makespan'], 'b-', label='Best Makespan')
        plt.plot(self.history['generation'], self.history['avg_makespan'], 'b--', label='Avg Makespan')
        plt.axhline(y=self.deadline, color='r', linestyle='-', label='Deadline')
        plt.xlabel('Generation')
        plt.ylabel('Makespan (ms)')
        plt.title('Makespan vs Generation')
        plt.legend()
        plt.grid(True)
        
        # Plot security utility vs generation
        plt.subplot(2, 1, 2)
        plt.plot(self.history['generation'], self.history['best_security'], 'g-', label='Best Security')
        plt.plot(self.history['generation'], self.history['avg_security'], 'g--', label='Avg Security')
        plt.xlabel('Generation')
        plt.ylabel('Security Utility')
        plt.title('Security Utility vs Generation')
        plt.legend()
        plt.grid(True)
    ...
    def _plot_history_3d(self):
        """Plot 3D graph of makespan vs security utility vs generation"""
        fig = plt.figure(figsize=(10, 8))
        ax = fig.add_subplot(111, projection='3d')
        
        generations = self.history['generation']
        makespan = self.history['best_makespan']
        security = self.history['best_security']
        
        # Create the 3D scatter plot
        sc = ax.scatter(generations, makespan, security, c=generations, cmap='viridis', 
                       marker='o', s=50, alpha=0.8)
        
        # Add lines to connect points
        ax.plot(generations, makespan, security, color='black', alpha=0.3)
        
        # Add projections
        ax.plot(generations, makespan, zs=min(security)-0.5, zdir='z', color='blue', alpha=0.3)
        ax.plot(generations, [min(makespan)-50]*len(generations), security, zdir='y', color='green', alpha=0.3)
        ...
        ax.set_title('3D Plot of Makespan vs Security Utility vs Generation')
        
        # Add deadline plane
        x_range = np.array([min(generations), max(generations)])
        y_range = np.array([self.deadline, self.deadline])
        z_range = np.array([[min(security)-0.5, max(security)+0.5], 
                           [min(security)-0.5, max(security)+0.5]])
        
        ax.plot_surface(
            np.array([[x_range[0], x_range[0]], [x_range[1], x_range[1]]]),
    ...
    def save_results_to_file(self, filename='ga_results.txt'):
        ...
        with open(filename, 'w') as f:
            ...
            f.write(f"  K Parameter: {self.k_param}\n")
            f.write(f"  Deadline: {self.deadline} ms\n\n")
            
            f.write(f"Initial HSMS Solution:\n")
            f.write(f"  Makespan: {self.initial_hsms.makespan:.2f} ms\n")
            security_utility = self.initial_hsms.calculate_security_utility()
            f.write(f"  Security Utility: {security_utility:.4f}\n\n")
            
            f.write(f"Best GA Solution:\n")
            f.write(f"  Makespan: {self.best_solution.makespan:.2f} ms\n")
            f.write(f"  Security Utility: {self.best_solution.security_utility:.4f}\n")
            f.write(f"  Fitness: {self.best_solution.fitness:.4f}\n\n")
            
            f.write("Task Assignments:\n")
            f.write(f"{'Task ID':<8} {'Task Name':<15} {'Processor':<10} {'Start Time':<12} {'Finish Time':<12}\n")
            f.write("-" * 60 + "\n")
            
            for task in sorted(self.best_solution.scheduler.tasks, key=lambda t: t.task_id):
                if task.is_scheduled:
                    f.write(f"{task.task_id:<8} {task.name:<15} {task.assigned_processor:<10} "
                           f"{task.start_time:<12.2f} {task.finish_time:<12.2f}\n")
            
            f.write("\nSecurity Assignments:\n")
            f.write(f"{'Message':<15} {'Source->Dest':<15} {'Conf':<6} {'Integ':<6} {'Auth':<6}\n")
            f.write("-" * 50 + "\n")
            
            for msg in self.best_solution.scheduler.messages:
                source_task = self.best_solution.scheduler.get_task_by_id(msg.source_id)
                dest_task = self.best_solution.scheduler.get_task_by_id(msg.dest_id)
                
                if source_task and dest_task and source_task.is_scheduled and dest_task.is_scheduled:
                    source_dest = f"{source_task.name}->{dest_task.name}"
                    f.write(f"{msg.id:<15} {source_dest:<15} {msg.assigned_security['confidentiality']:<6} "
                           f"{msg.assigned_security['integrity']:<6} {msg.assigned_security['authentication']:<6}\n")
...
def main():
    """Main function to run and test the GA Security-Aware Scheduler"""
...
# Run HSMS
    ...
    # Create and run GA with different k_param values
    k_values = [1.0]
    ga_results = []
    
    for k in k_values:
        print(f"\nRunning GA with k={k} (makespan vs security balance)...")
        ga = SecurityAwareGA(
            copy.deepcopy(tasks), 
            copy.deepcopy(messages), 
            copy.deepcopy(processors), 
            copy.deepcopy(network), 
            copy.deepcopy(security_service), 
            deadline,
            population_size=500, 
            generations=20,
            mutation_rate=0.2, 
        ...
        ga.save_results_to_file(f"ga_results_k{k}.txt")
        # ga.display_security_information("GA SCHEDULE")
        # ga.print_task_schedule()
        # plot_gantt_chart("Q-Learning Schedule", ga.schedule, len(processors), ql_makespan)
        # plt.savefig("ql_schedule.png")
        # ga.display_task_security_details()
        # ga.calculate_and_print_security_details()  # Add this line
        # ga.visualize_2d_plots()
        # ga.visualize_3d_plot()
        ga_results.append((k, best_solution, ga))
    
    # Compare results across different k values
    print("\n" + "="*80)
    print("Comparing results across different k values")
    print("="*80)
    print(f"{'k Value':<10} {'Makespan (ms)':<15} {'Security Utility':<20} {'Improvement over HSMS':<25}")
    print("-"*70)
    
    # for k, solution, _ in ga_results:
    #     makespan_improvement = (hsms_makespan - solution.makespan) / hsms_makespan * 100
    #     security_improvement = (solution.security_utility - hsms_security) / hsms_security * 100
        
    #     improvement_text = f"M: {makespan_improvement:.2f}%, S: {security_improvement:.2f}%"
    #     print(f"{k:<10.1f} {solution.makespan:<15.2f} {solution.security_utility:<20.4f} {improvement_text:<25}")
    
    # # Run detailed analysis on the best balanced solution (k=0.5)
    # _, best_balanced, ga_balanced = ga_results[2]  # k=0.5 is at index 2
    
    # print("\nDetailed analysis of balanced solution (k=0.5):")
    # print("\nGA Task Schedule:")
    # best_balanced.scheduler.print_task_schedule()
    
    # print("\nGA Security Information:")
    # best_balanced.scheduler.display_security_information("GA (k=0.5)")
    
    # print("\nGA Security Overhead Analysis:")
    # best_balanced.scheduler.calculate_and_print_security_details()
    
    # # Plot comparison of HSMS vs GA
    # plot_comparison(hsms_scheduler, ga_results)
    
    # print("\n" + "="*80)
    # print("Optimization complete. Check generated visualization and text files for detailed results.")
    # print("="*80)

def plot_comparison(hsms_scheduler, ga_results):
    """Plot comparison between HSMS and GA solutions"""
    # Collect data
    k_values = [result[0] for result in ga_results]
    makespan_values = [result[1].makespan for result in ga_results]
    security_values = [result[1].security_utility for result in ga_results]
    ...
    plt.subplot(2, 1, 1)
    plt.plot(k_values, makespan_values, 'bo-', label='GA Solutions')
    plt.axhline(y=hsms_makespan, color='r', linestyle='--', label='HSMS')
    plt.xlabel('k Parameter')
    plt.ylabel('Makespan (ms)')
    plt.title('Makespan Comparison: HSMS vs GA')
    plt.grid(True)
    plt.legend()
    
    # Plot security comparison
    plt.subplot(2, 1, 2)
    plt.plot(k_values, security_values, 'go-', label='GA Solutions')
    plt.axhline(y=hsms_security, color='r', linestyle='--', label='HSMS')
    plt.xlabel('k Parameter')
    plt.ylabel('Security Utility')
    plt.title('Security Utility Comparison: HSMS vs GA')
    plt.grid(True)
    plt.legend()
    ...
    # Create a pareto front plot (makespan vs security)
    plt.figure(figsize=(8, 6))
    
    # Plot HSMS point
    plt.scatter(hsms_makespan, hsms_security, color='red', s=100, marker='*', label='HSMS')
    
    # Plot GA points
    for i, (k, solution, _) in enumerate(ga_results):
        plt.scatter(solution.makespan, solution.security_utility, color='blue', s=50)
        plt.annotate(f"k={k}", (solution.makespan, solution.security_utility), 
                    xytext=(5, 5), textcoords='offset points')
    
    # Connect the points to visualize the pareto front
    pareto_makespan = [hsms_makespan] + makespan_values
    pareto_security = [hsms_security] + security_values
    
    # Sort points by makespan to ensure proper line connection
    combined = sorted(zip(pareto_makespan, pareto_security))
    pareto_makespan, pareto_security = zip(*combined)
    
    plt.plot(pareto_makespan, pareto_security, 'b--', alpha=0.5)
    
    plt.xlabel('Makespan (ms) - Lower is better')
    plt.ylabel('Security Utility - Higher is better')
    plt.title('Pareto Front: Makespan vs Security Utility')
    plt.grid(True)
    plt.legend()
