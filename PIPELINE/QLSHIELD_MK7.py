                ...
                if ready_tasks:
                    ...
                    for task in sorted_tasks:
                        if not task.is_scheduled and task not in ready_tasks:
                            ...
                            if pred_finish_times and not all(self.get_task_by_id(pred_id).is_scheduled 
                                                         for pred_id in task.predecessors):
                                next_times.append(max(pred_finish_times))
                                
                    if next_times:
                        current_time = min(next_times)
                    else:
                        # If we can't find any upcoming times, we might be done
                        all_scheduled = all(task.is_scheduled for task in self.tasks)
                        if not all_scheduled:
                            # Just increment time by a small amount to avoid getting stuck
                            current_time += 1
